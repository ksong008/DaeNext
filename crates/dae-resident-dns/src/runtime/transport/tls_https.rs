...
pub async fn forward_dns_tls_async(
...
async fn return_tls_stream_to_pool(
    pool: &AsyncMutex<Vec<ResidentDnsTlsStream>>,
    stream: ResidentDnsTlsStream,
) {
    let mut idle = pool.lock().await;
    if idle.len() < DNS_STREAM_POOL_MAX_IDLE {
        idle.push(stream);
...
pub async fn open_dns_boring_tls_stream_async(
    upstream: &ResidentDnsUpstream,
    stream: TokioTcpStream,
    alpn: &[&str],
    context: ProxyDnsRequestContext,
    ...
    .map_err(|err| {
        format!(
            "connect DNS BoringSSL upstream {} {}: {err}",
...
pub async fn forward_dns_https_async(
    upstream: &ResidentDnsUpstream,
    payload: &[u8],
    plan: &ResidentDnsPlan,
    forwarders: &Arc<ResidentDnsForwarderCache>,
...
mod tests {
    use super::*;
    use dae_resident_core::RESIDENT_IDLE_SLEEP;

...
use dae_resident_core::{
    ResidentDataplaneMetrics, ResidentUdpPayloadAdmission, ResidentUdpPayloadPermit,
};

use super::super::*;

pub const DNS_UDP_REQUEST_ID_SPACE: usize = (u16::MAX as usize) + 1;
mod actor;
mod executor;
mod id_allocator;
...
pub use self::executor::ResidentDnsUdpActorExecutor;
pub use self::id_allocator::DnsRequestIdAllocator;

#[derive(Clone)]
pub struct ResidentDnsUdpMultiplexHandle {
...
pub struct ResidentDnsUdpActorLifecycle {
...
pub struct ResidentDnsUdpActorCompletion {
    finished: AtomicBool,
...
pub struct ResidentDnsUdpActorRegistration<T> {
    pub handle: T,
    pub lifecycle: Weak<ResidentDnsUdpActorLifecycle>,
    pub completion: Arc<ResidentDnsUdpActorCompletion>,
...
#[cfg(all(test, feature = "dns-runtime-tests"))]
pub async fn open_udp_multiplex_handle(
    target: SocketAddr,
    mark: u32,
...
impl ResidentDnsUdpMultiplexHandle {
    ...
    pub fn record_recreated(&self) {
        self.metrics.dns_udp_forwarder_recreated();
    }

    #[cfg(all(test, feature = "dns-runtime-tests"))]
    pub async fn exchange(&self, payload: &[u8]) -> Result<Vec<u8>, String> {
    ...
    pub async fn exchange_once_until(
        &self,
        payload: &[u8],

...
pub fn socks5_dns_proxy(address: SocketAddr) -> Arc<ResidentProxyPlan> {
...
pub fn dns_proxy_binding(proxy: Arc<ResidentProxyPlan>, generation: u64) -> ResidentProxyBinding {
    if generation == 0 {
...
#[derive(Clone, Copy)]
pub enum DnsQuicTestProtocol {
    Doq,
    Doh3,
}

pub struct DnsQuicTestServer {
    address: SocketAddr,
    certificate: Vec<u8>,
...
pub fn dns_test_response(bytes: usize, marker: u8) -> Vec<u8> {
...
pub fn dns_a_test_response(query: &[u8], address: [u8; 4]) -> Vec<u8> {

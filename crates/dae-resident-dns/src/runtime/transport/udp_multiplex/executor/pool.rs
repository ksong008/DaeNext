...
pub struct ResidentDnsUdpActorPool {
    runtime: std::sync::Mutex<Option<tokio::runtime::Runtime>>,
}

impl ResidentDnsUdpActorPool {
    pub fn new(worker_count: usize, worker_stack_bytes: usize) -> Result<Self, String> {
    ...
    pub fn runtime_handle(&self) -> Result<tokio::runtime::Handle, String> {
        self.runtime
            .lock()
    ...
    pub async fn shutdown(&self, deadline: tokio::time::Instant) -> Result<(), String> {
        let runtime = self
            .runtime
            .lock()
            .map_err(|_| "shared DNS UDP actor runtime lock poisoned".to_owned())?

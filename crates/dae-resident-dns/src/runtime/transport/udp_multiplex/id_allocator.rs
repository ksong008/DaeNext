...
impl DnsRequestIdAllocator {
    ...
    pub fn allocate(&mut self, capacity: usize) -> Result<u16, String> {
        self.allocate_at(capacity, time::Instant::now())
    }

    pub fn allocate_at(&mut self, capacity: usize, now: time::Instant) -> Result<u16, String> {
    ...
    pub fn release_at(&mut self, id: u16, now: time::Instant) {
    ...
    pub fn is_occupied(&self, id: u16) -> bool {
    ...
    pub fn is_quarantined(&self, id: u16) -> bool {
        let (word, bit) = dns_request_id_bitmap_slot(id);
    ...
    pub fn reap_quarantine(&mut self, now: time::Instant) {
        while self

...
pub enum ResidentDnsTargetRefreshError {
    Deadline,
    Resolver(String),
}
...
#[derive(Debug)]
pub struct ResidentDnsResolvedTargetCache {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ResidentDnsResolvedTargetSnapshot {
...
impl ResidentDnsResolvedTargetSnapshot {
    pub fn literal(addr: SocketAddr) -> Self {
        Self {
            addrs: Arc::from([addr]),
            epoch: 0,
            stale: false,
...
impl ResidentDnsResolvedTargetCache {
    ...
    pub fn install_refresh_handle(&self, handle: ResidentDnsTargetRefreshHandle) {
    ...
    pub async fn refresh_after_stale_failure_and_resolve<F, Fut>(
        &self,
        snapshot: &ResidentDnsResolvedTargetSnapshot,
    ...
    pub fn complete_background_refresh(&self, result: Result<ResolvedHostAddrs, String>) {
        match result {
            Ok(resolved) => {
    ...
    pub fn cancel_background_refresh(&self) {

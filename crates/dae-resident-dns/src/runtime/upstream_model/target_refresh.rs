...
pub type ResidentDnsTargetRefreshOwnerTask = Pin<Box<dyn Future<Output = ()> + Send + 'static>>;
...
pub struct ResidentDnsTargetRefreshRequest {
...
#[derive(Clone, Debug)]
pub struct ResidentDnsTargetRefreshHandle {
    sender: mpsc::Sender<ResidentDnsTargetRefreshRequest>,
}

impl ResidentDnsTargetRefreshHandle {
    pub fn try_schedule<F, Fut>(&self, cache: &Arc<ResidentDnsResolvedTargetCache>, resolver: F)
    where
...
pub struct ResidentDnsTargetRefreshOwner {

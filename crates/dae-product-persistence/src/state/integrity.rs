...
#[derive(Debug)]
pub struct StateIntegritySnapshot {
...
pub fn state_schema_version(conn: &Connection) -> io::Result<i64> {
    conn.query_row("PRAGMA user_version", [], |row| row.get::<_, i64>(0))
        .map_err(sqlite_io_error)
}

pub fn reject_newer_state_schema(version: i64) -> io::Result<()> {
...
pub fn validate_state_connection_read_only(
    conn: &Connection,
) -> io::Result<StateIntegritySnapshot> {
    inspect_state_connection_read_only(conn, true)
}

pub fn inspect_state_connection_read_only(
    conn: &Connection,
...
pub fn state_check_report(state: &Path) -> io::Result<Value> {

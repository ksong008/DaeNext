...
fn test_hash_password(salt: &[u8], password: &str) -> String {
    let salt_digest = Sha256::digest(salt);
    let salt = SaltString::encode_b64(&salt_digest[..16]).unwrap();
    Argon2::default()
        ...
        callers.push(thread::spawn(move || {
            ...
            runtime.execute(
                ...
                move || {
                    let current = active.fetch_add(1, Ordering::SeqCst) + 1;
                    update_max(&maximum, current);
                    let hash = test_hash_password(&[index; 16], &format!("Fixture9-{index}"));
                    active.fetch_sub(1, Ordering::SeqCst);

...
use dae_product_http::HttpResponse;
...
pub struct ProductAuthJobOutcome {
    response: HttpResponse,
    attempt: ProductAuthAttemptOutcome,
}

impl ProductAuthJobOutcome {
    pub fn success(response: HttpResponse) -> Self {
        Self {
            response,
    ...
    pub fn credential_failure(response: HttpResponse) -> Self {
        Self {
            response,
    ...
    pub fn neutral(response: HttpResponse) -> Self {
        Self {
            response,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ProductAuthExecutionError {
    Busy { retry_after: Duration },
    Unavailable,
    TimedOut,
}

pub struct ProductAuthRuntime {
...
impl ProductAuthRuntime {
    ...
    pub fn start_for_test_config() -> io::Result<Arc<Self>> {
        Self::start(ProductAuthRuntimeConfig::for_test())
    }
...
pub fn product_auth_defaults_json() -> Value {
    auth_defaults_json()
}

...
pub struct UdpSessionFixedTarget {
    source: Option<SocketAddr>,
}

impl UdpSessionFixedTarget {
    pub fn bind(&mut self, source: SocketAddr, owner: &str) -> Result<(), String> {
        match self.source {
            None => {
...
pub struct UdpResponseIdentityToken([u8; 32]);
...
impl UdpResponseIdentityToken {
    ...
    pub fn from_protocol_identity(domain: &[u8], identity: &[u8]) -> Option<Self> {
...
pub struct UdpFixedTargetExpectation {
    source: SocketAddr,
...
impl UdpFixedTargetExpectation {
    pub const fn compatibility(source: SocketAddr) -> Self {
        Self {
            source,
    ...
    pub const fn decoded_source(source: SocketAddr) -> Self {
        Self {
            source,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum UdpResponseIdentityEvidence {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum UdpFixedTargetValidation {
    Validated,
    CompatibilityUnverified,
...
pub enum UdpFixedTargetPayload {
    Accepted {
        payload: Vec<u8>,
...
impl UdpFixedTargetPayload {
    pub const fn validation(&self) -> UdpFixedTargetValidation {
        match self {
    ...
    pub fn into_payload(self) -> Result<Vec<u8>, UdpResponseDropReason> {
        match self {
...
impl UdpFixedTargetValidation {
    ...
    pub const fn drop_reason(self) -> Option<UdpResponseDropReason> {
        match self {

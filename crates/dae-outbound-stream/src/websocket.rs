...
pub fn read_websocket_binary_frame(stream: &mut impl Read) -> Result<Vec<u8>, OutboundError> {
    ...
    if len == 126 {
        ...
        return Err(OutboundError::BadSharedTransport(
            "websocket 64-bit length unsupported in stream codec".to_owned(),
        ));
    }
    let mut mask_key = [0_u8; 4];
    if masked {
...
pub fn validate_http_field(value: &str, context: &str) -> Result<(), OutboundError> {
...
mod tests {
    ...
    fn websocket_accept_matches_rfc_sample() {
        assert_eq!(websocket_accept_for_key(DEFAULT_WS_KEY), WS_ACCEPT_SAMPLE);
    }

    #[test]
    fn websocket_codec_rejects_http_control_characters() {
        assert!(validate_http_field("bad\r\nheader", "host").is_err());
        assert!(
            websocket_client_binary_frame(&vec![0; u16::MAX as usize + 1], WS_MASK_KEY).is_err()
        );
    }
}

...
pub fn read_grpc_http2_request(
    ...
    let service_name = options.service_name_or_default();
    let decoded = crate::hpack_decode::decode_header_block(&headers.payload)?;
    ...
    if !crate::hpack_decode::semantic_headers_match(&decoded, expected) {
        return Err(OutboundError::BadSharedTransport(
...
fn push_hpack_string(out: &mut Vec<u8>, value: &[u8]) {
    crate::hpack::push_hpack_string(out, value);
}

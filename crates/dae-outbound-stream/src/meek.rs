...
pub fn meek_http_request(
    ...
    crate::websocket::validate_http_field(&options.host, "meek HTTP host")?;
    crate::websocket::validate_http_field(&options.path, "meek HTTP path")?;
    Ok(format!(
        "POST {} HTTP/1.1\r\nHost: {}\r\nX-Session-ID: {}\r\nContent-Length: {}\r\n\r\n",
        options.path,
        options.host,
...
fn read_http_response_body(stream: &mut TcpStream) -> Result<Vec<u8>, OutboundError> {
    ...
    let content_length = bounded_http_message_length(content_length(&head)?)?;
...
fn bounded_http_message_length(length: usize) -> Result<usize, OutboundError> {
    const MAX_HTTP_MESSAGE_BODY_BYTES: usize = 1024 * 1024;
    if length > MAX_HTTP_MESSAGE_BODY_BYTES {
        return Err(OutboundError::BadSharedTransport(format!(
            "meek response body too large: {length} bytes (max {MAX_HTTP_MESSAGE_BODY_BYTES})"
        )));
    }
    Ok(length)
}

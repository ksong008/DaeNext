...
fn parse_duration(input: &str) -> Result<ConfigDuration, ParseDurationError> {
    if input.is_empty() {
    ...
    Ok(ConfigDuration::from_nanos(nanos))
}
...
mod tests {
    ...
    #[test]
    fn parses_and_formats_basic_config_durations() {
        assert_eq!("30s".parse::<ConfigDuration>().unwrap().to_string(), "30s");
        assert_eq!(
            "1m30s".parse::<ConfigDuration>().unwrap().to_string(),
            "1m30s"
        );
        assert_eq!(
            "1h".parse::<ConfigDuration>().unwrap().to_string(),
            "1h0m0s"
        );
        assert_eq!("5s".parse::<ConfigDuration>().unwrap().to_string(), "5s");
        assert!("30".parse::<ConfigDuration>().is_err());
    }
}

...
pub struct XhttpXmuxManagerLifecycle {
    generation: u64,
    closing: AtomicBool,
    opening: AtomicUsize,
...
pub struct XhttpXmuxOpeningLease {
    lifecycle: Arc<XhttpXmuxManagerLifecycle>,
    generation: u64,
}

pub struct XhttpXmuxManagerHandle<M> {
    pub manager: Arc<tokio::sync::Mutex<M>>,
    pub lifecycle: Arc<XhttpXmuxManagerLifecycle>,
}
...
impl XhttpXmuxManagerLifecycle {
    ...
    pub fn reserve_opening(self: &Arc<Self>) -> Option<XhttpXmuxOpeningLease> {
        if self.is_closing() {
            return None;
        }
    ...
    pub fn signal(&self) -> XhttpXmuxStateSignal {
...
impl<M> XhttpXmuxManagerHandle<M> {
    pub fn new(build: impl FnOnce(Arc<XhttpXmuxManagerLifecycle>) -> M) -> Self {

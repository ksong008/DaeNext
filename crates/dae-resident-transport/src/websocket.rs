...
pub use duplex_control::*;
pub use framing::RESIDENT_WEBSOCKET_MAX_MESSAGE_BYTES;
pub use framing::WebSocketBinaryFrameDecoder;

pub const RESIDENT_WEBSOCKET_RELAY_BUFFER_SIZE: usize = 16 * 1024;

pub async fn websocket_handshake_over_resident_tls_async(
...
pub async fn httpupgrade_handshake_over_resident_tls_async(
    client: &mut AsyncVlessTlsClient,
    options: &HttpUpgradeOptions,
) -> Result<Vec<u8>, String> {
...
pub async fn websocket_handshake_over_async_stream<S>(
    stream: &mut S,
    options: &HttpUpgradeOptions,
) -> Result<Vec<u8>, String>
where
...
pub async fn httpupgrade_handshake_over_async_stream<S>(
    stream: &mut S,
    options: &HttpUpgradeOptions,
) -> Result<Vec<u8>, String>
where
...
pub async fn write_websocket_binary_frame_to_async_stream<S>(
    stream: &mut S,
    payload: &[u8],
    label: &str,
) -> Result<(), String>
...
pub async fn write_websocket_binary_frame_in_place_to_async_stream<S>(
    stream: &mut S,
    payload: &mut [u8],
    label: &str,
) -> Result<(), String>
...
pub async fn write_websocket_binary_frame_over_resident_tls_async(
    client: &mut AsyncVlessTlsClient,
    payload: &[u8],
    label: &str,
) -> Result<(), String> {
...
pub fn spawn_websocket_payload_stream<S>(client: S, leftover: Vec<u8>) -> SpawnedLogicalStream
where

...
#[derive(Default)]
pub struct AsyncWebSocketPayloadState {
...
pub struct AsyncWebSocketPayloadReader<'a, 'b, R> {
    client: &'a mut R,
...
{
    pub fn new(client: &'a mut R, state: &'b mut AsyncWebSocketPayloadState) -> Self {
        Self { client, state }
    }
}
...
pub struct AsyncWebSocketPayloadChannelState {
    decoder: WebSocketBinaryFrameDecoder,
    pending: CursorByteBuffer,
...
pub struct AsyncWebSocketPayloadChannelReader<'a, 'b, R> {
    client: &'a mut R,
...
where
    R: AsyncRead + Unpin,
{
    pub fn new(client: &'a mut R, state: &'b mut AsyncWebSocketPayloadChannelState) -> Self {
        Self { client, state }
    }
}
...
#[derive(Default)]
pub struct AsyncWebSocketControlWriter {
    pending: CursorByteBuffer,
    needs_flush: bool,
}

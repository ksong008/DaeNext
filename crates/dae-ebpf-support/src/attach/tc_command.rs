...
impl TcBpfAttachSpec {
    ...
    pub fn native_attach_spec(
        ...
        TcNativeAttachSpec {
            ...
            priority,
            handle,
            tcx_order: TcxAttachOrder::from_tc_priority(priority),
            protocol: ETH_P_ALL,
    ...
    pub fn attach_backend_report(
        ...
        let effective_backend = if native_backend_requested {
            plan.selected
        } else {
            Some(AttachBackend::TcCommand)
        };
        TcAttachBackendReport {
            ...
            command_backend_attempted: plan.attempt_order.contains(&AttachBackend::TcCommand),
            command_backend_required: effective_backend == Some(AttachBackend::TcCommand),
            command_backend_spec: self.filter_add_command(),
            cleanup_command_backend_spec: self.filter_del_command(),
            show_command_backend_spec: self.filter_show_command(true),
            plan,
            native_spec,
            effective_backend,

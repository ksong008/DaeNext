...
pub fn pin_aya_loaded_object_for_native_runtime(
    loaded: &mut AyaUserspaceLoadedObject,
    native_pin_root: &Path,
) -> Result<AyaNativeRuntimePinReport, String> {
    let map_pin_root = native_pin_root.join("maps");
    let program_pin_root = native_pin_root.join("programs");
    fs::create_dir_all(&map_pin_root)
        .map_err(|err| format!("create native runtime pinning map pin root failed: {err}"))?;
    fs::create_dir_all(&program_pin_root)
        .map_err(|err| format!("create native runtime pinning program pin root failed: {err}"))?;
    ...
    for (name, map) in loaded.ebpf.maps() {
        ...
        map.pin(&path)
            .map_err(|err| format!("pin map {name} for native runtime pinning failed: {err:?}"))?;
    ...
    if !missing_maps.is_empty() {
        return Err(format!(
            "native runtime pinning missing loaded catalog maps: {}",
            missing_maps.join(",")
        ));
    }
    ...
    for (name, program) in loaded.ebpf.programs_mut() {
        ...
        ensure_program_loaded_for_native_runtime(&name, program)?;
        let path = program_pin_root.join(&name);
        remove_existing_pin(&path)?;
        program.pin(&path).map_err(|err| {
            format!("pin program {name} for native runtime pinning failed: {err:?}")
        })?;
    ...
    Ok(AyaNativeRuntimePinReport {
        native_pin_root: native_pin_root.to_owned(),
        map_pin_root,
        program_pin_root,
        maps,
        programs,

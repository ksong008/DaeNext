...
#[test]
pub(super) fn runtime_dependency_plan_keeps_default_env_gates() {
    let plan = RuntimeDependencyPlan::default_env_gates();
...
#[test]
pub(super) fn control_api_typed_report_covers_formal_surfaces_with_current_schema() {
    ...
    assert!(report.current_report_schema);
}

#[test]
pub(super) fn runtime_state_report_requires_all_rust_owned_surfaces_for_product_control_plane() {
    let empty = RuntimeStateReport::new();
    assert!(empty.api_compatible);
    assert!(!empty.ready_for_product_control_plane());

    let ready = RuntimeStateReport::rust_owned_control_plane();
    assert_eq!(ready.schema_version, RuntimeStateReport::SCHEMA_VERSION);
    assert!(ready.ready_for_product_control_plane());

    let mut missing_active = ready;
    missing_active.active_handoff_available = false;
    assert!(!missing_active.ready_for_product_control_plane());
}

#[test]
pub(super) fn control_plane_product_admission_requires_native_tproxy_contract() {
    let ready = RuntimeStateReport::rust_owned_control_plane();
    let admission = ControlPlaneProductAdmission {
        runtime: ready,
        ...
        cleanup_passed: true,
        restore_passed: true,
        native_tproxy_contract_ready: true,
    };
    ...
    let mut missing_native_tproxy = admission;
    missing_native_tproxy.native_tproxy_contract_ready = false;
    assert!(!missing_native_tproxy.admitted());
}

...
#[test]
fn runtime_runner_outbound_socks5_commands_match_fixture() {
    let fixture = load("outbound/protocol/socks5_rust_native.json");
...
#[test]
fn runtime_runner_outbound_http_commands_match_fixture() {
    let fixture = load("outbound/protocol/http_rust_native.json");
    ...
    let (proxy, handle) = spawn_fake_http_proxy(
        "CONNECT front.fixture.invalid HTTP/1.1",
    ...
    let (proxy, handle) = spawn_fake_http_proxy(
        "PUT http://www.fixture.invalid/proxy-path HTTP/1.1",
        Some("Basic dXNlcjpwYXNz"),
    );

...
#[test]
fn runtime_runner_userspace_commands_match_engine_fixture() {
    let fixture = load("engine/userspace_runtime/runtime_contract.json");
...
#[test]
fn runtime_runner_active_datapath_commands_match_control_fixture() {
    let fixture = load("control/active_datapath/runtime_contract.json");
    ...
    assert!(
        contract_json["reload_restore_injects_previous_bpf"]
            .as_bool()
            .unwrap()
    );
    assert!(
    ...
    assert!(
        reload_json["reload_restore_injects_previous_bpf"]
            .as_bool()
            .unwrap()
    );

...
mod raw_tcp;
pub use self::raw_tcp::*;

pub type ResidentDuplexDirectionFuture<'a> =
    Pin<Box<dyn Future<Output = Result<(), String>> + Send + 'a>>;
pub type ResidentDuplexDirection<'a> =
    Pin<&'a mut (dyn Future<Output = Result<(), String>> + Send + 'a)>;

#[derive(Clone)]
pub struct ResidentDuplexProgress {
    inner: Arc<ResidentDuplexProgressInner>,
}

pub struct ResidentDuplexActivity {
...
pub fn resident_duplex_progress() -> (ResidentDuplexProgress, ResidentDuplexActivity) {
...
impl ResidentDuplexProgress {
    ...
    pub fn snapshot(&self) -> DirectTcpRelayStats {
        DirectTcpRelayStats {
...
pub async fn run_resident_duplex_relay(
...
pub async fn run_resident_duplex_relay_borrowed(

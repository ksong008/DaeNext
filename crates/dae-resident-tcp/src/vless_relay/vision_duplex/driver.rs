...
pub const VISION_PENDING_UPLINK_LIMIT: usize = TLS_RECORD_MAX_PAYLOAD_LEN * 4;
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum VisionDriverEvent {
    Progress,
    Complete,
}

pub struct VisionDuplexDriver {
    user_uuid: [u8; 16],
    stats: RelayStats,
...
impl VisionDuplexDriver {
    #[cfg(test)]
    pub fn new(user_uuid: [u8; 16], initial_payload: Vec<u8>) -> Result<Self, String> {
        Self::new_with_response_prefix(user_uuid, initial_payload, Vec::new())
    }

    pub fn new_with_response_prefix(
        user_uuid: [u8; 16],
    ...
    pub fn inbound_closed(&self) -> bool {
        self.inbound_closed
    }

    #[cfg(test)]
    pub fn uplink_state(&self) -> VisionUplinkState {
        self.uplink_state
    }

    #[cfg(test)]
    pub fn downlink_direct(&self) -> bool {
        self.downlink_state == VisionDownlinkState::DirectPass
    }

    #[cfg(test)]
    pub fn pending_uplink_input_len(&self) -> usize {
        self.pending_uplink_input.len()
    }

    #[cfg(test)]
    pub fn force_uplink_decision(&mut self, decision: VisionTlsDecision) {
        self.inner_tls.client_tls_observed = true;
        self.inner_tls.decision = decision;
    }

    #[cfg(test)]
    pub fn force_client_tls_filter_active(&mut self) {
        self.inner_tls.client_tls_observed = true;
    }

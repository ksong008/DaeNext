...
pub const RESIDENT_WEBSOCKET_MAX_MESSAGE_BYTES: usize = 16 * 1024 * 1024;

#[derive(Default)]
pub struct WebSocketBinaryFrameDecoder {
    pending: Vec<u8>,
    consumed: usize,
...
impl WebSocketBinaryFrameDecoder {
    ...
    pub fn push(&mut self, input: &[u8]) -> Result<Vec<Vec<u8>>, String> {
        self.extend(input)?;
    ...
    pub fn extend(&mut self, input: &[u8]) -> Result<(), String> {
        if self.closed {
            return Ok(());
        }
    ...
    pub fn next_message(&mut self) -> Result<Option<&[u8]>, String> {
        self.ready_fragmented = None;
        while !self.closed {
    ...
    pub fn take_control_responses(&mut self) -> Vec<Vec<u8>> {

use super::*;
pub const TCP_ROUTE_CHOSEN_EVENT: &str = "tcp_route_chosen";
pub const TCP_OUTBOUND_KIND_DIRECT: &str = "direct";
pub const TCP_OUTBOUND_KIND_BLOCK: &str = "block";
pub const TCP_OUTBOUND_KIND_PROXY: &str = "proxy";
pub const TCP_FIXED_POLICY: &str = "fixed";
pub const TCP_DIRECT_DIALER: &str = "direct";
pub const TCP_BLOCK_DIALER: &str = "block";

pub fn append_tcp_execution_fields(event: &mut Value, execution: &str) {
    append_runtime_execution_descriptor(event, tcp_execution_descriptor(execution));
}

pub fn append_proxy_tcp_execution_fields(
    event: &mut Value,
    execution: &str,
    handler: &str,
...
pub fn proxy_tcp_finished_event(
    peer: SocketAddr,
...
pub fn generic_proxy_tcp_finished_event(
    peer: SocketAddr,
    original_dst: SocketAddr,
    selection: &TcpProxySelection,
    sniff: &TcpSniffReport,
...
pub fn generic_proxy_tcp_failed_event(
    peer: SocketAddr,
    original_dst: SocketAddr,
    selection: &TcpProxySelection,
    sniff: &TcpSniffReport,
...
pub fn append_proxy_relay_stats(event: &mut Value, stats: &RelayStats) {
...
pub fn append_generic_proxy_relay_stats(event: &mut Value, stats: &DirectTcpRelayStats) {
    if let Some(map) = event.as_object_mut() {
        map.insert(
            "bytes_client_to_proxy".to_owned(),

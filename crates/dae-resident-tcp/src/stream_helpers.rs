...
pub async fn read_http_head_and_leftover_from_async_stream<S>(
    stream: &mut S,
...
pub fn validate_simple_obfs_http_response_status(response_head: &[u8]) -> Result<(), String> {
...
pub async fn read_simple_obfs_tls_response_payload_from_async_stream<S>(
    stream: &mut S,
...
pub fn simple_obfs_tls_application_data_frame(payload: &[u8]) -> Result<Vec<u8>, String> {
...
pub fn simple_obfs_tls_application_data_header(payload_len: usize) -> Result<[u8; 5], String> {
...
pub struct AsyncSimpleObfsTlsAppDataReader<'a, S> {
    ...
    pub state: AsyncSimpleObfsTlsReadState,
    pub stream: &'a mut S,
}

pub enum AsyncSimpleObfsTlsReadState {

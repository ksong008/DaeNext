...
pub const RESIDENT_TCP_FAILED_HANDLER_JOIN_GRACE: Duration = Duration::from_millis(100);

mod dns;

pub use self::dns::resident_dns_proxy_tcp_transport;
...
pub async fn fetch_resident_proxy_http_response_async(
...
pub async fn join_resident_tcp_handler_after_exchange_async(
...
pub fn sanitize_probe_event(event: Value) -> String {
    event
        .get("event")
...
#[allow(clippy::too_many_arguments)]
pub async fn exchange_resident_proxy_tcp_stream_async<F, Fut>(
    binding: ResidentProxyBinding,
    target: &str,
    dial_ip: bool,

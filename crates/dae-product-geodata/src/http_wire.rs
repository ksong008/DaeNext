...
use crate::{
    GEODATA_HTTP_BODY_LIMIT, GEODATA_HTTP_HEADER_LIMIT, GeodataFileDownload, GeodataKind,
    GeodataRelease, is_valid_geodata_release_version,
};

pub fn parse_geodata_latest_release(kind: GeodataKind, body: &[u8]) -> io::Result<GeodataRelease> {
    let release: serde_json::Value = serde_json::from_slice(body).map_err(|error| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            format!("parse geodata release metadata: {error}"),
        )
    })?;
    let version = release
        .get("tag_name")
    ...
    let download_url = url::Url::parse(download_url).map_err(|error| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            format!("invalid geodata release asset url: {error}"),
        )
    })?;
    Ok(GeodataRelease {
...
pub enum GeodataHttpResult {
    Body(Vec<u8>),
    Redirect(url::Url),
}

pub enum GeodataHttpFileResult {
    Body(GeodataFileDownload),
    Redirect(url::Url),
}
pub fn geodata_http_request(url: &url::Url) -> io::Result<String> {
    let host = url
        .host_str()
...
pub fn geodata_http_response_limit() -> io::Result<usize> {
...
pub fn read_geodata_http_response<R: Read>(reader: &mut R) -> io::Result<Vec<u8>> {
...
pub fn read_geodata_http_response_to_file<R: Read>(
    base_url: &url::Url,
    reader: &mut R,
    output_path: &Path,
...
pub fn geodata_http_response_to_file_from_bytes(
    base_url: &url::Url,
    response: Vec<u8>,
    output_path: &Path,
) -> io::Result<GeodataHttpFileResult> {

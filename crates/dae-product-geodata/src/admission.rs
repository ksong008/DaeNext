...
#[derive(Debug, Default)]
pub struct ProductGeodataUpdateCoordinator {
    state: Mutex<ProductGeodataUpdateState>,
    next_generation: AtomicU64,
}

#[derive(Debug)]
pub struct ProductGeodataUpdateLease {
    coordinator: Arc<ProductGeodataUpdateCoordinator>,
    kind: GeodataKind,
}

impl ProductGeodataUpdateCoordinator {
    pub fn acquire(self: &Arc<Self>, kind: GeodataKind) -> io::Result<ProductGeodataUpdateLease> {
        let mut state = self
            .state
            .lock()
...
mod tests {
    ...
    #[test]
    fn staging_paths_are_atomically_unique_and_skip_stale_files() {
        let dir = std::env::temp_dir().join(format!(
            "dae-product-geodata-staging-path-{}",
            fastrand::u64(..)
        ));

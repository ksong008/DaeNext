...
pub struct ProductHttpJobQueue<T> {
    capacity: usize,
...
#[derive(Debug)]
pub enum ProductHttpQueueSendError<T> {
    Full(T),
    Closed(T),
}

#[derive(Debug)]
pub enum ProductHttpQueueReceiveError {
    Timeout,
    Closed,
}
...
impl<T> ProductHttpJobQueue<T> {
    ...
    pub fn receive_timeout(&self, timeout: Duration) -> Result<T, ProductHttpQueueReceiveError> {

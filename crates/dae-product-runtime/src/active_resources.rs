...
use dae_product_core::{RuntimeNodeTag, decode_product_label};
use regex::Regex;
use rusqlite::Connection;
use serde_json::{Value, json};

use crate::{referenced_group_names_from_routing, render_routing_section};

#[derive(Debug)]
pub struct ActiveRuntimeResources {
    pub groups: Value,
    pub nodes: Value,
...
pub fn load_active_runtime_resources(
    conn: &Connection,
    routing_raw: &str,
...
fn active_node_value(row: &rusqlite::Row<'_>, offset: usize) -> rusqlite::Result<Value> {
    ...
    Ok(json!({
        ...
        "name": decode_product_label(&name),
        "address": row.get::<_, String>(offset + 3)?,
        "protocol": row.get::<_, String>(offset + 4)?,
        "transport": Value::Null,
        "tag": tag.as_deref().map(decode_product_label),
...
fn compile_name_filter(value: Option<&str>) -> io::Result<Option<Regex>> {

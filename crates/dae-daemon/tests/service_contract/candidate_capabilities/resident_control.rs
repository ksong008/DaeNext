...
pub(crate) fn assert_resident_and_control_plane_contract(report: &Value) {
    ...
    assert!(
        !report["resident_dataplane_admission_env_required"]
            .as_bool()
            .unwrap()
    );
    assert!(
    ...
    assert!(
        report["native_daemon_benchmark_gate_ready"]
            .as_bool()
            .unwrap()
    );
    assert!(
    ...
    assert!(
        report["control_plane_runtime_state_report"]["ready_for_product_control_plane"]
            .as_bool()
            .unwrap()
    );
    assert!(
        report["control_plane_native_tproxy_contract_ready"]
            .as_bool()
            .unwrap()
    );
    assert!(
        report["control_plane_native_dependency_contract_ready"]
            .as_bool()
            .unwrap()
    );
    assert!(

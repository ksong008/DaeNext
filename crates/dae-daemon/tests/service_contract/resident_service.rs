...
const RESIDENT_SERVICE_INTEGRATION_ENV: &str = "RUN_RESIDENT_SERVICE_INTEGRATION";

#[test]
pub(super) fn resident_service_notifies_reloads_rejects_bad_config_and_cleans_pid() {
    if std::env::var(RESIDENT_SERVICE_INTEGRATION_ENV).as_deref() != Ok("1") {
        eprintln!(
            "skipping resident service integration; set {RESIDENT_SERVICE_INTEGRATION_ENV}=1 to run"
        );
        return;
    }

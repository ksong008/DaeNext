...
pub(crate) fn daed_resident_adapter_matrix_reports_shadowsocks_2022_admitted() {
    for conf in cipher_confs() {
        ...
        let shadowsocks_2022 = shadowsocks_2022_fixture_url(*conf);
        fs::write(
            &config,
            r#"
global {
...
node {
  ss_live: '__SHADOWSOCKS_2022_SOURCE__'
}
group {
  proxy {
    filter: name(ss_live)
...
"#
        ...
        assert!(!stdout.contains(&shadowsocks_2022));
        assert!(!stdout.contains(&["ss", "://"].concat()));

...
pub(super) fn daed_product_contract_reports_runtime_state_paths() {
    ...
    assert!(
        package["current_runtime_ready"]["local_package_admission"]
            .as_bool()
            .unwrap()
    );
    assert!(
        package["current_runtime_ready"]["validate_command"]
            .as_bool()
            .unwrap()
    );
    assert!(
        !package["package_surface"]["live_host_replacement_applied"]
            .as_bool()
            .unwrap()
    );
    assert!(
        !package["package_surface"]["final_state_validation_applied_on_live_host"]
            .as_bool()
            .unwrap()
    );
    assert!(
    ...
    assert!(
        !package["package_surface"]["native_product_shell_ready"]
            .as_bool()
            .unwrap()
    );

...
pub(super) fn assert_protocol_matrix_source_uses_generic_semantics(source: &str) {
    ...
    for digit in '0'..='9' {
        ...
        assert!(
            !source.contains(&needle),
            "protocol matrix source fixtures must not use planning labels, found {needle}"
        );
    }
    for needle in [["#", "sta", "ge"].concat(), ["sta", "ge", "-"].concat()] {
        assert!(
            !source.contains(&needle),
            "protocol matrix source fixtures must not use planning labels, found {needle}"
        );
...
pub(super) fn assert_current_config_matrix_scope_contract(report: &Value) {
    ...
    assert!(
        !report["full_matrix_expanded_source_final_state_ready"]
            .as_bool()
            .unwrap()
    );
    assert_eq!(
    ...
    assert!(
        !contract["finalStateMayUseAsExpandedSourceMatrix"]
            .as_bool()
            .unwrap()
    );

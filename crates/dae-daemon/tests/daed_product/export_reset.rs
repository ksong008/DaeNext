use super::*;
#[test]
pub(super) fn daed_export_commands_report_product_package_surface() {
    ...
    assert!(
        !manifest["admission"]["finalStateValidationAppliedOnLiveHost"]
            .as_bool()
            .unwrap()
    );
...
pub(super) fn daed_resetpass_updates_daed_db_users_without_wing_db() {
    ...
    fs::write(&wing, b"legacy import db").unwrap();

...
#[test]
pub(super) fn daed_run_serves_product_resource_runtime_log_latency_and_bundle_surface() {
    let temp = temp_dir("run-product-api");
    let web = temp.join("web");
    fs::create_dir_all(&web).unwrap();
    fs::write(web.join("index.html"), "<!doctype html><title>daed</title>").unwrap();
    let port = free_port();
    ...
    let manual_runtime_tag = json_body(&tag_node)["runtimeTag"]
        .as_str()
        .unwrap()
        .to_owned();
    ...
    let selected_subscription_runtime_tag = subscription_binding["matchedNodes"][0]["runtimeTag"]
        .as_str()
        .unwrap()
        .to_owned();
    assert!(
    ...
    assert!(generated.contains("generated by Rust daed production local product surface"));
    assert!(generated.contains("node {"), "{generated}");
    assert!(generated.contains("group {"), "{generated}");
    let expected_filter =
        format!("filter: name('{manual_runtime_tag}', '{selected_subscription_runtime_tag}')");
    assert!(generated.contains(&expected_filter), "{generated}");
    assert!(!expected_filter.contains("n1-renamed"));
    assert!(!expected_filter.contains("selected-sub-node"));

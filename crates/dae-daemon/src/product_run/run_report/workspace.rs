use super::*;
pub(crate) struct ProductRunWorkspace {
...
pub(crate) fn prepare_product_run_workspace(
    options: &RunOptions,
) -> Result<ProductRunWorkspace, String> {
    ...
    let manifest_file = run_dir.join("daed-run.json");
    let run_config_file = run_dir.join("input-config.dae");
    let pid_file = run_dir.join("daed.pid");
    let progress_file = run_dir.join("daed.progress");

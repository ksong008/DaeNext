...
pub struct ProductRunIdentityAdmissionOptions {
    pub root: PathBuf,
    pub config: PathBuf,
    pub logfile: PathBuf,
...
impl ProductRunIdentityAdmissionOptions {
    pub fn under_root(root: impl Into<PathBuf>) -> Self {
        let root = root.into();
        Self {
            config: root.join("config").join("product-run-identity.dae"),
            logfile: root.join("log").join("dae-product-run-identity.log"),
            root,
...
pub fn product_run_identity_admission_root() -> PathBuf {
    PathBuf::from("/tmp/dae-product-run-identity-admission")
}

pub fn product_run_identity_admission_report(
    options: &ProductRunIdentityAdmissionOptions,
) -> Result<Value, String> {
    ensure_safe_run_identity_admission_root(&options.root)?;
    if options.root.exists() {
        fs::remove_dir_all(&options.root).map_err(|err| {
            format!(
                "failed to remove existing product-run-identity root {}: {err}",
    ...
    let manifest_file = run_dir.join("product-run-identity-admission.json");
    let pid_file = run_dir.join("dae-product-run-identity.pid");
    let progress_file = run_dir.join("dae-product-run-identity.progress");
    let sdnotify_file = run_dir.join("sdnotify.ready");
    let run_entrypoint_root = derived_root(
        "/tmp/dae-run-entrypoint-product-run-identity",
        &options.root,
    );

    fs::create_dir_all(&run_dir).map_err(|err| {
        format!(
            "failed to create product-run-identity run dir {}: {err}",
            path_string(&run_dir)
        )
    })?;
    if let Some(parent) = options.logfile.parent() {
        fs::create_dir_all(parent).map_err(|err| {
            format!(
                "failed to create product-run-identity log dir {}: {err}",
                path_string(parent)
            )
        })?;
    }
    ...
    let config = fs::read_to_string(&options.config).map_err(|err| {
        format!(
            "failed to read product-run-identity config {}: {err}",
    ...
    if config.trim().is_empty() {
        return Err(format!(
            "product-run-identity config must not be empty: {}",
            path_string(&options.config)
        ));
    }
    ...
    if !options.disable_pidfile {
        fs::write(&pid_file, format!("{}\n", std::process::id()))
            .map_err(|err| format!("failed to write product-run-identity pid file: {err}"))?;
    }
    fs::write(&progress_file, [RELOAD_DONE])
        .map_err(|err| format!("failed to write product-run-identity progress file: {err}"))?;
    fs::write(&sdnotify_file, "READY=1\n")
        .map_err(|err| format!("failed to write product-run-identity sdnotify file: {err}"))?;
    fs::write(&options.logfile, "product-run-identity admission\n")
        .map_err(|err| format!("failed to write product-run-identity log file: {err}"))?;

    let wrapper = run_entrypoint_preflight_report(&run_entrypoint_root)?;
    let mut report = json!({
        "name": "rust-product-run-identity-admission",
        ...
        "command": "product-run-identity-admission",
        "run_identity": "Rust product run identity shape",
        "primary_command": "run",
    ...
    fs::write(&manifest_file, manifest)
        .map_err(|err| format!("failed to write product-run-identity manifest: {err}"))?;
    Ok(report)
}
...
fn prepare_config(config: &Path, root: &Path) -> Result<(), String> {
    ...
    if !config.starts_with(root) {
        return Err(format!(
            "product-run-identity config does not exist and is outside root: {}",
            path_string(config)
        ));
    }
    if let Some(parent) = config.parent() {
        fs::create_dir_all(parent).map_err(|err| {
            format!(
                "failed to create product-run-identity config dir {}: {err}",
                path_string(parent)
            )
        })?;
    }
...
fn ensure_safe_run_identity_admission_root(root: &Path) -> Result<(), String> {
    if !root.is_absolute() {
        return Err(format!(
            "product-run-identity root must be absolute: {}",
            path_string(root)
        ));
    }
    let root_string = path_string(root);
    if !root_string.starts_with("/tmp/dae-product-run-identity") {
        return Err(format!(
            "product-run-identity root must be under /tmp/dae-product-run-identity*: {root_string}"
        ));
    }
    Ok(())
}
...
fn derived_root(prefix: &str, root: &Path) -> PathBuf {
    let suffix = root
        ...
        .unwrap_or("product-run-identity");

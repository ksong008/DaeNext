...
pub(super) const SUBSCRIPTION_HELPER_SCHEMA_VERSION: u64 = 1;
pub(super) const SUBSCRIPTION_HELPER_MAX_REQUEST_BYTES: usize = 128 * 1024;
pub(super) const SUBSCRIPTION_HELPER_MAX_RESPONSE_BYTES: usize = 32 * 1024 * 1024;

#[derive(Clone, Debug)]
pub(super) struct SubscriptionHelperRequest {
    pub(super) state: PathBuf,
    pub(super) config_dir: PathBuf,
    pub(super) response: PathBuf,
    pub(super) persist_staging: PathBuf,
    pub(super) source: SubscriptionSourceIdentity,
}

#[derive(Clone, Debug)]
pub(super) enum DecodedSubscriptionHelperOutcome {
    Prepared(node_stage::PreparedSubscriptionRefresh),
    FetchFailed(fetch_error::SubscriptionFetchFailure),
}

pub(super) fn encode_subscription_helper_request(
...
pub(super) fn decode_subscription_helper_request(
    input: &[u8],
) -> io::Result<SubscriptionHelperRequest> {
...
pub(super) fn encode_subscription_helper_success(
    source: &SubscriptionSourceIdentity,
    prepared: &node_stage::PreparedSubscriptionRefresh,
) -> Value {
    json!({
...
pub(super) fn encode_subscription_helper_failure(
    source: &SubscriptionSourceIdentity,
...
pub(super) fn decode_subscription_helper_response(
    reader: impl Read,
    ...
    match required_string(&value, "status")? {
        "fail" => {
            ...
            let failure =
                fetch_error::SubscriptionFetchFailure::from_code(code).ok_or_else(|| {
                    io::Error::new(
                        io::ErrorKind::InvalidData,
...
fn decode_prepared(value: &Value) -> io::Result<node_stage::PreparedSubscriptionRefresh> {
    ...
    let not_admitted = required_array(value, "notAdmitted")?
        ...
        .map(decode_rejected_node)
        .collect::<io::Result<Vec<_>>>()?;
    Ok(node_stage::PreparedSubscriptionRefresh {
        ...
        nodes: node_stage::PreparedSubscriptionNodes {
            admitted,
            invalid,
            not_admitted,
        },
...
fn decode_admitted_node(value: &Value) -> io::Result<node_stage::PreparedSubscriptionNode> {
    ...
    Ok(node_stage::PreparedSubscriptionNode {
        stored_link,
...
fn encode_rejected_node(node: &node_stage::RejectedSubscriptionNode) -> Value {
    json!({"link": node.link, "reason": node.reason})
}

fn decode_rejected_node(value: &Value) -> io::Result<node_stage::RejectedSubscriptionNode> {
    ...
    Ok(node_stage::RejectedSubscriptionNode {
        link: required_string(value, "link")?.to_owned(),
        reason: reason.to_owned(),
    })
}

fn decode_content_kind(value: &str) -> io::Result<content::SubscriptionContentKind> {
    match value {
        "empty" => Ok(content::SubscriptionContentKind::Empty),
        "sip008" => Ok(content::SubscriptionContentKind::Sip008),
        "plain-text" => Ok(content::SubscriptionContentKind::PlainText),
        "base64" => Ok(content::SubscriptionContentKind::Base64),
        "unrecognized" => Ok(content::SubscriptionContentKind::Unrecognized),
        _ => Err(io::Error::new(
            io::ErrorKind::InvalidData,
...
mod tests {
    ...
    fn helper_protocol_round_trips_prepared_nodes_without_reparsing_links() {
        let content = content::SubscriptionContentReport::from_links(&[
            "socks://127.0.0.1:1080#fixture".to_owned(),
        ]);
    ...
    fn helper_protocol_round_trips_safe_fetch_failure_codes() {
        let failure = fetch_error::SubscriptionFetchFailure::from_io_error(&io::Error::new(

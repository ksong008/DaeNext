    ...
    report.insert(
        ...
        json!({
            ...
            "reload_runtime": {
                ...
                "requires_active_tcp": true,
                "scope": "production owner lifecycle listener reuse, live listen_socket_map re-handoff, BPF/map owner transfer observation, DNS cache migration guard, bounded close, RuntimeOverview fields, invalid-config restore, and post-reload active TCP probe",
            },
            "udp_dns_datapath": context.udp_dns_contract.clone(),
            "ebpf_backend": context.ebpf_capability_json.clone(),
            "native_ebpf": {
                "native_backend_requested": context.options.native_ebpf_requested,
                "requested_backend": context.options.native_ebpf_backend.as_str(),
                "local_admission": context.options.native_ebpf_local_admission,

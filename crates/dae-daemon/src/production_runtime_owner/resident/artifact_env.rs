...
pub(super) fn resolve_source_object(artifact_dir: &Path) -> Result<PathBuf, String> {
    ...
    let _ = artifact_dir;
    Err("external PARAM source object is unavailable in Rust-native resident build".to_owned())
}
...
pub(super) fn resolve_native_backend() -> Result<AttachBackend, String> {
    ...
    parse_native_backend(&raw).ok_or_else(|| {
        format!(
            "{env_name} must be one of auto, tcx, tc-netlink, tc_netlink, tc-command, tc_command; got {raw}"
        )
    })
}

...
use crate::production_runtime_owner::resident_dataplane::TcpRelayResourceProfile;

pub(crate) type WebSocketControlSender = tokio::sync::mpsc::Sender<Vec<u8>>;
pub(crate) type WebSocketControlReceiver = tokio::sync::mpsc::Receiver<Vec<u8>>;

type WebSocketControlPermitFuture = Pin<
    Box<dyn Future<Output = Result<mpsc::OwnedPermit<Vec<u8>>, mpsc::error::SendError<()>>> + Send>,
>;

pub(crate) struct WebSocketControlPollSender {
...
impl WebSocketControlPollSender {
    pub(crate) fn new(sender: WebSocketControlSender) -> Self {
        Self {
            sender,
    ...
    pub(crate) fn poll_flush(&mut self, cx: &mut Context<'_>) -> Poll<std::io::Result<()>> {
...
pub(crate) fn websocket_control_channel() -> (WebSocketControlSender, WebSocketControlReceiver) {
    tokio::sync::mpsc::channel(TcpRelayResourceProfile::selected().websocket_control_queue_depth())
}

pub(crate) async fn queue_websocket_control_responses(
...
pub(crate) async fn write_websocket_control_response<W>(
    writer: &mut W,
    response: Vec<u8>,
    context: &str,
) -> Result<(), String>

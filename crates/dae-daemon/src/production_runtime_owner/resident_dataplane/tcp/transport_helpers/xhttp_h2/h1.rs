...
pub(super) async fn read_xhttp_h1_response_head<T>(
    client: &mut T,
    context: &str,
...
impl XhttpH1ChunkedWriter {
    pub(super) fn from_client(client: AsyncResidentTlsClient) -> Self {
        Self {
    ...
    pub(super) fn from_write_half(writer: tokio::io::WriteHalf<AsyncResidentTlsClient>) -> Self {
        Self {
...
impl XhttpH1DownloadBody {
    ...
    pub(super) fn new_with_read_half(
    ...
    pub(super) async fn read_next(&mut self) -> Result<Option<Bytes>, String> {
    ...
    pub(super) fn poll_next(
        &mut self,
        cx: &mut Context<'_>,
    ) -> Poll<Result<Option<Bytes>, String>> {
        loop {
            match self.state {
                XhttpH1BodyState::ChunkSize => {

...
use crate::production_runtime_owner::resident_dataplane::plan::ResidentXhttpEndpointPlan;
use crate::production_runtime_owner::resident_dataplane::{
    RESIDENT_CONNECT_TIMEOUT, authority_from_host_port, resolve_socket_addr_candidates,
};

#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub(super) enum XhttpAddressFamily {
    Ipv4,
    Ipv6,
}

#[derive(Clone, Debug, Eq, Hash, PartialEq)]
pub(super) struct XhttpResolvedEndpointIdentity {
    candidates: Vec<SocketAddr>,
    families: Vec<XhttpAddressFamily>,
}

pub(super) struct XhttpResolvedEndpoint {
    candidates: Vec<SocketAddr>,
    identity: XhttpResolvedEndpointIdentity,
}

impl XhttpResolvedEndpoint {
    pub(super) async fn resolve(endpoint: &ResidentXhttpEndpointPlan) -> Result<Self, String> {
    ...
    pub(super) fn candidates(&self) -> &[SocketAddr] {
        &self.candidates
    }

    pub(super) fn identity(&self) -> &XhttpResolvedEndpointIdentity {
        &self.identity
    }
...
impl XhttpResolvedEndpointIdentity {
    pub(super) fn from_candidates(candidates: &[SocketAddr]) -> Self {

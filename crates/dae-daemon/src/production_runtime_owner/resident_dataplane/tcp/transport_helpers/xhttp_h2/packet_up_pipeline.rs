...
pub(crate) struct XhttpPacketUpPipeline {
...
impl XhttpPacketUpPipeline {
    pub(crate) fn for_upload(upload: &XhttpUploadClient) -> Self {
    ...
    pub(crate) fn poll_ready(&mut self) -> Result<(), String> {
        loop {
    ...
    pub(crate) async fn wait_one(&mut self) -> Result<bool, String> {
    ...
    pub(crate) fn max_post_bytes(&self) -> usize {
        self.max_post_bytes
    }

    #[cfg(test)]
    pub(crate) fn for_test(max_in_flight: usize) -> Self {
        Self {
            completions: FuturesUnordered::new(),
            max_in_flight,
    ...
    #[cfg(test)]
    pub(crate) fn push_test_completion(&self, completion: XhttpPacketUpCompletion) {
        self.completions.push(completion);
    }
}

...
pub(super) enum HttpChunkedDecodeError {
...
pub(super) fn decode_http_chunked_body(raw: &[u8]) -> Result<Vec<u8>, String> {
    decode_http_chunked_body_with_consumed(raw)
        .map(|(body, _)| body)
        .map_err(|err| err.to_string())
}

pub(super) fn decode_http_chunked_body_with_consumed(
    raw: &[u8],

...
impl UdpRequestIdAllocator {
    ...
    fn allocate_at(&mut self, capacity: usize, now: time::Instant) -> Result<u16, String> {
    ...
    fn release_at(&mut self, id: u16, now: time::Instant) {
        ...
        self.quarantine_deadlines
            .push_back((id, now + dns_udp_forward_attempt_timeout()));
    }
    ...
    fn is_quarantined(&self, id: u16) -> bool {
        let (word, bit) = dns_udp_request_id_bitmap_slot(id);
    ...
    fn reap_quarantine(&mut self, now: time::Instant) {
        while self
...
mod tests {
    ...
    fn udp_request_id_allocator_enforces_capacity_and_releases_ids() {
        ...
        let after_quarantine = now + dns_udp_forward_attempt_timeout();

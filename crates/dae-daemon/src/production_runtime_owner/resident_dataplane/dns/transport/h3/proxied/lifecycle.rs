...
pub(super) trait ProxiedDoh3ExchangeTarget: Send {
    fn exchange(&mut self) -> impl Future<Output = Result<Vec<u8>, String>> + Send;
...
pub(super) async fn run_cancelable_proxied_doh3_exchange<T>(target: T) -> Result<Vec<u8>, String>
where
    T: ProxiedDoh3ExchangeTarget + 'static,
{
    ...
    let result = owner_task
        .await
        .map_err(|error| format!("proxied DNS over HTTP/3 owner task failed: {error}"))?;
    cancellation.disarm();
    result
}
    ...
    T: ProxiedDoh3ExchangeTarget,
{
    let exchange = tokio::select! {
        biased;
        _ = &mut cancelled => Err(PROXIED_DOH3_CANCELLED.to_owned()),
...
fn merge_exchange_and_cleanup_result(
    ...
    match exchange {
        Ok(_) => Err(format!("proxied DNS over HTTP/3 cleanup failed: {cleanup}")),
        Err(error) => Err(format!(
            "{error}; proxied DNS over HTTP/3 cleanup failed: {cleanup}"
        )),
    }
}

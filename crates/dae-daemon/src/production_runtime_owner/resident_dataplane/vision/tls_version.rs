use super::*;
pub(crate) fn client_hello_advertises_tls13(extensions: Option<&[u8]>) -> Option<bool> {
    parse_supported_versions(extensions, parse_tls_client_hello_extension)
        .map(supported_versions_contains_tls13)
}

pub(crate) fn server_hello_selects_tls13(extensions: Option<&[u8]>) -> bool {
    parse_supported_versions(extensions, parse_tls_server_hello_extension)
        .map(supported_versions_contains_tls13)
        .unwrap_or(false)
}

pub(crate) fn parse_supported_versions<'a, F>(
...
pub(crate) fn supported_versions_contains_tls13(versions: Vec<TlsVersion>) -> bool {
    versions
        .into_iter()

...
pub(crate) fn build_trojan_proxy_plan(
    ...
    let inner_shadowsocks = parse_trojan_inner_shadowsocks(&parsed.encryption, &node_tag)?;
...
pub(crate) fn parse_trojan_inner_shadowsocks(
    encryption: &str,
    node_tag: &str,
    ...
    let spec = cipher_spec(&cipher).map_err(|err| {
        format!("admit Trojan inner Shadowsocks cipher for node {node_tag}: {err}")
    })?;

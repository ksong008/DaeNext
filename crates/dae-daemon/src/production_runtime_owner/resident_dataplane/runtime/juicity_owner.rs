...
pub(crate) struct JuicityTransportLease {
    shared: Arc<JuicitySharedTransport>,
}

impl JuicityTransportLease {
    #[cfg(test)]
    pub(crate) fn connection_stable_id(&self) -> usize {
        self.shared.connection.stable_id()
    }

    pub(crate) fn physical_owner_id(&self) -> u64 {
        self.shared.instance_id
    }

    pub(crate) fn auth_token_nonzero(&self) -> bool {
...
#[derive(Clone)]
pub(crate) struct JuicityOwnerRegistryHandle {
    generation: OwnerGeneration,
...
pub(crate) fn start_juicity_owner_registry(
    generation: u64,
...
pub(super) fn start_juicity_owner_registry_with_resources(
    generation: u64,
    stop: SharedResidentStopSignal,
    stack_bytes: usize,
    resources: JuicityOwnerResourceProfile,

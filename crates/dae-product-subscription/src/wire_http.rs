...
pub const SUBSCRIPTION_HTTP_HEADER_LIMIT: usize = 128 * 1024;
pub const SUBSCRIPTION_MAX_BYTES: usize = 8 * 1024 * 1024;
...
pub fn subscription_http_request(url: &url::Url) -> io::Result<String> {
...
pub fn http_response_body_with_limit(response: &[u8], body_limit: usize) -> io::Result<String> {
    ...
    String::from_utf8(response.body).map_err(|error| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            format!("subscription response is not UTF-8: {error}"),
        )
    })
}

pub fn parse_subscription_http_response(
    response: &[u8],
    body_limit: usize,
...
fn response_headers(response: &[u8]) -> io::Result<(&str, usize)> {
    ...
    if split > SUBSCRIPTION_HTTP_HEADER_LIMIT {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!("subscription response headers exceed {SUBSCRIPTION_HTTP_HEADER_LIMIT} bytes"),
        ));
    }
    ...
    let header_text = std::str::from_utf8(&response[..split]).map_err(|error| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            format!("subscription response headers are not UTF-8: {error}"),
        )
    })?;
...
fn validate_content_length(
    ...
    let content_length = content_length.parse::<usize>().map_err(|error| {
        io::Error::new(
            io::ErrorKind::InvalidData,
            format!("invalid subscription Content-Length: {error}"),
        )
    })?;
...
pub fn first_header<'a>(headers: &'a BTreeMap<String, Vec<String>>, name: &str) -> Option<&'a str> {
    headers
        .get(name)
...
pub fn is_subscription_redirect(status: u16) -> bool {
...
fn read_decoded_limited<R: Read>(
    ...
    loop {
        let read = reader.read(&mut buffer).map_err(|error| {
            io::Error::new(
                io::ErrorKind::InvalidData,
                format!("decode subscription {encoding} body: {error}"),
            )
        })?;
        if read == 0 {
            return Ok(out);
...
pub fn decode_chunked_body(body: &[u8]) -> io::Result<Vec<u8>> {
    decode_chunked_body_with_limit(body, SUBSCRIPTION_MAX_BYTES)
}

pub fn decode_chunked_body_with_limit(body: &[u8], body_limit: usize) -> io::Result<Vec<u8>> {
    let mut index = 0;
    let mut out = Vec::new();
...
pub fn subscription_http_response_limit(body_limit: usize) -> io::Result<usize> {

...
use dae_product_core::{product_iso8601_utc, product_now_text};
use dae_product_persistence::{
    RuntimeDesiredStateRevision, ensure_state_schema, open_state_connection,
};
use rusqlite::{Connection, params};
use serde_json::{Value, json};

use crate::{
    LatencyProbeNode, node_name_from_link, runtime_execution_identity, runtime_link_hash,
    runtime_link_identity_value, runtime_redacted_link_source,
};

#[derive(Clone, Debug)]
pub struct NodeLatencyWrite {
    pub node_id: i64,
    pub node_link: String,
    pub probe_generation: Option<u64>,
    pub desired_state_revision: Option<Arc<RuntimeDesiredStateRevision>>,
    ...
    pub fn results_for_snapshots(
        ...
        for snapshot in snapshots {
            ...
            let checked_at = snapshot
                ...
                .map(|checked_at| product_iso8601_utc(checked_at as u64))
                .unwrap_or_else(product_now_text);
...
pub fn stored_successful_node_latency_seed_snapshots(state: &Path) -> io::Result<Vec<Value>> {
    ensure_state_schema(state)?;
    let conn = open_state_connection(state)?;
    stored_successful_node_latency_seed_snapshots_from_conn(&conn)
}

pub fn stored_successful_node_latency_seed_snapshots_from_conn(
    conn: &Connection,
) -> io::Result<Vec<Value>> {
    let mut stmt = conn
        .prepare(
    ...
    for row in rows {
        snapshots.push(row.map_err(latency_sqlite_io_error)?);
    }
    Ok(snapshots)
}

pub fn list_stored_node_latencies_value(state: &Path) -> io::Result<Value> {
    ensure_state_schema(state)?;
    let conn = open_state_connection(state)?;
    ...
    for row in rows {
        items.push(row.map_err(latency_sqlite_io_error)?);
    }
    Ok(json!({"items": items}))
}

pub fn node_latency_results_for_runtime_snapshots(
    ...
    let tested_at = product_now_text();
...
pub fn runtime_node_latency_results_for_nodes(
    nodes: &[LatencyProbeNode],
    snapshots: &[Value],
) -> (Vec<NodeLatencyWrite>, HashSet<i64>) {
...
pub fn runtime_latency_snapshot_link_hash(snapshot: &Value) -> Option<&str> {
    snapshot
        .get("linkHash")
        .and_then(Value::as_str)
        .or_else(|| {
...
pub fn runtime_latency_snapshot_has_result(snapshot: &Value) -> bool {
...
pub fn store_node_latency_result(conn: &Connection, result: &NodeLatencyWrite) -> io::Result<()> {
    conn.execute(
...
pub fn native_probe_missing_results(
    nodes: &[LatencyProbeNode],
    tested_at: &str,
...
pub fn all_node_ids(conn: &Connection) -> io::Result<Vec<i64>> {
    let mut stmt = conn
    ...
    let mut ids = Vec::new();
    for row in rows {
        ids.push(row.map_err(latency_sqlite_io_error)?);
    }
    Ok(ids)
}

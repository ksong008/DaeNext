...
use crate::{compile_subscription_name_filter, subscription_node_matches_name_filter};
...
pub fn list_group_summaries_batched(
    conn: &Connection,
...
fn render_subscription_summary(
    ...
    let filter = compile_subscription_name_filter(binding.name_filter_regex.as_deref())?;
    ...
    for candidate in candidates {
        if !subscription_node_matches_name_filter(&candidate.node, filter.as_ref()) {
            continue;
        }

...
use dae_product_core::RUNTIME_PROBE_GENERATION_METADATA_KEY;
use dae_product_persistence::{
    RuntimeDesiredStateRevision, open_state_connection,
    runtime_desired_state_revision_from_connection,
};
use rusqlite::{Connection, OptionalExtension, params};

use crate::{NodeLatencyWrite, store_node_latency_result};

pub const NODE_LATENCY_DB_WRITE_BATCH_SIZE: usize = 128;

#[derive(Debug, Default)]
pub struct LatencyPersistenceQueue {
    pending: Mutex<BTreeMap<i64, NodeLatencyWrite>>,
}

#[derive(Debug, Default)]
pub struct LatencyPersistenceFlush {
    pub pending: usize,
    pub error: Option<String>,
}

impl LatencyPersistenceQueue {
    pub fn queue(&self, results: &[NodeLatencyWrite]) -> io::Result<usize> {
        if results
            .iter()
    ...
    pub fn flush(&self, state: &Path) -> LatencyPersistenceFlush {
...
pub fn write_node_latency_results(
    conn: &mut Connection,
...
fn write_node_latency_results_chunk(
    ...
    let tx = conn.transaction().map_err(latency_sqlite_io_error)?;
    ...
    tx.commit().map_err(latency_sqlite_io_error)?;
    Ok((written, alive))
}
...
mod tests {
    ...
    use dae_product_persistence::ensure_state_schema;
    ...
    impl FreshProductState {
        fn new(scope: &str) -> Self {
            let root = std::env::temp_dir()
                .join(format!("dae-product-latency-{scope}-{}", fastrand::u64(..)));
        ...
        fn connection(&self) -> Connection {
            open_state_connection(&self.state).unwrap()
        }

        fn seed_selected_resources(&self) {
            self.connection()
                .execute_batch(

...
use dae_geodata::{decode_entry_bytes, decode_entry_view_bytes, decode_hex};
...
pub(crate) fn cases() -> Vec<BenchCase> {
    vec![
        ...
        BenchCase {
            id: "geodata/streaming_geoip_entry_view",
            default_iters: 100_000,
            run: bench_geodata_streaming_geoip_entry_view,
        },
    ]
}
...
fn bench_geodata_streaming_geoip_entry_view(
    iters: u64,
    warmup: u64,
    ...
    Ok(measure(
        || {
            let entry = decode_entry_view_bytes(black_box(&geoip), black_box("cn"))
                .expect("decode geoip entry view");
            black_box(entry.len() as u64 ^ entry[0] as u64)
        },
        iters,
        warmup,
    ))

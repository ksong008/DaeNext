...
const NATIVE_CHAIN_HASH_HEX: &str =
...
#[test]
fn case_juicity_certchain_hash_matches_native_vector() {
    let raw = case_raw_certs();
    let hash = juicity::generate_cert_chain_hash(&raw);
    assert_eq!(hex_encode(&hash), NATIVE_CHAIN_HASH_HEX);
}

#[test]
fn case_juicity_certchain_pin_verifies_url_and_std_base64() {
    let raw = case_raw_certs();

    let url_check = juicity::verify_pinned_certchain(&raw, URL_SAFE_PIN).unwrap();
    assert_eq!(url_check.pin_format, "url-base64");
    assert_eq!(hex_encode(&url_check.chain_hash), NATIVE_CHAIN_HASH_HEX);
...
#[test]
fn case_juicity_certchain_hex_looking_sha256_pin_records_native_decode_caveat() {
    let raw = case_raw_certs();
    let check = juicity::check_pinned_certchain(&raw, NATIVE_CHAIN_HASH_HEX).unwrap();
    ...
    let err = juicity::verify_pinned_certchain(&raw, NATIVE_CHAIN_HASH_HEX).unwrap_err();
    assert!(err.to_string().contains("pinned hash of cert chain"));
}

#[test]
fn case_juicity_certchain_bad_pin_fails_decode() {
    let raw = case_raw_certs();

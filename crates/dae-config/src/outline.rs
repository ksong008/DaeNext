...
fn global_outline() -> Value {
    json!({
        ...
        "structure": [
            ...
            leaf("SoMarkFromDae", "so_mark_from_dae", "uint32", None, Some("Socket mark for dae-originated traffic. When zero, dae uses its internal control-plane mark to avoid self-capture loops."), false, false),
            ...
            leaf("UdpEndpointPoolSize", "udp_endpoint_pool_size", "int", Some("4096"), Some("Deprecated compatibility field. Resident UDP capacity is selected by the runtime profile and resident_udp_session_limit; this field is not consumed by the production dataplane."), false, false),
...
4. "domain++". Based on domain+ mode but force to re-route traffic using sniffed domain to partially recover domain based traffic split ability. It doesn't work for direct traffic and consumes more CPU resources."#), false, false),
            ...
            leaf("TlsImplementation", "tls_implementation", "string", Some("tls"), Some("TLS implementation. \"tls\" is to use standard TLS. \"utls\" is to use uTLS, which can imitate browser's Client Hello."), false, false),
            leaf("UtlsImitate", "utls_imitate", "string", Some("chrome_auto"), Some("The Client Hello ID for uTLS to imitate. This takes effect only if tls_implementation is utls."), false, false),
            ...
            leaf("ResidentTcpConnectionLimit", "resident_tcp_connection_limit", "uint64", None, Some("Optional maximum number of admitted resident TCP flows. Excess connections remain in the kernel listen backlog until capacity is available."), false, false),
...
mod tests {
    ...
    #[test]
    fn outline_matches_native_export_outline_golden() {

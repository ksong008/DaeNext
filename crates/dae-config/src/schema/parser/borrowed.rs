use super::*;
pub(crate) fn parse_global(section: &Section) -> Result<Global, String> {
    ...
    for item in &section.items {
        ...
        match param.key.as_str() {
            ...
            "resident_udp_session_limit" => {
                global.resident_udp_session_limit = Some(decode_param(param, "uint64")?)
            }
            "resident_udp_session_queue_depth" => {
                global.resident_udp_session_queue_depth = Some(decode_param(param, "uint64")?)
            }
            ...
            "resident_event_queue_depth" => {
                global.resident_event_queue_depth = Some(decode_param(param, "uint64")?)
            }
            ...
            "http_queue" => global.http_queue = Some(decode_param(param, "uint64")?),
            "http_workers" => global.http_workers = Some(decode_param(param, "uint64")?),
            "http_worker_stack_bytes" => {
                global.http_worker_stack_bytes = Some(decode_param(param, "uint64")?)
            }
            "allocator_idle_reclaim_enabled" => {
                global.allocator_idle_reclaim_enabled = Some(decode_param(param, "bool")?)
            }
            "allocator_idle_reclaim_sample_interval" => {
                global.allocator_idle_reclaim_sample_interval =
                    Some(decode_param(param, "time.Duration")?)
            }
            ...
            "allocator_idle_reclaim_pressure_threshold_bytes" => {
                global.allocator_idle_reclaim_pressure_threshold_bytes =
                    Some(decode_param(param, "uint64")?)
            }
...
pub(crate) fn parse_group_section(section: &Section) -> Result<Vec<Group>, String> {
...
pub(crate) fn parse_group(group: &mut Group, section: &Section) -> Result<(), String> {
...
pub(crate) fn parse_routing(section: &Section) -> Result<Routing, String> {
...
pub(crate) fn parse_dns(section: &Section) -> Result<Dns, String> {
...
pub(crate) fn parse_dns_routing(section: &Section) -> Result<DnsRouting, String> {
...
pub(crate) fn parse_dns_rule_set(
    section: &Section,
...
pub(crate) fn parse_string_section(section: &Section) -> Result<Vec<KeyableString>, String> {
    let mut out = Vec::new();
    for item in &section.items {
        let Item::Param(param) = item else {
            return Err(format!(

...
pub(in crate::dns) enum ResidentDnsTlsStream {
    Boring(tokio_boring::SslStream<TokioTcpStream>),
}

pub(in crate::dns) struct ResidentDnsTlsConnection {
    pub(in crate::dns) tls: ResidentDnsTlsStream,
    pub(in crate::dns) reader: DnsTcpFrameReader,
}
...
impl ResidentDnsTlsStream {
    pub(in crate::dns) fn alpn_protocol(&self) -> Option<Vec<u8>> {
        let Self::Boring(stream) = self;
...
pub(crate) use self::transport::udp_multiplex::{
...
impl ResidentDnsPlan {
    ...
    pub(crate) fn reload_handle(&self) -> ResidentDnsReloadHandle {
        ResidentDnsReloadHandle::new(Arc::clone(&self.cache), self.domain_routing.clone())
    }

    pub(crate) fn restore_reload_snapshot(
        &self,

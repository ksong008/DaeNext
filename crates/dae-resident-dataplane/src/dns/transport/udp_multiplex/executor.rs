...
pub(crate) struct ResidentDnsUdpActorExecutor {
...
impl ResidentDnsUdpActorExecutor {
    ...
    #[cfg(test)]
    pub(in crate::dns) async fn open_handle(
        &self,
    ...
    pub(in crate::dns) async fn open_handle_with_config(
        &self,
        target: std::net::SocketAddr,
        mark: u32,
        runtime_config: ResidentDnsUdpRuntimeConfig,
    ...
    pub(crate) async fn spawn_actor<T, Build, BuildFuture>(&self, build: Build) -> Result<T, String>
    where
        T: Send + 'static,
    ...
    pub(in crate::dns) async fn execute_owned_task<T, Task>(&self, task: Task) -> Result<T, String>
    where
        T: Send + 'static,
    ...
    async fn reap_finished_actors(&self) -> Result<(), String> {
        ...
        let deadline = tokio::time::Instant::now() + crate::RESIDENT_RUNTIME_TASK_JOIN_GRACE;
    ...
    pub(crate) async fn join_actor_task(
        &self,
    ...
    pub(crate) async fn shutdown(&self, deadline: tokio::time::Instant) -> serde_json::Value {
        self.closing

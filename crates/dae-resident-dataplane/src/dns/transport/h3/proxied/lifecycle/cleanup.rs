...
pub(super) fn new_cleanup_deadline() -> ProxiedDoh3CleanupDeadline {
    ProxiedDoh3CleanupDeadline::from_profile()
}

pub(super) async fn cleanup_proxied_doh3_exchange<T>(target: &mut T) -> ProxiedDoh3CleanupOutcome
where
...
pub(super) fn merge_exchange_and_cleanup_result(

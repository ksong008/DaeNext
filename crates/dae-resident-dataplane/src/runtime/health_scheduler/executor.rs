...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct ResidentHealthRuntimeConfig {
...
impl ResidentHealthRuntimeConfig {
    ...
    pub(crate) fn os_thread_count(self) -> usize {
        self.worker_threads.saturating_add(self.owner_threads)
    }

    pub(crate) fn maximum_os_thread_count(self) -> usize {
        self.os_thread_count()
            .saturating_add(self.max_blocking_threads)
    }

    pub(crate) fn bootstrap_concurrency(
        self,

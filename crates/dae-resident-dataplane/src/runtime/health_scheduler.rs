...
mod executor;
pub(crate) use self::executor::ResidentHealthRuntimeConfig;
#[cfg(test)]
...
pub(crate) fn resident_health_scheduler_contract() -> Value {
    json!({
...
#[derive(Clone, Debug)]
pub(crate) struct ResidentHealthResuscitationHandle {
...
pub(crate) fn resident_health_scheduler_value(
    group_count: usize,
...
#[cfg(test)]
pub(crate) fn resident_health_scheduler_loop(
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn resident_health_scheduler_async(
    groups: Vec<Arc<plan::ResidentProxyGroupPlan>>,
    proxy_groups: SharedResidentProxyGroupMap,
    resuscitation_rx: Receiver<ResidentHealthResuscitationRequest>,
    stop: SharedResidentStopSignal,

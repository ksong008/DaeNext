...
mod raw_tcp;
pub(crate) use self::raw_tcp::*;

pub(crate) type ResidentDuplexDirectionFuture<'a> =
    Pin<Box<dyn Future<Output = Result<(), String>> + Send + 'a>>;
pub(crate) type ResidentDuplexDirection<'a> =
    Pin<&'a mut (dyn Future<Output = Result<(), String>> + Send + 'a)>;

#[derive(Clone)]
pub(crate) struct ResidentDuplexProgress {
    inner: Arc<ResidentDuplexProgressInner>,
}

pub(crate) struct ResidentDuplexActivity {
...
pub(crate) fn resident_duplex_progress() -> (ResidentDuplexProgress, ResidentDuplexActivity) {
...
impl ResidentDuplexProgress {
    ...
    pub(crate) fn snapshot(&self) -> DirectTcpRelayStats {
        DirectTcpRelayStats {
...
pub(crate) async fn run_resident_duplex_relay(
...
pub(crate) async fn run_resident_duplex_relay_borrowed(

...
impl UdpSessionExecutor {
    pub(in crate::udp) fn set_runtime_metrics(&mut self, metrics: Arc<ResidentDataplaneMetrics>) {
        if let Self::Shadowsocks2022(session) = self {
            session.set_runtime_metrics(metrics);
        }
    }

    pub(in crate::udp) fn set_owner_acquisition_deadline(
        &mut self,
    ...
    pub(in crate::udp) fn has_response_buffer(&self) -> bool {
        match self {
    ...
    pub(in crate::udp) fn reclaim_response_buffer(&mut self) -> bool {
        match self {
...
mod test_support;
mod wait;
pub(crate) use self::connect_udp::{
    clear_connect_udp_h2_pools, clear_connect_udp_h3_pools, connect_udp_pool_metrics_snapshot,
};
use self::datagram::*;
#[cfg(test)]
pub(crate) use self::test_support::{ProxyUdpSessionCheckpoint, exercise_proxy_udp_packet_session};
...
#[cfg(test)]
pub(crate) use self::anytls::exercise_anytls_udp_stream_session;
#[cfg(test)]
pub(crate) use self::quic::exercise_juicity_udp_stream_session;
use self::quic::{

...
pub(super) struct ResidentOwnedTaskShutdown<T> {
...
impl<T> ResidentOwnedTaskShutdown<T> {
    ...
    pub(super) fn safety_status(&self) -> &'static str {
        self.status()
    }

    pub(super) fn graceful(&self) -> bool {
        self.completed_safely() && !self.forced
    }

    pub(super) fn completion_mode(&self) -> &'static str {
        if !self.completed_safely() {
            "incomplete"
        } else if self.forced {
            "forced-bounded"
...
pub(super) async fn shutdown_resident_owned_task<T>(

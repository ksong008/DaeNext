...
impl ResidentProxyDnsUdpForwarder {
    ...
    pub(crate) fn new_with_optional_transport_owner(
        binding: ResidentProxyBinding,
        original_dst: SocketAddr,
        runtime_config: ResidentDnsUdpRuntimeConfig,
        metrics: Arc<ResidentDataplaneMetrics>,
    ...
    #[cfg(test)]
    pub(crate) async fn exchange(&self, payload: &[u8]) -> Result<Vec<u8>, String> {
    ...
    pub(crate) async fn exchange_with_context(
        &self,
        payload: &[u8],
    ...
    pub(crate) async fn shutdown(&self, deadline: time::Instant) -> Value {
    ...
    pub(crate) fn owner_observation(&self) -> Arc<ResidentDnsTransportOwnerObservation> {

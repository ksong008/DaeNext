...
impl Entry {
    ...
    pub fn decode(ctx: &SslContextRef, mut encoded: Bytes) -> Result<Self> {
        // Decode the session.
        let len = take_length_prefix(&mut encoded, "session")?;
        if encoded.remaining() < len {
            return Err(Error::invalid_input(format!(
                "cached session length {len} exceeds remaining {} bytes",
                encoded.remaining()
            )));
        }
        ...
        let len = take_length_prefix(&mut encoded, "transport parameters")?;
        if encoded.remaining() < len {
            return Err(Error::invalid_input(format!(
                "cached transport parameters length {len} exceeds remaining {} bytes",
                encoded.remaining()
            )));
        }
        ...
        if encoded_params.has_remaining() || encoded.has_remaining() {
            return Err(Error::invalid_input(
                "cached session entry contains trailing bytes".to_owned(),
            ));
        }
...
fn take_length_prefix(encoded: &mut Bytes, field: &str) -> Result<usize> {
    if encoded.remaining() < std::mem::size_of::<u64>() {
        return Err(Error::invalid_input(format!(
            "cached {field} is missing its length prefix"
        )));
    }
    usize::try_from(encoded.get_u64())
        .map_err(|_| Error::invalid_input(format!("cached {field} length exceeds usize")))
}
...
mod tests {
    ...
    use crate::{ClientConfig, QuicSslSession};
    ...
    fn entry_encoding() {
        ...
        let mut decoded_session = BytesMut::new();
        decoded.session.encode(&mut decoded_session).unwrap();
        let session_length = u64::from_be_bytes(encoded[..8].try_into().unwrap()) as usize;
        assert_eq!(&decoded_session[..], &encoded[8..8 + session_length]);

        let re_encoded = decoded.encode().unwrap();
        let re_decoded = Entry::decode(ctx, re_encoded).unwrap();
        assert_eq!(decoded.params, re_decoded.params);
    }

    #[test]
    fn entry_decode_rejects_truncated_lengths_without_panicking() {
        let cfg = ClientConfig::new().unwrap();
        let ctx = cfg.ctx().as_ref();

        let missing_prefix = Entry::decode(ctx, Bytes::from_static(&[0; 7]));
        assert!(missing_prefix.is_err());

        let mut truncated_session = BytesMut::new();
        truncated_session.put_u64(16);
        truncated_session.extend_from_slice(&[0; 4]);
        let truncated_session = Entry::decode(ctx, truncated_session.freeze());
        assert!(truncated_session.is_err());
    }
}

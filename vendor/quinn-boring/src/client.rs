...
type CustomVerifyCallback =
    dyn Fn(&mut boring::ssl::SslRef) -> std::result::Result<(), SslVerifyError> + Send + Sync;
type VerifyCallback = dyn Fn(bool, &mut X509StoreContextRef) -> bool + Send + Sync;

/// Configuration for a client-side QUIC. Wraps around a BoringSSL [SslContext].
pub struct Config {
    ctx: SslContext,
    session_cache: Arc<dyn SessionCache>,
    session_cache_namespace: Bytes,
    key_log: Option<Arc<dyn KeyLog>>,
    custom_verify_callback: Option<Arc<CustomVerifyCallback>>,
...
impl Config {
    ...
    /// Sets an application-defined cache namespace. The namespace is combined
    /// with SNI so configurations with different ALPN, verification, or
    /// ClientHello policy cannot consume each other's tickets.
    pub fn set_session_cache_namespace(&mut self, namespace: impl Into<Bytes>) {
        self.session_cache_namespace = namespace.into();
    }
    ...
    /// Installs a per-connection custom certificate verifier. Quinn creates a
    /// fresh `Ssl` for every handshake, so this callback must be copied onto
    /// that `Ssl`; installing it only on the shared `SSL_CTX` is insufficient.
    pub fn set_custom_verify_callback<F>(&mut self, mode: SslVerifyMode, callback: F)
    where
        F: Fn(&mut boring::ssl::SslRef) -> std::result::Result<(), SslVerifyError>
            + Send
            + Sync
            + 'static,
    {
        self.custom_verify_callback = Some(Arc::new(callback));
        self.ctx.verify_peer(!mode.is_empty());
    }

    /// Installs a per-connection certificate-chain verification callback.
...
impl Session {
    fn new(
        ...
        let session_cache_key = scoped_session_cache_key(&cfg.session_cache_namespace, server_name);
        let mut ssl = Ssl::new(&cfg.ctx).unwrap();

        if let Some(callback) = &cfg.custom_verify_callback {
            let callback = Arc::clone(callback);
            ssl.set_custom_verify_callback(SslVerifyMode::PEER, move |ssl| callback(ssl));
        }
        if let Some(callback) = &cfg.verify_callback {
            let callback = Arc::clone(callback);
            ssl.set_verify_callback(SslVerifyMode::PEER, move |ok, context| {
                callback(ok, context)
            });
        }
        ...
        if let Some(entry) = session_cache.get(session_cache_key.clone()) {
            match Entry::decode(ssl.ssl_context(), entry) {
                Ok(entry) => {
                    zero_rtt_peer_params =
                        entry.session.early_data_capable().then_some(entry.params);
                    match unsafe { ssl.set_session(entry.session.as_ref()) } {
                        Ok(()) => {
                            trace!("attempting session resumption for server: {}.", server_name);
                        }
                        Err(e) => {
                            warn!(
                ...
                Err(e) => {
                    ...
                    session_cache.remove(session_cache_key.clone());
                }
            }
        } else {
            trace!(
    ...
    fn on_zero_rtt_rejected(&mut self) {
        ...
        self.session_cache.remove(self.session_cache_key.clone());
    ...
    fn on_new_session(&mut self, session: SslSession) {
        ...
        match entry.encode() {
            Ok(value) => {
                // Cache the session.
                self.session_cache
                    .put(self.session_cache_key.clone(), value)
            }
            Err(e) => {
...
fn scoped_session_cache_key(namespace: &[u8], server_name: &str) -> Bytes {
    let server_name = server_name.as_bytes();
    let mut key = BytesMut::with_capacity(16 + namespace.len() + server_name.len());
    key.put_u64(namespace.len() as u64);
    key.put_slice(namespace);
    key.put_u64(server_name.len() as u64);
    key.put_slice(server_name);
    key.freeze()
}

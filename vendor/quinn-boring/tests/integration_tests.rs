...
#[tokio::test]
async fn shared_cache_resumes_across_configs_without_application_zero_rtt() -> Result<()> {
    let _guard = subscribe();
    let server = Server::run(server_config(server_crypto())?)?;
    let session_cache: Arc<dyn quinn_boring::SessionCache> =
        Arc::new(quinn_boring::SimpleCache::new(8));

    let mut first_crypto = client_crypto();
    first_crypto.ctx_mut().enable_early_data(false);
    first_crypto.set_session_cache(Arc::clone(&session_cache));
    first_crypto.set_session_cache_namespace(Bytes::from_static(b"policy-a"));
    let first = server.connect_1rtt(client_config(first_crypto)).await?;
    first.send_ping().await?;
    first.receive_pong().await?;
    assert!(!first.conn_info().handshake_data.session_reused);

    let mut second_crypto = client_crypto();
    second_crypto.ctx_mut().enable_early_data(false);
    second_crypto.set_session_cache(Arc::clone(&session_cache));
    second_crypto.set_session_cache_namespace(Bytes::from_static(b"policy-a"));
    let second = server.connect_1rtt(client_config(second_crypto)).await?;
    second.send_ping().await?;
    second.receive_pong().await?;
    assert!(second.conn_info().handshake_data.session_reused);

    let mut isolated_crypto = client_crypto();
    isolated_crypto.ctx_mut().enable_early_data(false);
    isolated_crypto.set_session_cache(session_cache);
    isolated_crypto.set_session_cache_namespace(Bytes::from_static(b"policy-b"));
    let isolated = server.connect_1rtt(client_config(isolated_crypto)).await?;
    isolated.send_ping().await?;
    isolated.receive_pong().await?;
    assert!(!isolated.conn_info().handshake_data.session_reused);
    Ok(())
}

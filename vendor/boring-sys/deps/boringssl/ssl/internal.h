...
class SSLKeyShare {
 public:
  ...
  // GetX25519PrivateKeyForDaeReality writes the X25519 private key used by this
  // key share to |out|. It returns false for non-X25519 key shares. This is a
  // DaeNext-specific internal hook used to bind Reality authentication to the
  // same key share used by the TLS 1.3 key schedule.
  virtual bool GetX25519PrivateKeyForDaeReality(uint8_t out[32]) const {
    return false;
  }
};
...
struct DAERealityConfig {
  bool enabled = false;
  bool auth_key_ready = false;
  uint8_t server_public_key[32] = {0};

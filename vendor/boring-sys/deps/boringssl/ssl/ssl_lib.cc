...
int DAE_SSL_set1_reality_config(SSL *ssl, const uint8_t *server_public_key,
  ...
  if (ssl == nullptr || ssl->config == nullptr ||
      server_public_key == nullptr || client_version == nullptr ||
      server_public_key_len != 32 || short_id_len > 8 ||
      (short_id_len > 0 && short_id == nullptr) || SSL_is_server(ssl)) {
    OPENSSL_PUT_ERROR(SSL, SSL_R_BAD_LENGTH);
    return 0;
  }
  ...
  DAERealityConfig *config = &ssl->config->dae_reality;
  *config = DAERealityConfig{};
  config->enabled = true;
  OPENSSL_memcpy(config->server_public_key, server_public_key, 32);
  OPENSSL_memcpy(config->client_version, client_version, 3);
  if (short_id_len > 0) {
    OPENSSL_memcpy(config->short_id, short_id, short_id_len);
  }
...
int DAE_SSL_get0_reality_auth_key(const SSL *ssl, const uint8_t **out_key,
                                  size_t *out_key_len) {
  if (ssl == nullptr || ssl->config == nullptr || out_key == nullptr ||
      out_key_len == nullptr || !ssl->config->dae_reality.auth_key_ready) {
    return 0;
  }

  *out_key = ssl->config->dae_reality.auth_key;
  *out_key_len = sizeof(ssl->config->dae_reality.auth_key);
  return 1;
}

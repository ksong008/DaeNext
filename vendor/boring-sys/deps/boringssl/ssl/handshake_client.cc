...
static bool dae_reality_apply_session_id(SSL_HANDSHAKE *hs,
  ...
  if (hs->max_version < TLS1_3_VERSION || hs->early_data_offered ||
      hs->selected_ech_config || SSL_is_dtls(ssl) || SSL_is_quic(ssl) ||
  ...
  if (!dae_reality_x25519_private_key(hs, private_key) ||
      ...
      !HKDF_extract(prk, &prk_len, EVP_sha256(), shared_secret,
  ...
  OPENSSL_memcpy(plaintext, config->client_version,
                 sizeof(config->client_version));
  const uint32_t unix_seconds = static_cast<uint32_t>(time(nullptr));
  plaintext[4] = static_cast<uint8_t>(unix_seconds >> 24);
  plaintext[5] = static_cast<uint8_t>(unix_seconds >> 16);
  plaintext[6] = static_cast<uint8_t>(unix_seconds >> 8);
  plaintext[7] = static_cast<uint8_t>(unix_seconds);
  OPENSSL_memcpy(plaintext + 8, config->short_id, config->short_id_len);

  ScopedEVP_AEAD_CTX aead;
  if (!EVP_AEAD_CTX_init(aead.get(), EVP_aead_aes_256_gcm(),
                         config->auth_key, sizeof(config->auth_key),
                         EVP_AEAD_DEFAULT_TAG_LENGTH, nullptr) ||
      !EVP_AEAD_CTX_seal(aead.get(), session_id, &session_id_len,
                         sizeof(session_id), ssl->s3->client_random + 20, 12,
                         plaintext, sizeof(plaintext), aad.data(),
                         aad.size()) ||
      session_id_len != sizeof(session_id)) {
    OPENSSL_cleanse(private_key, sizeof(private_key));
    OPENSSL_cleanse(shared_secret, sizeof(shared_secret));
    OPENSSL_cleanse(prk, sizeof(prk));
  ...
  hs->session_id.CopyFrom(Span<const uint8_t>(session_id, sizeof(session_id)));
  config->auth_key_ready = true;
  OPENSSL_cleanse(private_key, sizeof(private_key));
  OPENSSL_cleanse(shared_secret, sizeof(shared_secret));
  OPENSSL_cleanse(prk, sizeof(prk));
  OPENSSL_cleanse(plaintext, sizeof(plaintext));

...
// |short_id| may be empty and must be at most eight bytes. |client_version|
// must point to a three-byte Reality client version.
//
// When enabled, BoringSSL sends a single X25519 key share, computes the Reality
// encrypted session_id from the final ClientHello shape with a zero session_id
// as AAD, and stores the per-connection Reality auth key for certificate
// verification. This API is DaeNext-specific and intentionally narrow.
OPENSSL_EXPORT int DAE_SSL_set1_reality_config(
    SSL *ssl, const uint8_t *server_public_key, size_t server_public_key_len,
    const uint8_t *short_id, size_t short_id_len,
    const uint8_t client_version[3]);

// DAE_SSL_get0_reality_auth_key returns the per-connection Reality auth key
// computed during ClientHello construction, or zero if it is not available.
// The returned pointer is owned by |ssl| and is valid while the handshake config
// is retained.
OPENSSL_EXPORT int DAE_SSL_get0_reality_auth_key(const SSL *ssl,

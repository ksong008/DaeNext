...
#[derive(Debug, Clone)]
pub(super) struct Stage136Options {
...
impl Default for Stage136Options {
    fn default() -> Self {
        Self {
            execute_smoke: false,
            benchmark_iters: 1,
    ...
    pub(super) fn parse(args: &[String]) -> Result<Self, RunnerOutput> {
            ...
            match arg.as_str() {
                ...
                "--benchmark-iters" => {
                    opts.benchmark_iters =
                        parse_usize(&next_value(&mut iter, "stage136 --benchmark-iters")?, arg)?;
                }
                "--vless-uuid" => opts.vless_uuid = next_value(&mut iter, "stage136 --vless-uuid")?,
                "--vmess-uuid" => opts.vmess_uuid = next_value(&mut iter, "stage136 --vmess-uuid")?,
                "--vless-target" => {
                    opts.vless_target = next_value(&mut iter, "stage136 --vless-target")?
                }
                "--vmess-target" => {
                    opts.vmess_target = next_value(&mut iter, "stage136 --vmess-target")?
                }
                "--xhttp-host" => opts.xhttp_host = next_value(&mut iter, "stage136 --xhttp-host")?,
                "--xhttp-path" => opts.xhttp_path = next_value(&mut iter, "stage136 --xhttp-path")?,
                "--xhttp-mode" => opts.xhttp_mode = next_value(&mut iter, "stage136 --xhttp-mode")?,
                "--xhttp-security" => {
                    opts.xhttp_security = next_value(&mut iter, "stage136 --xhttp-security")?
                }
                "--xhttp-alpn" => opts.xhttp_alpn = next_value(&mut iter, "stage136 --xhttp-alpn")?,
                "--xhttp-session-id" => {
                    opts.xhttp_session_id = next_value(&mut iter, "stage136 --xhttp-session-id")?
                }
                "--xhttp-seq" => {
                    opts.xhttp_seq =
                        parse_u64(&next_value(&mut iter, "stage136 --xhttp-seq")?, arg)?;
                }
                "--vless-payload" => {
                    opts.vless_payload =
                        next_value(&mut iter, "stage136 --vless-payload")?.into_bytes()
                }
                "--vmess-payload" => {
                    opts.vmess_payload =
                        next_value(&mut iter, "stage136 --vmess-payload")?.into_bytes()
                }
                "--timeout-ms" => {
                    let timeout_ms =
                        parse_u64(&next_value(&mut iter, "stage136 --timeout-ms")?, arg)?;
                ...
                _ => {
                    return Err(RunnerOutput::usage(format!(
                        "unsupported stage136 argument: {arg}"
                    )));
                }
            }
        }
        if opts.benchmark_iters == 0 {
            return Err(RunnerOutput::usage(
                "stage136 --benchmark-iters must be greater than zero",
            ));
        }
        vless::password_to_key(&opts.vless_uuid)
            .map_err(|err| RunnerOutput::usage(format!("stage136 vless uuid is invalid: {err}")))?;
        vmess::vmess_cmd_key_from_uuid(&opts.vmess_uuid)
            .map_err(|err| RunnerOutput::usage(format!("stage136 vmess uuid is invalid: {err}")))?;
        dae_outbound::VMessMetadata::parse("tcp", &opts.vless_target).map_err(|err| {
            RunnerOutput::usage(format!("stage136 vless target is invalid: {err}"))
        })?;
        dae_outbound::VMessMetadata::parse("tcp", &opts.vmess_target).map_err(|err| {
            RunnerOutput::usage(format!("stage136 vmess target is invalid: {err}"))
        })?;
        opts.xhttp_options_for_seq(opts.xhttp_seq)
            .map_err(|err| RunnerOutput::usage(format!("stage136 xhttp options invalid: {err}")))?;
        if opts.xhttp_mode != "packet-up" {
            return Err(RunnerOutput::usage(
                "stage136 --xhttp-mode must remain packet-up",
            ));
        }
        if opts.xhttp_alpn != "h2" {
            return Err(RunnerOutput::usage(
                "stage136 --xhttp-alpn must remain h2 for the HTTP/2 gate",
            ));
        }
        Ok(opts)
    }

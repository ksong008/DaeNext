...
const DEFAULT_TARGET: &str = "stage72-vmess-meek.example:443";
const DEFAULT_MEEK_URL: &str = "https://front.example/dae-stage72-meek";
const DEFAULT_MEEK_SESSION_TAG: &[u8] = b"dae-stage72-meek";
const DEFAULT_PAYLOAD: &[u8] = b"stage72-vmess-meek-ping";

pub(crate) fn run_stage72_vmess_meek_polling_dataplane_admission(args: &[String]) -> RunnerOutput {
    let opts = match Stage72Options::parse(args) {
        Ok(opts) => opts,
        Err(output) => return output,
    };
    let report = stage72_report(&opts);
    let passed = report["vmess_meek_polling_smoke_passed"]
        .as_bool()
        .unwrap_or(false);
...
impl Default for Stage72Options {
    fn default() -> Self {
        Self {
            execute_smoke: false,
            ack_root_gate: false,
...
impl Stage72Options {
    fn parse(args: &[String]) -> Result<Self, RunnerOutput> {
            ...
            match arg.as_str() {
                ...
                "--benchmark-iters" => {
                    opts.benchmark_iters =
                        parse_usize(&next_value(&mut iter, "stage72 --benchmark-iters")?, arg)?;
                }
                "--uuid" => opts.uuid = next_value(&mut iter, "stage72 --uuid")?,
                "--target" => opts.target = next_value(&mut iter, "stage72 --target")?,
                "--meek-url" => opts.meek_url = next_value(&mut iter, "stage72 --meek-url")?,
                "--meek-session-tag" => {
                    opts.meek_session_tag =
                        next_value(&mut iter, "stage72 --meek-session-tag")?.into_bytes();
                }
                "--payload" => {
                    opts.payload = next_value(&mut iter, "stage72 --payload")?.into_bytes();
                }
                "--so-mark" => {
                    opts.so_mark = parse_u32(&next_value(&mut iter, "stage72 --so-mark")?, arg)?;
                }
                "--mptcp" => opts.mptcp = true,
                "--no-mptcp" => opts.mptcp = false,
                "--timeout-ms" => {
                    let timeout_ms =
                        parse_u64(&next_value(&mut iter, "stage72 --timeout-ms")?, arg)?;
                ...
                _ => {
                    return Err(RunnerOutput::usage(format!(
                        "unsupported stage72 argument: {arg}"
                    )));
                }
            }
        }
        if opts.benchmark_iters == 0 {
            return Err(RunnerOutput::usage(
                "stage72 --benchmark-iters must be greater than zero",
            ));
        }
        vmess::vmess_cmd_key_from_uuid(&opts.uuid)
            .map_err(|err| RunnerOutput::usage(format!("stage72 uuid is invalid: {err}")))?;
        dae_outbound::VMessMetadata::parse("tcp", &opts.target)
            .map_err(|err| RunnerOutput::usage(format!("stage72 target is invalid: {err}")))?;
        shared_transport::MeekRoundTripOptions::from_https_url(
            &opts.meek_url,
            opts.meek_session_tag.clone(),
        )
        .map_err(|err| RunnerOutput::usage(format!("stage72 meek url is invalid: {err}")))?;
        Ok(opts)
    }
    ...
    fn meek_options(&self) -> shared_transport::MeekRoundTripOptions {
        ...
        .expect("validated stage72 meek url")
    }
}

fn stage72_report(opts: &Stage72Options) -> Value {
    let cmd_key = match vmess::vmess_cmd_key_from_uuid(&opts.uuid) {
        Ok(key) => key,
        Err(err) => {
            return json!({
                "name": "stage72-vmess-meek-polling-dataplane-admission",
                "stage": "stage72",
                "blocked": true,
                "blockers": [format!("stage72 uuid is invalid: {err}")]
            });
        }
    };
    ...
    let mut report = json!({
        "name": "stage72-vmess-meek-polling-dataplane-admission",
        "stage": "stage72",
        "evidence_class": "opt-in-protocol-vmess-aead-meek-polling-shared-transport-dataplane-smoke",
    ...
    report["vmess_meek_polling_smoke_passed"] = json!(false);
    report["vmess_meek_polling_admitted"] = json!(false);
    ...
    report["vmess_meek_polling_contract"] = json!({
        "protocol": "vmess",
        "scope": "VMess AEAD TCP request/response carried by Meek HTTP polling bodies over a Rust TCP stream",
        "uuid": opts.uuid,
        ...
        "other_shared_transport_deferred": "VMess WSS, HTTP/H2, xHTTP, full gRPC HTTP/2, and full Meek HTTPS lifecycle require separate transport gates",
    ...
    report["benchmark"] = json!({
        ...
        "elapsed_ns": null,
        "ns_per_vmess_meek_polling_exchange": null,
        "scope": "VMess AEAD TCP request/response carried by Meek polling HTTP bodies on a SO_MARKed Rust TCP socket",
    ...
    report["protocol_matrix"] = json!({
        ...
        "vmess_meek_polling_admitted": false,
    ...
    if !opts.ack_root_gate {
        ...
        report["blockers"] = json!([
            "stage72 root-gated smoke requires --ack-root-gate because it attempts SO_MARK/MPTCP underlay socket observation"
        ]);
        return report;
    }

    match run_stage72_smoke(opts) {
        Ok(outcome) => apply_stage72_outcome(&mut report, outcome),
        Err(err) => {
            report["blocked"] = json!(true);
...
fn run_stage72_smoke(opts: &Stage72Options) -> Result<Stage72Outcome, String> {
    let (server_addr, listener_report, handle) = spawn_vmess_meek_polling_server(opts)?;
    ...
    for _ in 0..opts.benchmark_iters {
        ...
        .map_err(|err| format!("stage72 magic_tcp_connect failed: {err}"))?;
        ...
        .map_err(|err| format!("stage72 VMess Meek polling exchange failed: {err}"))?;
        if report.echoed_payload != opts.payload {
            return Err("stage72 VMess Meek polling payload response mismatch".to_owned());
        }
    ...
    let server_summary = handle
        .join()
        .map_err(|_| "stage72 VMess Meek polling server thread panicked".to_owned())??;
    if server_summary.accepted != opts.benchmark_iters {
        return Err(format!(
            "stage72 VMess Meek polling server accepted {} connections, want {}",
            server_summary.accepted, opts.benchmark_iters
        ));
    }
    let last_exchange =
        last_exchange.ok_or_else(|| "stage72 missing exchange report".to_owned())?;
    Ok(Stage72Outcome {
        listener_report,
        last_dial_report: last_dial_report
            .ok_or_else(|| "stage72 missing underlay dial report".to_owned())?,
        server_summary,
        elapsed_ns,
...
fn apply_stage72_outcome(report: &mut Value, outcome: Stage72Outcome) {
    ...
    report["vmess_meek_polling_smoke_passed"] = json!(passed);
    report["vmess_meek_polling_admitted"] = json!(passed);
    report["vmess_shared_transport_partial_admitted"] = json!(true);
    report["vmess_protocol_partial_admitted"] = json!(true);
    report["vmess_meek_polling_contract"]["server"] = json!(outcome.last_dial_report.peer_addr);
    report["vmess_meek_polling_contract"]["cmd_key_hex"] = json!(outcome.cmd_key_hex);
    report["vmess_meek_polling_contract"]["meek_polling_validated"] = json!(passed);
    report["vmess_meek_polling_contract"]["meek_session_id_validated"] = json!(passed);
    report["vmess_meek_polling_contract"]["eauth_crc_validated"] = json!(passed);
    report["vmess_meek_polling_contract"]["request_header_aead_validated"] = json!(passed);
    report["vmess_meek_polling_contract"]["response_header_aead_validated"] = json!(passed);
    report["vmess_meek_polling_contract"]["shake128_chunk_masking_validated"] = json!(passed);
    report["vmess_meek_polling_contract"]["tcp_command_validated"] = json!(passed);
    report["vmess_meek_polling_contract"]["target_metadata_validated"] = json!(passed);
    report["vmess_meek_polling_contract"]["payload_roundtrip_validated"] = json!(passed);
    ...
    report["benchmark"]["ns_per_vmess_meek_polling_exchange"] = json!(outcome.ns_per_exchange);
    ...
    report["protocol_matrix"]["vmess_meek_polling_admitted"] = json!(passed);
}

#[derive(Debug, Default)]
struct VMessMeekPollingServerSummary {
    accepted: usize,
...
fn spawn_vmess_meek_polling_server(
    opts: &Stage72Options,
) -> Result<
    (
        SocketAddrV4,
        TcpLoopbackListenerReport,
        thread::JoinHandle<Result<VMessMeekPollingServerSummary, String>>,
    ),
    String,
> {
    let (listener, listener_report) = bind_loopback_tcp_listener(opts.mptcp)
        .map_err(|err| format!("stage72 bind loopback VMess Meek polling server failed: {err}"))?;
    let local_addr = listener
        .local_addr()
        .map_err(|err| format!("stage72 VMess Meek polling server local_addr failed: {err}"))?;
    let server_addr = match local_addr {
        SocketAddr::V4(addr) => addr,
        SocketAddr::V6(addr) => {
            return Err(format!(
                "stage72 VMess Meek polling listener is not IPv4: {addr}"
            ));
        }
    };
    listener
        .set_nonblocking(true)
        .map_err(|err| format!("stage72 VMess Meek polling nonblocking failed: {err}"))?;
...
fn accept_vmess_meek_polling(
    ...
    timeout: Duration,
) -> Result<VMessMeekPollingServerSummary, String> {
    let mut summary = VMessMeekPollingServerSummary::default();
        ...
        match listener.accept() {
            Ok((mut stream, _)) => {
                stream
                    ...
                    .map_err(|err| format!("stage72 server set read timeout failed: {err}"))?;
                stream
                    .set_write_timeout(Some(timeout))
                    .map_err(|err| format!("stage72 server set write timeout failed: {err}"))?;
                handle_vmess_meek_polling(
                    &mut stream,
                    uuid,
                    expected_target,
                    expected_payload,
            ...
            Err(err) if err.kind() == ErrorKind::WouldBlock => {
                return Err(format!(
                    "stage72 VMess Meek polling server timed out after accepting {} of {} connections",
                    summary.accepted, iterations
                ));
            }
            Err(err) => return Err(format!("stage72 VMess Meek polling accept failed: {err}")),
        }
    }
    Ok(summary)
}

fn handle_vmess_meek_polling(
    stream: &mut TcpStream,
    uuid: &str,
    expected_target: &str,
    expected_payload: &[u8],
    meek_options: &shared_transport::MeekRoundTripOptions,
    summary: &mut VMessMeekPollingServerSummary,
) -> Result<(), String> {
    let request = vmess::read_aead_tcp_request_from_meek_polling_stream(stream, uuid, meek_options)
        .map_err(|err| format!("stage72 read VMess Meek polling request failed: {err}"))?;
    if !request.request.eauth_crc_validated {
        return Err("stage72 VMess EAuthID checksum was not validated".to_owned());
    }
    if request.request.command != dae_outbound::VMessNetwork::Tcp.byte() {
        return Err(format!(
            "stage72 VMess command mismatch: got {}, want {}",
    ...
    if request.request.target != expected_target {
        return Err(format!(
            "stage72 VMess target mismatch: got {}, want {expected_target}",
            request.request.target
        ));
    }
    if request.request.payload != expected_payload {
        return Err("stage72 VMess Meek polling payload mismatch".to_owned());
    }
    let response = vmess::aead_tcp_response_packet(&request.request, &request.request.payload)
        .map_err(|err| format!("stage72 encode VMess Meek polling response failed: {err}"))?;
    stream
        .write_all(
            format!(
                "HTTP/1.1 200 OK\r\nContent-Length: {}\r\n\r\n",
        ...
        .map_err(|err| format!("stage72 write Meek response head failed: {err}"))?;
    stream
        .write_all(&response)
        .map_err(|err| format!("stage72 write Meek response body failed: {err}"))?;

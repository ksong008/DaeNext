...
#[derive(Debug)]
pub(super) struct Stage85Outcome {
    pub(super) listener_report: TcpLoopbackListenerReport,
    pub(super) last_dial_report: TcpDirectDialReport,
    pub(super) client_report: trojan::TrojanGoHttpUpgradeTcpExchangeReport,
    pub(super) server_summary: TrojanGoHttpUpgradeServerSummary,
...
pub(super) fn run_stage85_smoke(opts: &Stage85Options) -> Result<Stage85Outcome, String> {
    let tls_options = opts
        .tls_options()
        .map_err(|err| format!("stage85 tls options invalid: {err}"))?;
    let material = shared_transport::tls_loopback_material(&tls_options)
        .map_err(|err| format!("stage85 build tls material failed: {err}"))?;
    let certificate_der_len = material.certificate_der_len;
    let (server_addr, listener_report, handle) =
        spawn_trojan_go_httpupgrade_server(opts, &material)?;
    ...
    for _ in 0..opts.benchmark_iters {
        ...
        .map_err(|err| format!("stage85 magic_tcp_connect failed: {err}"))?;
        ...
        .map_err(|err| format!("stage85 trojan-go HTTPUpgrade exchange failed: {err}"))?;
        if report.echoed_payload != opts.payload {
            return Err("stage85 trojan-go HTTPUpgrade payload response mismatch".to_owned());
        }
    ...
    let server_summary = handle
        .join()
        .map_err(|_| "stage85 trojan-go HTTPUpgrade server thread panicked".to_owned())??;
    if server_summary.accepted != opts.benchmark_iters {
        return Err(format!(
            "stage85 trojan-go HTTPUpgrade server accepted {} connections, want {}",
    ...
    Ok(Stage85Outcome {
        ...
        last_dial_report: last_dial_report
            .ok_or_else(|| "stage85 missing underlay dial report".to_owned())?,
        client_report: last_client_report
            .ok_or_else(|| "stage85 missing trojan-go HTTPUpgrade client report".to_owned())?,
        server_summary,
        certificate_der_len,
        elapsed_ns,
...
pub(super) fn apply_stage85_outcome(report: &mut Value, outcome: Stage85Outcome) {
    ...
    let server_complete = outcome.server_summary.accepted == outcome.exchange_count
        ...
        && outcome.server_summary.httpupgrade_count == outcome.exchange_count
    ...
    let client_complete = outcome.client_report.true_dataplane
        && outcome.client_report.trojan_go_httpupgrade
        ...
        && outcome.client_report.httpupgrade_handshake_validated;
    let passed = server_complete && client_complete && so_mark_observed && mptcp_status_recorded;

    report["read_only"] = json!(false);
    report["trojan_go_httpupgrade_smoke_passed"] = json!(passed);
    report["trojan_go_httpupgrade_admitted"] = json!(passed);
    report["trojan_go_httpupgrade_contract"]["server"] = json!(outcome.last_dial_report.peer_addr);
    report["trojan_go_httpupgrade_contract"]["selected_alpn"] =
        json!(outcome.client_report.selected_alpn);
    report["trojan_go_httpupgrade_contract"]["certificate_der_len"] =
        json!(outcome.certificate_der_len);
    report["trojan_go_httpupgrade_contract"]["tls_handshake_validated"] = json!(passed);
    report["trojan_go_httpupgrade_contract"]["certificate_chain_validated"] = json!(passed);
    report["trojan_go_httpupgrade_contract"]["server_name_validated"] = json!(passed);
    report["trojan_go_httpupgrade_contract"]["alpn_validated"] = json!(passed);
    report["trojan_go_httpupgrade_contract"]["httpupgrade_validated"] = json!(passed);
    report["trojan_go_httpupgrade_contract"]["password_sha224_validated"] = json!(passed);
    report["trojan_go_httpupgrade_contract"]["tcp_command_validated"] = json!(passed);
    report["trojan_go_httpupgrade_contract"]["target_metadata_validated"] = json!(passed);
    report["trojan_go_httpupgrade_contract"]["payload_roundtrip_validated"] = json!(passed);
    ...
    report["server_observation"] = json!({
        ...
        "httpupgrade_count": outcome.server_summary.httpupgrade_count,
    ...
    report["benchmark"]["ns_per_trojan_go_httpupgrade_exchange"] = json!(outcome.ns_per_exchange);
    report["benchmark"]["payload_len"] = json!(outcome.client_report.payload_len);
    report["benchmark"]["httpupgrade_request_len"] =
        json!(outcome.client_report.httpupgrade_request_len);
    report["protocol_matrix"]["trojan_go_httpupgrade_admitted"] = json!(passed);
}

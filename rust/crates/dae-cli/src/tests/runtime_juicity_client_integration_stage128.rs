use super::*;

#[test]
fn stage128_juicity_client_integration_fixture_matches() {
    let fixture = load("engine/runtime_stage128/juicity_client_integration_admission.json");
    let output = run_with_args(["runtime", "stage128-juicity-client-integration-admission"]);
    assert_eq!(output.exit_code, 0, "{}", output.stdout);
    assert_eq!(output.stderr, "");
    let json: Value = serde_json::from_str(&output.stdout).unwrap();

    assert_eq!(json, fixture);
    assert!(json["read_only"].as_bool().unwrap());
    assert!(json["blocked"].as_bool().unwrap());
    assert!(
        json["juicity_congestion_behavior_admitted"]
            .as_bool()
            .unwrap()
    );
    assert!(
        !json["juicity_client_integration_candidate_admitted"]
            .as_bool()
            .unwrap()
    );
    assert!(
        !json["juicity_true_quic_h3_dataplane_admitted"]
            .as_bool()
            .unwrap()
    );
    assert!(!json["outbound_true_dataplane_admitted"].as_bool().unwrap());
    assert_eq!(json["client_integration"]["total_exchange_count"], 19);
    assert_eq!(json["client_integration"]["auth_iterations"], 1);
    assert_eq!(json["client_integration"]["transport_iterations"], 8);
    assert_eq!(json["client_integration"]["stream_iterations"], 2);
    assert_eq!(json["client_integration"]["congestion_iterations"], 8);
    assert_eq!(json["client_integration"]["max_in_flight_streams"], 4);
}

#[test]
fn stage128_juicity_client_integration_rejects_bad_args() {
    for (flag, message) in [
        (
            "--auth-iters=0",
            "stage128 --auth-iters must be greater than zero",
        ),
        (
            "--transport-iters=0",
            "stage128 --transport-iters must be greater than zero",
        ),
        (
            "--stream-iters=0",
            "stage128 --stream-iters must be greater than zero",
        ),
        (
            "--congestion-iters=0",
            "stage128 --congestion-iters must be greater than zero",
        ),
        (
            "--max-in-flight-streams=0",
            "stage128 --max-in-flight-streams must be greater than zero",
        ),
    ] {
        let blocked = run_with_args([
            "runtime",
            "stage128-juicity-client-integration-admission",
            flag,
        ]);
        assert_eq!(blocked.exit_code, 2);
        assert!(blocked.stderr.contains(message), "{}", blocked.stderr);
    }
}

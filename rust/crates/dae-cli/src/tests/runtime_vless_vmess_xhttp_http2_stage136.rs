...
#[test]
fn stage136_runtime_admission_fixture_matches() {
    let fixture = load("engine/runtime_stage136/vless_vmess_xhttp_http2_lifecycle_admission.json");
    let output = run_with_args([
        "runtime",
        "stage136-vless-vmess-xhttp-http2-lifecycle-admission",
    ]);
    ...
    assert_eq!(
        json["vless_vmess_xhttp_http2_contract"]["xhttp_alpn"]
            .as_str()
            .unwrap(),
        "h2"
    );
    assert!(
        json["vless_vmess_xhttp_http2_contract"]["h3_deferred"]
            .as_bool()
            .unwrap()
    );
}
...
#[test]
fn stage136_runtime_execute_smoke_admits_only_xhttp_http2_subrows() {
    let output = run_with_args([
        "runtime",
        "stage136-vless-vmess-xhttp-http2-lifecycle-admission",
        "--execute-smoke",
    ...
    assert!(!json["read_only"].as_bool().unwrap());
    assert!(
        json["vless_vmess_xhttp_http2_lifecycle_smoke_passed"]
            .as_bool()
            .unwrap()
    );
    assert!(
    ...
    assert_eq!(
        json["vless_vmess_xhttp_http2_contract"]["xhttp_mode"]
            .as_str()
            .unwrap(),
        "packet-up"
    );

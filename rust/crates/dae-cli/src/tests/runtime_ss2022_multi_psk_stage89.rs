...
#[test]
fn stage89_runtime_admission_fixture_matches() {
    let fixture = load("engine/runtime_stage89/ss2022_multi_psk_tcp_dataplane_admission.json");
    ...
    assert!(
        !json["ss2022_multi_psk_identity_header_smoke_passed"]
            .as_bool()
            .unwrap()
    );
    assert!(
    ...
    assert_eq!(json["ss2022_contract"]["psk_count"].as_u64().unwrap(), 2);
    assert_eq!(json["ss2022_contract"]["upsk_index"].as_u64().unwrap(), 1);
...
#[test]
fn stage89_runtime_admission_blocks_unsafe_execution() {
    ...
    assert!(
        blocked
            .stdout
            .contains("stage89 root-gated smoke requires --ack-root-gate")
    );
}

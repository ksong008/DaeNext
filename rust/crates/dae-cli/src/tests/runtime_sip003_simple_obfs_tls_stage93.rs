...
#[test]
fn stage93_runtime_admission_fixture_matches() {
    let fixture = load("engine/runtime_stage93/sip003_simple_obfs_tls_dataplane_admission.json");
    ...
    assert!(
        !json["sip003_simple_obfs_tls_smoke_passed"]
            .as_bool()
            .unwrap()
    );
    ...
    assert_eq!(json["sip003_contract"]["obfs"].as_str().unwrap(), "tls");
    assert_eq!(
        json["sip003_contract"]["server_name"].as_str().unwrap(),
        "tls-front.example"
    );
}

#[test]
fn stage93_runtime_admission_blocks_unsafe_execution() {
    ...
    assert!(
        blocked
            .stdout
            .contains("stage93 root-gated smoke requires --ack-root-gate")
    );
}

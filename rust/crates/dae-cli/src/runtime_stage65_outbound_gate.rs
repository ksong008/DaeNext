...
const DEFAULT_TARGET: &str = "stage65-vmess.example:443";
const DEFAULT_PAYLOAD: &[u8] = b"stage65-vmess-aead-ping";

pub(crate) fn run_stage65_vmess_aead_tcp_dataplane_admission(args: &[String]) -> RunnerOutput {
    let opts = match Stage65Options::parse(args) {
        Ok(opts) => opts,
        Err(output) => return output,
    };
    let report = stage65_report(&opts);
    let passed = report["vmess_aead_tcp_smoke_passed"]
        .as_bool()
        .unwrap_or(false);
...
impl Default for Stage65Options {
    fn default() -> Self {
        Self {
            execute_smoke: false,
            ack_root_gate: false,
    ...
    fn parse(args: &[String]) -> Result<Self, RunnerOutput> {
            ...
            match arg.as_str() {
                ...
                "--benchmark-iters" => {
                    opts.benchmark_iters =
                        parse_usize(&next_value(&mut iter, "stage65 --benchmark-iters")?, arg)?;
                }
                "--uuid" => opts.uuid = next_value(&mut iter, "stage65 --uuid")?,
                "--target" => opts.target = next_value(&mut iter, "stage65 --target")?,
                "--payload" => {
                    opts.payload = next_value(&mut iter, "stage65 --payload")?.into_bytes();
                }
                "--so-mark" => {
                    opts.so_mark = parse_u32(&next_value(&mut iter, "stage65 --so-mark")?, arg)?;
                }
                "--mptcp" => opts.mptcp = true,
                "--no-mptcp" => opts.mptcp = false,
                "--timeout-ms" => {
                    let timeout_ms =
                        parse_u64(&next_value(&mut iter, "stage65 --timeout-ms")?, arg)?;
                ...
                _ => {
                    return Err(RunnerOutput::usage(format!(
                        "unsupported stage65 argument: {arg}"
                    )));
                }
            }
        }
        if opts.benchmark_iters == 0 {
            return Err(RunnerOutput::usage(
                "stage65 --benchmark-iters must be greater than zero",
            ));
        }
        vmess::vmess_cmd_key_from_uuid(&opts.uuid)
            .map_err(|err| RunnerOutput::usage(format!("stage65 uuid is invalid: {err}")))?;
        Ok(opts)
    }
}

fn stage65_report(opts: &Stage65Options) -> Value {
    let cmd_key = match vmess::vmess_cmd_key_from_uuid(&opts.uuid) {
        Ok(key) => key,
        Err(err) => {
            return json!({
                "name": "stage65-vmess-aead-tcp-dataplane-admission",
                "stage": "stage65",
                "blocked": true,
                "blockers": [format!("stage65 uuid is invalid: {err}")]
            });
        }
    };
    ...
    let mut report = json!({
        "name": "stage65-vmess-aead-tcp-dataplane-admission",
        "stage": "stage65",
        "evidence_class": "opt-in-protocol-vmess-aead-raw-tcp-true-dataplane-smoke",
    ...
    report["vmess_aead_tcp_smoke_passed"] = json!(false);
    report["vmess_aead_tcp_true_dataplane_admitted"] = json!(false);
    ...
    report["vmess_aead_tcp_contract"] = json!({
        "protocol": "vmess",
        "scope": "VMess AEAD request header plus AES-128-GCM raw TCP stream chunk over Rust TCP stream",
        "uuid": opts.uuid,
        ...
        "udp_mux_deferred": "VMess UDP packet-addr and mux rows are deferred",
    ...
    report["benchmark"] = json!({
        ...
        "elapsed_ns": null,
        "ns_per_vmess_aead_tcp_exchange": null,
        "scope": "VMess AEAD raw TCP header/chunk echo over SO_MARKed Rust TCP socket",
    ...
    report["protocol_matrix"] = json!({
        ...
        "vmess_aead_tcp_true_dataplane_admitted": false,
    ...
    report["remaining_blockers"] = json!([
        ...
        "VMess TLS/shared transport/UDP packet addr/mux rows are still incomplete",
    ...
    report["validation_commands"] = json!([
        "python3 -m json.tool testdata/rebuild-golden/engine/runtime_stage65/vmess_aead_tcp_dataplane_admission.json",
        "python3 -m json.tool testdata/rebuild-golden/product/daemon/stage65_vmess_aead_tcp_dataplane_gate.json",
        "cargo test --manifest-path rust/Cargo.toml -p dae-outbound stage65 -- --nocapture",
        "cargo test --manifest-path rust/Cargo.toml -p dae-cli stage65 -- --nocapture",
        "cargo test --manifest-path rust/Cargo.toml -p dae-product stage65 -- --nocapture",
        "cargo run --manifest-path rust/Cargo.toml -p dae-cli --bin dae-cli-optin --quiet -- runtime stage65-vmess-aead-tcp-dataplane-admission --execute-smoke --ack-root-gate --benchmark-iters 10",
        "git diff --check"
    ]);
    report["source"] = json!([
        "DAEX_RUST_REBUILD_PLAN_2026-05-16.md:stage65",
    ...
    if !opts.ack_root_gate {
        ...
        report["blockers"] = json!([
            "stage65 root-gated smoke requires --ack-root-gate because it attempts SO_MARK/MPTCP underlay socket observation"
        ]);
        return report;
    }

    match run_stage65_smoke(opts) {
        Ok(outcome) => apply_stage65_outcome(&mut report, outcome),
        Err(err) => {
            report["blocked"] = json!(true);
...
#[derive(Debug)]
struct Stage65Outcome {
    listener_report: TcpLoopbackListenerReport,
    last_dial_report: TcpDirectDialReport,
    server_summary: VMessAeadTcpServerSummary,
    elapsed_ns: u128,
    ns_per_vmess_aead_tcp_exchange: f64,
...
fn run_stage65_smoke(opts: &Stage65Options) -> Result<Stage65Outcome, String> {
    let (server_addr, listener_report, handle) = spawn_vmess_aead_tcp_server(opts)?;
    ...
    for _ in 0..opts.benchmark_iters {
        ...
        .map_err(|err| format!("stage65 magic_tcp_connect failed: {err}"))?;
        ...
        .map_err(|err| format!("stage65 VMess AEAD TCP exchange failed: {err}"))?;
        if report.echoed_payload != opts.payload {
            return Err("stage65 VMess AEAD TCP payload response mismatch".to_owned());
        }
    ...
    let server_summary = handle
        .join()
        .map_err(|_| "stage65 VMess server thread panicked".to_owned())??;
    if server_summary.accepted != opts.benchmark_iters {
        return Err(format!(
            "stage65 VMess server accepted {} connections, want {}",
            server_summary.accepted, opts.benchmark_iters
        ));
    }
    let last_exchange =
        last_exchange.ok_or_else(|| "stage65 missing exchange report".to_owned())?;
    Ok(Stage65Outcome {
        listener_report,
        last_dial_report: last_dial_report
            .ok_or_else(|| "stage65 missing underlay dial report".to_owned())?,
        server_summary,
        elapsed_ns,
        ns_per_vmess_aead_tcp_exchange: elapsed_ns as f64 / opts.benchmark_iters as f64,
...
fn apply_stage65_outcome(report: &mut Value, outcome: Stage65Outcome) {
    ...
    let server_complete = outcome.server_summary.accepted == outcome.exchange_count
        ...
        && outcome.server_summary.security_count == outcome.exchange_count
    ...
    report["vmess_aead_tcp_smoke_passed"] = json!(passed);
    report["vmess_aead_tcp_true_dataplane_admitted"] = json!(passed);
    report["vmess_protocol_partial_admitted"] = json!(passed);
    report["vmess_aead_tcp_contract"]["server"] = json!(outcome.last_dial_report.peer_addr);
    report["vmess_aead_tcp_contract"]["cmd_key_hex"] = json!(outcome.cmd_key_hex);
    report["vmess_aead_tcp_contract"]["eauth_crc_validated"] = json!(passed);
    report["vmess_aead_tcp_contract"]["request_header_aead_validated"] = json!(passed);
    report["vmess_aead_tcp_contract"]["response_header_aead_validated"] = json!(passed);
    report["vmess_aead_tcp_contract"]["shake128_chunk_masking_validated"] = json!(passed);
    report["vmess_aead_tcp_contract"]["target_metadata_validated"] = json!(passed);
    report["vmess_aead_tcp_contract"]["payload_roundtrip_validated"] = json!(passed);
    ...
    report["server_observation"] = json!({
        ...
        "security_count": outcome.server_summary.security_count,
    ...
    report["benchmark"]["ns_per_vmess_aead_tcp_exchange"] =
        json!(outcome.ns_per_vmess_aead_tcp_exchange);
    ...
    report["protocol_matrix"]["vmess_aead_tcp_true_dataplane_admitted"] = json!(passed);
}

#[derive(Debug, Default)]
struct VMessAeadTcpServerSummary {
    accepted: usize,
...
fn spawn_vmess_aead_tcp_server(
    opts: &Stage65Options,
) -> Result<
    (
        SocketAddrV4,
        TcpLoopbackListenerReport,
        thread::JoinHandle<Result<VMessAeadTcpServerSummary, String>>,
    ),
    String,
> {
    let (listener, listener_report) = bind_loopback_tcp_listener(opts.mptcp)
        .map_err(|err| format!("stage65 bind loopback VMess server failed: {err}"))?;
    let local_addr = listener
        .local_addr()
        .map_err(|err| format!("stage65 VMess server local_addr failed: {err}"))?;
    let server_addr = match local_addr {
        SocketAddr::V4(addr) => addr,
        SocketAddr::V6(addr) => return Err(format!("stage65 VMess listener is not IPv4: {addr}")),
    };
    listener
        .set_nonblocking(true)
        .map_err(|err| format!("stage65 VMess nonblocking failed: {err}"))?;
    ...
    let handle = thread::spawn(move || {
        accept_vmess_aead_tcp(listener, iterations, &uuid, &target, &payload, timeout)
    });
    Ok((server_addr, listener_report, handle))
}

fn accept_vmess_aead_tcp(
    ...
    expected_payload: &[u8],
    timeout: Duration,
) -> Result<VMessAeadTcpServerSummary, String> {
    let mut summary = VMessAeadTcpServerSummary::default();
        ...
        match listener.accept() {
            Ok((mut stream, _)) => {
                stream
                    ...
                    .map_err(|err| format!("stage65 server set read timeout failed: {err}"))?;
                stream
                    .set_write_timeout(Some(timeout))
                    .map_err(|err| format!("stage65 server set write timeout failed: {err}"))?;
                handle_vmess_aead_tcp(
                    &mut stream,
                    uuid,
                    expected_target,
                    expected_payload,
            ...
            Err(err) if err.kind() == ErrorKind::WouldBlock => {
                return Err(format!(
                    "stage65 VMess server timed out after accepting {} of {} connections",
                    summary.accepted, iterations
                ));
            }
            Err(err) => return Err(format!("stage65 VMess accept failed: {err}")),
        }
    }
    Ok(summary)
}

fn handle_vmess_aead_tcp(
    stream: &mut TcpStream,
    uuid: &str,
    expected_target: &str,
    expected_payload: &[u8],
    summary: &mut VMessAeadTcpServerSummary,
) -> Result<(), String> {
    let request = vmess::read_aead_tcp_request_from_stream(stream, uuid)
        .map_err(|err| format!("stage65 read VMess request failed: {err}"))?;
    if !request.eauth_crc_validated {
        return Err("stage65 VMess EAuthID checksum was not validated".to_owned());
    }
    if request.version != 1 {
        return Err(format!(
            "stage65 VMess version mismatch: got {}, want 1",
            request.version
        ));
    }
    if request.security != vmess::VMESS_AEAD_SECURITY_AES_128_GCM {
        return Err(format!(
            "stage65 VMess security mismatch: got {}, want {}",
            request.security,
    ...
    if request.command != dae_outbound::VMessNetwork::Tcp.byte() {
        return Err(format!(
            "stage65 VMess command mismatch: got {}, want {}",
            request.command,
    ...
    if request.target != expected_target {
        return Err(format!(
            "stage65 VMess target mismatch: got {}, want {}",
            request.target, expected_target
        ));
    }
    if request.payload != expected_payload {
        return Err("stage65 VMess request payload mismatch".to_owned());
    }
    let response = vmess::aead_tcp_response_packet(&request, &request.payload)
        .map_err(|err| format!("stage65 encode VMess response failed: {err}"))?;
    stream
        .write_all(&response)
        .map_err(|err| format!("stage65 write VMess echo failed: {err}"))?;

...
const DEFAULT_STAGE37_ROOT: &str = "/tmp/dae-stage37-candidate";
const DEFAULT_STAGE37_IFACE: &str = "dae37p0";
const DEFAULT_STAGE37_OBJECT: &str = "control/bpf_bpfel.o";
const DEFAULT_STAGE37_SECTION: &str = "tc/dae0peer_ingress";
const STAGE37_FILTER_PREF: &str = "49370";
const LISTEN_SOCKET_MAP_KERNEL_NAME: &str = "listen_socket_m";

pub(crate) fn run_stage37_loaded_listen_socket_map_admission(args: &[String]) -> RunnerOutput {
    let opts = match Stage37Options::parse(args) {
        Ok(opts) => opts,
        Err(output) => return output,
    };
    let report = stage37_report(&opts);
...
#[derive(Debug, Clone)]
struct Stage37Options {
    root: PathBuf,
    stage36_report: Option<PathBuf>,
    execute_smoke: bool,
    ack_root_gate: bool,
    iface: String,
    object_path: PathBuf,
...
impl Default for Stage37Options {
    fn default() -> Self {
        Self {
            root: PathBuf::from(DEFAULT_STAGE37_ROOT),
            stage36_report: None,
            execute_smoke: false,
            ack_root_gate: false,
            iface: DEFAULT_STAGE37_IFACE.to_owned(),
            object_path: PathBuf::from(DEFAULT_STAGE37_OBJECT),
            section: DEFAULT_STAGE37_SECTION.to_owned(),
        }
    }
}
...
impl Stage37Options {
    fn parse(args: &[String]) -> Result<Self, RunnerOutput> {
        ...
        while let Some(arg) = iter.next() {
            match arg.as_str() {
                "--root" => opts.root = PathBuf::from(next_value(&mut iter, "stage37 --root")?),
                "--stage36-report" => {
                    opts.stage36_report = Some(PathBuf::from(next_value(
                        &mut iter,
                        "stage37 --stage36-report",
                    )?));
                }
                "--execute-smoke" => opts.execute_smoke = true,
                "--ack-root-gate" => opts.ack_root_gate = true,
                "--iface" => opts.iface = next_value(&mut iter, "stage37 --iface")?,
                "--object" => {
                    opts.object_path = PathBuf::from(next_value(&mut iter, "stage37 --object")?);
                }
                "--section" => opts.section = next_value(&mut iter, "stage37 --section")?,
                _ if arg.starts_with("--root=") => {
                    opts.root = PathBuf::from(value_after_equals(arg, "stage37 --root")?);
                }
                _ if arg.starts_with("--stage36-report=") => {
                    opts.stage36_report = Some(PathBuf::from(value_after_equals(
                        arg,
                        "stage37 --stage36-report",
                    )?));
                }
                _ if arg.starts_with("--iface=") => {
                    opts.iface = value_after_equals(arg, "stage37 --iface")?;
                }
                _ if arg.starts_with("--object=") => {
                    opts.object_path = PathBuf::from(value_after_equals(arg, "stage37 --object")?);
                }
                _ if arg.starts_with("--section=") => {
                    opts.section = value_after_equals(arg, "stage37 --section")?;
                }
                _ => {
                    return Err(RunnerOutput::usage(format!(
                        "unsupported runtime stage37-loaded-listen-socket-map-admission argument: {arg}"
                    )));
                }
            }
        }
...
fn stage37_report(opts: &Stage37Options) -> Value {
    ...
    push_check(
        ...
        &mut blockers,
        "stage37 root must be an absolute /tmp child path",
    );
    push_check(
        &mut checks,
        ...
        &mut blockers,
        "stage37 root-gated smoke requires --ack-root-gate",
    );
    push_check(
        &mut checks,
        ...
        &mut blockers,
        "stage37 temporary interface name is invalid",
    );
    push_check(
        &mut checks,
        ...
        &mut blockers,
        "stage37 cannot target production dae0/dae0peer names",
    );
    for tool in ["ip", "tc"] {
        push_check(
            &mut checks,
    ...
    push_check(
        ...
        &mut blockers,
        "stage37 real dae eBPF object is missing",
    );
    let stage36 = read_report(
        opts.stage36_report.as_deref(),
        "listen_socket_map_fd_update_smoke_passed",
    );
    push_check(
        &mut checks,
        "stage36-temporary-sockmap-report-passed",
        !opts.execute_smoke || stage36.passed,
        json!({
            "path": stage36.path.clone(),
            "status": stage36.status,
            "listen_socket_map_fd_update_smoke_passed": stage36.passed,
            "blockers": stage36.blockers.clone(),
        }),
        &mut blockers,
        "stage37 root-gated smoke requires a passed Stage 36 temporary SockMap report",
    );
    if opts.execute_smoke {
        push_check(
            ...
            json!({"iface": opts.iface}),
            &mut blockers,
            "stage37 temporary interface name already exists",
        );
    }
    ...
    let before_map_ids = if opts.execute_smoke && blockers.is_empty() {
        match map_ids() {
            Ok(ids) => ids,
            Err(err) => {
                blockers.push(format!("stage37 cannot snapshot BPF map ids: {err}"));
                Vec::new()
            }
        }
    } else {
    ...
    let mut loaded_listen_socket_map_handoff_smoke_passed = false;
    let mut discovered_map_id = None;
    if opts.execute_smoke && blockers.is_empty() {
        let result = execute_stage37_smoke(opts, &before_map_ids);
        executed_steps = result.executed_steps;
        cleanup_steps = result.cleanup_steps;
        attach_show = result.attach_show;
        loaded_map_handoff = result.loaded_map_handoff;
        loaded_listen_socket_map_handoff_smoke_passed = result.passed;
        discovered_map_id = result.discovered_map_id;
        if !loaded_listen_socket_map_handoff_smoke_passed {
            blockers.push(
                "stage37 real loaded object listen_socket_map handoff smoke failed".to_owned(),
            );
        }
    }
    ...
    let after_map_ids = if opts.execute_smoke {
        map_ids().unwrap_or_default()
    } else {
        Vec::new()
    };
    let temporary_map_cleaned = discovered_map_id.is_none_or(|id| !after_map_ids.contains(&id));
    if opts.execute_smoke && !temporary_map_cleaned {
        blockers
            .push("stage37 temporary loaded listen_socket_map remains after cleanup".to_owned());
    }
    let leftovers = iface_leftovers(&opts.iface);
    if opts.execute_smoke && !leftovers.is_empty() {
        blockers.push("stage37 temporary resources remain after cleanup".to_owned());
    }
    let sys_fs_bpf_dae_mutated = before_pin_snapshot != after_pin_snapshot;
    if opts.execute_smoke && sys_fs_bpf_dae_mutated {
        blockers.push("stage37 unexpectedly mutated /sys/fs/bpf/dae".to_owned());
    }

    json!({
        "name": "stage37-loaded-listen-socket-map-admission",
        "stage": "stage37",
        "evidence_class": "root-gated-real-loaded-object-listen-socket-map-handoff-smoke",
        "root": path_string(&opts.root),
        "execute_smoke": opts.execute_smoke,
        "root_gate_acknowledged": opts.ack_root_gate,
        "read_only": !opts.execute_smoke,
        "blocked": !blockers.is_empty(),
        "loaded_listen_socket_map_handoff_smoke_passed": loaded_listen_socket_map_handoff_smoke_passed,
        "real_loaded_object_listen_socket_map_fd_update_executed": opts.execute_smoke && loaded_listen_socket_map_handoff_smoke_passed,
        "real_loaded_object_listen_socket_map_cleanup_passed": opts.execute_smoke && loaded_listen_socket_map_handoff_smoke_passed && temporary_map_cleaned,
        ...
        "stage36_report": {
            "path": stage36.path,
            "status": stage36.status,
            "passed": stage36.passed,
            "blockers": stage36.blockers,
        },
        "real_loaded_object_contract": {
            "object_path": path_string(&opts.object_path),
            "section": opts.section,
            "filter_pref": STAGE37_FILTER_PREF,
        ...
        "map_id_snapshots": {
            ...
            "temporary_loaded_map_cleaned": temporary_map_cleaned,
        },
...
fn execute_stage37_smoke(opts: &Stage37Options, before_map_ids: &[u32]) -> Stage37SmokeResult {
    let mut executed_steps = Vec::new();
    let mut cleanup_steps = Vec::new();
    let mut ok = true;

...
const DEFAULT_MUX_TARGET: &str = "stage68-vmess-mux.example:443";
const DEFAULT_PAYLOAD: &[u8] = b"stage68-vmess-mux-ping";
const DEFAULT_MUX_ID: [u8; 2] = [0x68, 0x01];

pub(crate) fn run_stage68_vmess_mux_dataplane_admission(args: &[String]) -> RunnerOutput {
    let opts = match Stage68Options::parse(args) {
        Ok(opts) => opts,
        Err(output) => return output,
    };
    let report = stage68_report(&opts);
    let passed = report["vmess_mux_smoke_passed"].as_bool().unwrap_or(false);
...
impl Default for Stage68Options {
    fn default() -> Self {
        Self {
            execute_smoke: false,
            ack_root_gate: false,
            ...
            mux_target: DEFAULT_MUX_TARGET.to_owned(),
    ...
    fn parse(args: &[String]) -> Result<Self, RunnerOutput> {
            ...
            match arg.as_str() {
                ...
                "--benchmark-iters" => {
                    opts.benchmark_iters =
                        parse_usize(&next_value(&mut iter, "stage68 --benchmark-iters")?, arg)?;
                }
                "--uuid" => opts.uuid = next_value(&mut iter, "stage68 --uuid")?,
                "--mux-target" => opts.mux_target = next_value(&mut iter, "stage68 --mux-target")?,
                "--payload" => {
                    opts.payload = next_value(&mut iter, "stage68 --payload")?.into_bytes();
                }
                "--so-mark" => {
                    opts.so_mark = parse_u32(&next_value(&mut iter, "stage68 --so-mark")?, arg)?;
                }
                "--mptcp" => opts.mptcp = true,
                "--no-mptcp" => opts.mptcp = false,
                "--timeout-ms" => {
                    let timeout_ms =
                        parse_u64(&next_value(&mut iter, "stage68 --timeout-ms")?, arg)?;
                ...
                _ if arg.starts_with("--mux-target=") => {
                    opts.mux_target = arg.split_once('=').unwrap().1.to_owned();
                }
                ...
                _ => {
                    return Err(RunnerOutput::usage(format!(
                        "unsupported stage68 argument: {arg}"
                    )));
                }
            }
        }
        if opts.benchmark_iters == 0 {
            return Err(RunnerOutput::usage(
                "stage68 --benchmark-iters must be greater than zero",
            ));
        }
        vmess::vmess_cmd_key_from_uuid(&opts.uuid)
            .map_err(|err| RunnerOutput::usage(format!("stage68 uuid is invalid: {err}")))?;
        dae_outbound::VMessMetadata::parse("tcp", &opts.mux_target)
            .map_err(|err| RunnerOutput::usage(format!("stage68 mux target is invalid: {err}")))?;
        Ok(opts)
    }
}

fn stage68_report(opts: &Stage68Options) -> Value {
    let cmd_key = match vmess::vmess_cmd_key_from_uuid(&opts.uuid) {
        Ok(key) => key,
        Err(err) => {
            return json!({
                "name": "stage68-vmess-mux-dataplane-admission",
                "stage": "stage68",
                "blocked": true,
                "blockers": [format!("stage68 uuid is invalid: {err}")]
            });
        }
    };
    if let Err(err) = dae_outbound::VMessMetadata::parse("tcp", &opts.mux_target) {
        return json!({
            "name": "stage68-vmess-mux-dataplane-admission",
            "stage": "stage68",
            "blocked": true,
            "blockers": [format!("stage68 mux target is invalid: {err}")]
        });
    }
    ...
    let mut report = json!({
        "name": "stage68-vmess-mux-dataplane-admission",
        "stage": "stage68",
        "evidence_class": "opt-in-protocol-vmess-aead-mux-true-dataplane-smoke",
    ...
    report["vmess_mux_smoke_passed"] = json!(false);
    ...
    report["vmess_mux_contract"] = json!({
        "protocol": "vmess",
        "scope": "VMess AEAD mux command plus mux new/data/end frames over Rust TCP stream",
        "uuid": opts.uuid,
        ...
        "request_target": "0.0.0.0:0",
        "mux_id_hex": hex_encode(&DEFAULT_MUX_ID),
        "mux_target": opts.mux_target,
        ...
        "vmess_default_mux_path_deferred": "Go VMess default dialer does not expose mux as a default path in this branch",
    ...
    report["benchmark"] = json!({
        ...
        "elapsed_ns": null,
        "ns_per_vmess_mux_exchange": null,
        "scope": "VMess AEAD mux command plus mux new/data/end frame echo over SO_MARKed Rust TCP socket",
    ...
    report["remaining_blockers"] = json!([
        ...
        "VMess TLS/shared transport rows are still incomplete",
    ...
    if !opts.ack_root_gate {
        ...
        report["blockers"] = json!([
            "stage68 root-gated smoke requires --ack-root-gate because it attempts SO_MARK/MPTCP underlay socket observation"
        ]);
        return report;
    }

    match run_stage68_smoke(opts) {
        Ok(outcome) => apply_stage68_outcome(&mut report, outcome),
        Err(err) => {
            report["blocked"] = json!(true);
...
fn run_stage68_smoke(opts: &Stage68Options) -> Result<Stage68Outcome, String> {
    let (server_addr, listener_report, handle) = spawn_vmess_mux_server(opts)?;
    ...
    for _ in 0..opts.benchmark_iters {
        ...
        .map_err(|err| format!("stage68 magic_tcp_connect failed: {err}"))?;
        ...
        .map_err(|err| format!("stage68 VMess mux exchange failed: {err}"))?;
        if report.echoed_payload != opts.payload {
            return Err("stage68 VMess mux payload response mismatch".to_owned());
        }
    ...
    let server_summary = handle
        .join()
        .map_err(|_| "stage68 VMess mux server thread panicked".to_owned())??;
    if server_summary.accepted != opts.benchmark_iters {
        return Err(format!(
            "stage68 VMess server accepted {} connections, want {}",
            server_summary.accepted, opts.benchmark_iters
        ));
    }
    let last_exchange =
        last_exchange.ok_or_else(|| "stage68 missing exchange report".to_owned())?;
    Ok(Stage68Outcome {
        listener_report,
        last_dial_report: last_dial_report
            .ok_or_else(|| "stage68 missing underlay dial report".to_owned())?,
        server_summary,
        elapsed_ns,
...
fn apply_stage68_outcome(report: &mut Value, outcome: Stage68Outcome) {
    ...
    report["vmess_mux_smoke_passed"] = json!(passed);
    report["vmess_mux_admitted"] = json!(passed);
    report["vmess_protocol_partial_admitted"] = json!(passed);
    report["vmess_mux_contract"]["server"] = json!(outcome.last_dial_report.peer_addr);
    report["vmess_mux_contract"]["cmd_key_hex"] = json!(outcome.cmd_key_hex);
    report["vmess_mux_contract"]["eauth_crc_validated"] = json!(passed);
    report["vmess_mux_contract"]["request_header_aead_validated"] = json!(passed);
    report["vmess_mux_contract"]["response_header_aead_validated"] = json!(passed);
    report["vmess_mux_contract"]["shake128_chunk_masking_validated"] = json!(passed);
    report["vmess_mux_contract"]["mux_command_validated"] = json!(passed);
    report["vmess_mux_contract"]["mux_new_frame_validated"] = json!(passed);
    report["vmess_mux_contract"]["mux_data_frame_validated"] = json!(passed);
    report["vmess_mux_contract"]["mux_end_frame_validated"] = json!(passed);
    report["vmess_mux_contract"]["target_metadata_validated"] = json!(passed);
    report["vmess_mux_contract"]["payload_roundtrip_validated"] = json!(passed);
    ...
    report["benchmark"]["ns_per_vmess_mux_exchange"] = json!(outcome.ns_per_exchange);
    ...
    report["protocol_matrix"]["vmess_mux_admitted"] = json!(passed);
}

#[derive(Debug, Default)]
struct VMessMuxServerSummary {
    accepted: usize,
...
fn spawn_vmess_mux_server(
    opts: &Stage68Options,
) -> Result<
    (
        SocketAddrV4,
        TcpLoopbackListenerReport,
        thread::JoinHandle<Result<VMessMuxServerSummary, String>>,
    ),
    String,
> {
    let (listener, listener_report) = bind_loopback_tcp_listener(opts.mptcp)
        .map_err(|err| format!("stage68 bind loopback VMess server failed: {err}"))?;
    let local_addr = listener
        .local_addr()
        .map_err(|err| format!("stage68 VMess server local_addr failed: {err}"))?;
    let server_addr = match local_addr {
        SocketAddr::V4(addr) => addr,
        SocketAddr::V6(addr) => return Err(format!("stage68 VMess listener is not IPv4: {addr}")),
    };
    listener
        .set_nonblocking(true)
        .map_err(|err| format!("stage68 VMess nonblocking failed: {err}"))?;

    let iterations = opts.benchmark_iters;
    let uuid = opts.uuid.clone();
    let mux_target = opts.mux_target.clone();
    let payload = opts.payload.clone();
    let timeout = opts.timeout;
    let handle = thread::spawn(move || {
        accept_vmess_mux(listener, iterations, &uuid, &mux_target, &payload, timeout)
    });
...
fn accept_vmess_mux(
    ...
    expected_payload: &[u8],
    timeout: Duration,
) -> Result<VMessMuxServerSummary, String> {
    let mut summary = VMessMuxServerSummary::default();
        ...
        match listener.accept() {
            Ok((mut stream, _)) => {
                stream
                    ...
                    .map_err(|err| format!("stage68 server set read timeout failed: {err}"))?;
                stream
                    .set_write_timeout(Some(timeout))
                    .map_err(|err| format!("stage68 server set write timeout failed: {err}"))?;
                handle_vmess_mux(
                    &mut stream,
                    uuid,
                    expected_target,
            ...
            Err(err) if err.kind() == ErrorKind::WouldBlock => {
                return Err(format!(
                    "stage68 VMess server timed out after accepting {} of {} connections",
                    summary.accepted, iterations
                ));
            }
            Err(err) => return Err(format!("stage68 VMess accept failed: {err}")),
        }
    }
    Ok(summary)
}
...
fn handle_vmess_mux(
    ...
    expected_target: &str,
    expected_payload: &[u8],
    summary: &mut VMessMuxServerSummary,
) -> Result<(), String> {
    let request = vmess::read_aead_mux_request_from_stream(stream, uuid)
        .map_err(|err| format!("stage68 read VMess mux request failed: {err}"))?;
    if !request.request.eauth_crc_validated {
        return Err("stage68 VMess EAuthID checksum was not validated".to_owned());
    }
    if request.request.command != dae_outbound::VMessNetwork::Mux.byte() {
        return Err(format!(
            "stage68 VMess command mismatch: got {}, want {}",
            request.request.command,
            dae_outbound::VMessNetwork::Mux.byte()
        ));
    }
    if request.request.target != "0.0.0.0:0" {
        return Err(format!(
            "stage68 VMess mux request target mismatch: got {}, want 0.0.0.0:0",
            request.request.target
        ));
    }
    let (host, port) = expected_target
        .rsplit_once(':')
        .ok_or_else(|| format!("stage68 bad mux target: {expected_target}"))?;
    let options = shared_transport::MuxFrameOptions::new(
        DEFAULT_MUX_ID,
        host,
        port.parse::<u16>()
            .map_err(|err| format!("stage68 bad mux target port: {err}"))?,
        "tcp",
    );
    let expected_new = shared_transport::mux_new_frame(&options);
    if request.new_frame.metadata != expected_new[2..] {
        return Err("stage68 VMess mux new frame metadata mismatch".to_owned());
    }
    if request.data_frame.id != DEFAULT_MUX_ID
        || request.data_frame.status != shared_transport::mux::SESSION_STATUS_KEEP
        || request.data_frame.option != shared_transport::mux::OPTION_DATA
    {
        return Err(format!(
            "stage68 VMess mux data frame mismatch: id={:?} status={} option={}",
            request.data_frame.id, request.data_frame.status, request.data_frame.option
        ));
    }
    if request.data_frame.payload != expected_payload {
        return Err("stage68 VMess mux payload mismatch".to_owned());
    }
    if request.end_frame.id != DEFAULT_MUX_ID
        || request.end_frame.status != shared_transport::mux::SESSION_STATUS_END
    {
        return Err(format!(
            "stage68 VMess mux end frame mismatch: id={:?} status={}",
            request.end_frame.id, request.end_frame.status
        ));
    }
    let response = shared_transport::mux_data_frame(DEFAULT_MUX_ID, &request.data_frame.payload)
        .map_err(|err| format!("stage68 encode mux response failed: {err}"))?;
    let response = vmess::aead_tcp_response_packet(&request.request, &response)
        .map_err(|err| format!("stage68 encode VMess response failed: {err}"))?;
    stream
        .write_all(&response)
        .map_err(|err| format!("stage68 write VMess mux echo failed: {err}"))?;
    ...
    summary
        .payload_ascii
        .push(String::from_utf8_lossy(&request.data_frame.payload).to_string());
    summary
        .response_ascii
        .push(String::from_utf8_lossy(&request.data_frame.payload).to_string());
    Ok(())

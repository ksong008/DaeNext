...
pub(super) fn run_stage80_smoke(opts: &Stage80Options) -> Result<Stage80Outcome, String> {
    let key = vless::password_to_key(&opts.uuid)
        .map_err(|err| format!("stage80 uuid is invalid: {err}"))?;
    let (server_addr, listener_report, handle) = spawn_vless_xhttp_xmux_server(opts)?;
    ...
    for index in 0..opts.benchmark_iters {
        ...
        .map_err(|err| format!("stage80 magic_tcp_connect failed: {err}"))?;
        let xhttp_options = opts
            .xhttp_options_for_seq(opts.xhttp_seq + index as u64)
        ...
        .map_err(|err| format!("stage80 VLESS xHTTP xmux exchange failed: {err}"))?;
        if report.echoed_payload != opts.payload {
            return Err("stage80 VLESS xHTTP xmux payload response mismatch".to_owned());
        }
    ...
    let server_summary = handle
        .join()
        .map_err(|_| "stage80 VLESS xHTTP xmux server thread panicked".to_owned())??;
    if server_summary.accepted != opts.benchmark_iters {
        return Err(format!(
            "stage80 VLESS xHTTP xmux server accepted {} connections, want {}",
            server_summary.accepted, opts.benchmark_iters
        ));
    }
    let last_exchange =
        last_exchange.ok_or_else(|| "stage80 missing exchange report".to_owned())?;
    Ok(Stage80Outcome {
        listener_report,
        last_dial_report: last_dial_report
            .ok_or_else(|| "stage80 missing underlay dial report".to_owned())?,
        server_summary,
        elapsed_ns,
...
pub(super) fn apply_stage80_outcome(report: &mut Value, outcome: Stage80Outcome) {
    ...
    let server_complete = outcome.server_summary.accepted == outcome.exchange_count
        ...
        && outcome.server_summary.xhttp_xmux_count == outcome.exchange_count
        && outcome.server_summary.xmux_session_reuse_count
            == outcome.exchange_count.saturating_sub(1);
    let passed = server_complete && so_mark_observed && mptcp_status_recorded;

    report["read_only"] = json!(false);
    report["vless_xhttp_xmux_smoke_passed"] = json!(passed);
    report["vless_xhttp_admitted"] = json!(passed);
    report["vless_xhttp_xmux_admitted"] = json!(passed);
    ...
    report["vless_xhttp_contract"]["xhttp_xmux_enabled"] = json!(passed);
    report["vless_xhttp_contract"]["xmux_session_reuse_validated"] = json!(passed);
    ...
    report["server_observation"] = json!({
        ...
        "xhttp_xmux_count": outcome.server_summary.xhttp_xmux_count,
        "xmux_session_reuse_count": outcome.server_summary.xmux_session_reuse_count,
        "xmux_max_connections": outcome.server_summary.xmux_max_connections,
        "xmux_c_max_reuse_times": outcome.server_summary.xmux_c_max_reuse_times,
        "targets": outcome.server_summary.targets,
        "xhttp_request_paths": outcome.server_summary.xhttp_request_paths,
        "xhttp_seqs": outcome.server_summary.xhttp_seqs,
    ...
    report["benchmark"]["ns_per_vless_xhttp_xmux_exchange"] = json!(outcome.ns_per_exchange);
    ...
    report["protocol_matrix"]["vless_xhttp_xmux_admitted"] = json!(passed);
}

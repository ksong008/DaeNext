...
const DEFAULT_PACKET_TARGET: &str = "1.2.3.4:53";
const DEFAULT_PAYLOAD: &[u8] = b"stage67-vmess-packet-addr-ping";

pub(crate) fn run_stage67_vmess_packet_addr_udp_dataplane_admission(
    args: &[String],
) -> RunnerOutput {
    let opts = match Stage67Options::parse(args) {
        Ok(opts) => opts,
        Err(output) => return output,
    };
    let report = stage67_report(&opts);
    let passed = report["vmess_packet_addr_udp_smoke_passed"]
        .as_bool()
        .unwrap_or(false);
...
impl Default for Stage67Options {
    fn default() -> Self {
        Self {
            execute_smoke: false,
            ack_root_gate: false,
    ...
    fn parse(args: &[String]) -> Result<Self, RunnerOutput> {
            ...
            match arg.as_str() {
                ...
                "--benchmark-iters" => {
                    opts.benchmark_iters =
                        parse_usize(&next_value(&mut iter, "stage67 --benchmark-iters")?, arg)?;
                }
                "--uuid" => opts.uuid = next_value(&mut iter, "stage67 --uuid")?,
                "--packet-target" => {
                    opts.packet_target = next_value(&mut iter, "stage67 --packet-target")?;
                }
                "--payload" => {
                    opts.payload = next_value(&mut iter, "stage67 --payload")?.into_bytes();
                }
                "--so-mark" => {
                    opts.so_mark = parse_u32(&next_value(&mut iter, "stage67 --so-mark")?, arg)?;
                }
                "--mptcp" => opts.mptcp = true,
                "--no-mptcp" => opts.mptcp = false,
                "--timeout-ms" => {
                    let timeout_ms =
                        parse_u64(&next_value(&mut iter, "stage67 --timeout-ms")?, arg)?;
                ...
                _ => {
                    return Err(RunnerOutput::usage(format!(
                        "unsupported stage67 argument: {arg}"
                    )));
                }
            }
        }
        if opts.benchmark_iters == 0 {
            return Err(RunnerOutput::usage(
                "stage67 --benchmark-iters must be greater than zero",
            ));
        }
        vmess::vmess_cmd_key_from_uuid(&opts.uuid)
            .map_err(|err| RunnerOutput::usage(format!("stage67 uuid is invalid: {err}")))?;
        vmess::put_packet_addr_payload(&opts.packet_target, &opts.payload).map_err(|err| {
            RunnerOutput::usage(format!("stage67 packet target is invalid: {err}"))
        })?;
        Ok(opts)
    }
}

fn stage67_report(opts: &Stage67Options) -> Value {
    let cmd_key = match vmess::vmess_cmd_key_from_uuid(&opts.uuid) {
        Ok(key) => key,
        Err(err) => {
            return json!({
                "name": "stage67-vmess-packet-addr-udp-dataplane-admission",
                "stage": "stage67",
                "blocked": true,
                "blockers": [format!("stage67 uuid is invalid: {err}")]
            });
        }
    };
    let request_target = match vmess::packet_addr_magic_target(&opts.packet_target) {
        Ok(target) => target,
        Err(err) => {
            return json!({
                "name": "stage67-vmess-packet-addr-udp-dataplane-admission",
                "stage": "stage67",
                "blocked": true,
                "blockers": [format!("stage67 packet target is invalid: {err}")]
            });
        }
    };
    let packet = match vmess::put_packet_addr_payload(&opts.packet_target, &opts.payload) {
        Ok(packet) => packet,
        Err(err) => {
            return json!({
                "name": "stage67-vmess-packet-addr-udp-dataplane-admission",
                "stage": "stage67",
                "blocked": true,
                "blockers": [format!("stage67 packet payload is invalid: {err}")]
            });
        }
    };
    let (_, packet_addr_len, _) = vmess::parse_packet_addr_payload(&packet)
        .expect("stage67 packet addr payload is built locally");
    let payload_ascii = String::from_utf8_lossy(&opts.payload).to_string();
    let mut report = json!({
        "name": "stage67-vmess-packet-addr-udp-dataplane-admission",
        "stage": "stage67",
        "evidence_class": "opt-in-protocol-vmess-packet-addr-udp-true-dataplane-smoke",
    ...
    report["vmess_packet_addr_udp_smoke_passed"] = json!(false);
    ...
    report["vmess_packet_addr_udp_contract"] = json!({
        "protocol": "vmess",
        "scope": "VMess AEAD packet-addr UDP packet stream over Rust TCP stream",
        "uuid": opts.uuid,
        ...
        "magic_domain": vmess::VMESS_PACKET_ADDR_MAGIC_ADDRESS,
        "request_target": request_target,
        "packet_target": opts.packet_target,
        "payload_ascii": payload_ascii,
        "packet_addr_len": packet_addr_len,
        ...
        "magic_domain_validated": false,
        "packet_addr_prefix_validated": false,
        "packet_target_validated": false,
        "payload_roundtrip_validated": false,
        "mux_deferred": "VMess mux row is deferred",
    ...
    report["benchmark"] = json!({
        ...
        "elapsed_ns": null,
        "ns_per_vmess_packet_addr_udp_exchange": null,
        "scope": "VMess AEAD packet-addr UDP packet echo over SO_MARKed Rust TCP socket",
    ...
    if !opts.ack_root_gate {
        ...
        report["blockers"] = json!([
            "stage67 root-gated smoke requires --ack-root-gate because it attempts SO_MARK/MPTCP underlay socket observation"
        ]);
        return report;
    }

    match run_stage67_smoke(opts) {
        Ok(outcome) => apply_stage67_outcome(&mut report, outcome),
        Err(err) => {
            report["blocked"] = json!(true);
...
#[derive(Debug)]
struct Stage67Outcome {
    listener_report: TcpLoopbackListenerReport,
    last_dial_report: TcpDirectDialReport,
    server_summary: VMessPacketAddrUdpServerSummary,
    elapsed_ns: u128,
    ns_per_exchange: f64,
    exchange_count: usize,
    payload_len: usize,
    packet_addr_len: usize,
    packet_len: usize,
...
fn run_stage67_smoke(opts: &Stage67Options) -> Result<Stage67Outcome, String> {
    let (server_addr, listener_report, handle) = spawn_vmess_packet_addr_udp_server(opts)?;
    ...
    for _ in 0..opts.benchmark_iters {
        ...
        .map_err(|err| format!("stage67 magic_tcp_connect failed: {err}"))?;
        ...
        .map_err(|err| format!("stage67 VMess packet-addr UDP exchange failed: {err}"))?;
        if report.echoed_payload != opts.payload {
            return Err("stage67 VMess packet-addr UDP payload response mismatch".to_owned());
        }
    ...
    let server_summary = handle
        .join()
        .map_err(|_| "stage67 VMess packet-addr server thread panicked".to_owned())??;
    if server_summary.accepted != opts.benchmark_iters {
        return Err(format!(
            "stage67 VMess server accepted {} connections, want {}",
            server_summary.accepted, opts.benchmark_iters
        ));
    }
    let last_exchange =
        last_exchange.ok_or_else(|| "stage67 missing exchange report".to_owned())?;
    Ok(Stage67Outcome {
        listener_report,
        last_dial_report: last_dial_report
            .ok_or_else(|| "stage67 missing underlay dial report".to_owned())?,
        server_summary,
        elapsed_ns,
        ns_per_exchange: elapsed_ns as f64 / opts.benchmark_iters as f64,
        exchange_count: opts.benchmark_iters,
        payload_len: opts.payload.len(),
        packet_addr_len: last_exchange.packet_addr_len,
        packet_len: last_exchange.packet_len,
...
fn apply_stage67_outcome(report: &mut Value, outcome: Stage67Outcome) {
    ...
    let server_complete = outcome.server_summary.accepted == outcome.exchange_count
        ...
        && outcome.server_summary.magic_domain_count == outcome.exchange_count
        && outcome.server_summary.packet_addr_prefix_count == outcome.exchange_count
    ...
    report["vmess_packet_addr_udp_smoke_passed"] = json!(passed);
    report["vmess_udp_packet_addr_admitted"] = json!(passed);
    report["vmess_protocol_partial_admitted"] = json!(passed);
    report["vmess_packet_addr_udp_contract"]["server"] = json!(outcome.last_dial_report.peer_addr);
    report["vmess_packet_addr_udp_contract"]["cmd_key_hex"] = json!(outcome.cmd_key_hex);
    report["vmess_packet_addr_udp_contract"]["eauth_crc_validated"] = json!(passed);
    report["vmess_packet_addr_udp_contract"]["request_header_aead_validated"] = json!(passed);
    report["vmess_packet_addr_udp_contract"]["response_header_aead_validated"] = json!(passed);
    report["vmess_packet_addr_udp_contract"]["shake128_chunk_masking_validated"] = json!(passed);
    report["vmess_packet_addr_udp_contract"]["udp_command_validated"] = json!(passed);
    report["vmess_packet_addr_udp_contract"]["magic_domain_validated"] = json!(passed);
    report["vmess_packet_addr_udp_contract"]["packet_addr_prefix_validated"] = json!(passed);
    report["vmess_packet_addr_udp_contract"]["packet_target_validated"] = json!(passed);
    report["vmess_packet_addr_udp_contract"]["payload_roundtrip_validated"] = json!(passed);
    ...
    report["server_observation"] = json!({
        ...
        "magic_domain_count": outcome.server_summary.magic_domain_count,
        "packet_addr_prefix_count": outcome.server_summary.packet_addr_prefix_count,
        ...
        "request_targets": outcome.server_summary.request_targets,
    ...
    report["benchmark"]["ns_per_vmess_packet_addr_udp_exchange"] = json!(outcome.ns_per_exchange);
    report["benchmark"]["payload_len"] = json!(outcome.payload_len);
    report["benchmark"]["packet_addr_len"] = json!(outcome.packet_addr_len);
    ...
    report["protocol_matrix"]["vmess_udp_packet_addr_admitted"] = json!(passed);
}

#[derive(Debug, Default)]
struct VMessPacketAddrUdpServerSummary {
    accepted: usize,
    eauth_crc_count: usize,
    request_header_aead_count: usize,
    response_header_aead_count: usize,
    udp_command_count: usize,
    magic_domain_count: usize,
    packet_addr_prefix_count: usize,
...
fn spawn_vmess_packet_addr_udp_server(
    opts: &Stage67Options,
) -> Result<
    (
        SocketAddrV4,
        TcpLoopbackListenerReport,
        thread::JoinHandle<Result<VMessPacketAddrUdpServerSummary, String>>,
    ),
    String,
> {
    let (listener, listener_report) = bind_loopback_tcp_listener(opts.mptcp)
        .map_err(|err| format!("stage67 bind loopback VMess server failed: {err}"))?;
    let local_addr = listener
        .local_addr()
        .map_err(|err| format!("stage67 VMess server local_addr failed: {err}"))?;
    let server_addr = match local_addr {
        SocketAddr::V4(addr) => addr,
        SocketAddr::V6(addr) => return Err(format!("stage67 VMess listener is not IPv4: {addr}")),
    };
    listener
        .set_nonblocking(true)
        .map_err(|err| format!("stage67 VMess nonblocking failed: {err}"))?;
...
fn accept_vmess_packet_addr_udp(
    ...
    expected_payload: &[u8],
    timeout: Duration,
) -> Result<VMessPacketAddrUdpServerSummary, String> {
    let mut summary = VMessPacketAddrUdpServerSummary::default();
        ...
        match listener.accept() {
            Ok((mut stream, _)) => {
                stream
                    ...
                    .map_err(|err| format!("stage67 server set read timeout failed: {err}"))?;
                stream
                    .set_write_timeout(Some(timeout))
                    .map_err(|err| format!("stage67 server set write timeout failed: {err}"))?;
                handle_vmess_packet_addr_udp(
                    &mut stream,
                    uuid,
            ...
            Err(err) if err.kind() == ErrorKind::WouldBlock => {
                return Err(format!(
                    "stage67 VMess server timed out after accepting {} of {} connections",
                    summary.accepted, iterations
                ));
            }
            Err(err) => return Err(format!("stage67 VMess accept failed: {err}")),
        }
    }
    Ok(summary)
}

fn handle_vmess_packet_addr_udp(
    stream: &mut TcpStream,
    uuid: &str,
    expected_packet_target: &str,
    expected_payload: &[u8],
    summary: &mut VMessPacketAddrUdpServerSummary,
) -> Result<(), String> {
    let request = vmess::read_aead_packet_addr_udp_request_from_stream(stream, uuid)
        .map_err(|err| format!("stage67 read VMess packet-addr request failed: {err}"))?;
    if !request.request.eauth_crc_validated {
        return Err("stage67 VMess EAuthID checksum was not validated".to_owned());
    }
    if request.request.command != dae_outbound::VMessNetwork::Udp.byte() {
        return Err(format!(
            "stage67 VMess command mismatch: got {}, want {}",
            request.request.command,
            dae_outbound::VMessNetwork::Udp.byte()
        ));
    }
    let expected_request_target = vmess::packet_addr_magic_target(expected_packet_target)
        .map_err(|err| format!("stage67 packet target invalid: {err}"))?;
    if request.request.target != expected_request_target {
        return Err(format!(
            "stage67 VMess magic target mismatch: got {}, want {}",
            request.request.target, expected_request_target
        ));
    }
    if request.packet_target != expected_packet_target {
        return Err(format!(
            "stage67 VMess packet target mismatch: got {}, want {}",
            request.packet_target, expected_packet_target
        ));
    }
    if request.packet_payload != expected_payload {
        return Err("stage67 VMess packet-addr payload mismatch".to_owned());
    }
    let response = vmess::aead_tcp_response_packet(&request.request, &request.request.payload)
        .map_err(|err| format!("stage67 encode VMess packet-addr response failed: {err}"))?;
    stream
        .write_all(&response)
        .map_err(|err| format!("stage67 write VMess packet-addr echo failed: {err}"))?;
    ...
    summary.magic_domain_count += 1;
    summary.packet_addr_prefix_count += 1;
    summary.packet_target_count += 1;
    summary.shake128_chunk_masking_count += 1;
    summary.payload_roundtrip_count += 1;
    summary.request_targets.push(request.request.target);
    summary.packet_targets.push(request.packet_target);
    summary
        .payload_ascii
        .push(String::from_utf8_lossy(&request.packet_payload).to_string());
    summary
        .response_ascii
        .push(String::from_utf8_lossy(&request.packet_payload).to_string());
    Ok(())

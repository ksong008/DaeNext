...
#[derive(Debug)]
pub(super) struct Stage89Outcome {
    ...
    server_summary: Ss2022MultiPskServerSummary,
    elapsed_ns: u128,
    ns_per_exchange: f64,
    exchange_count: usize,
}

pub(super) fn run_stage89_smoke(opts: &Stage89Options) -> Result<Stage89Outcome, String> {
    let conf = shadowsocks::ss2022::cipher_conf(&opts.cipher)
        .ok_or_else(|| format!("stage89 unsupported SS2022 cipher: {}", opts.cipher))?;
    let (server_addr, listener_report, handle) =
        spawn_ss2022_multi_psk_server(opts, conf.salt_len)?;
    ...
    for index in 0..opts.benchmark_iters {
        ...
        .map_err(|err| format!("stage89 magic_tcp_connect failed: {err}"))?;
        let client_salt = salt_for(index, conf.salt_len, 0x51);
        let server_salt = salt_for(index, conf.salt_len, 0xa1);
        ...
        .map_err(|err| format!("stage89 SS2022 multi-PSK exchange failed: {err}"))?;
    ...
    let server_summary = handle
        .join()
        .map_err(|_| "stage89 SS2022 multi-PSK server thread panicked".to_owned())??;
    if server_summary.accepted != opts.benchmark_iters {
        return Err(format!(
            "stage89 SS2022 multi-PSK server accepted {} connections, want {}",
    ...
    Ok(Stage89Outcome {
        ...
        last_dial_report: last_dial_report
            .ok_or_else(|| "stage89 missing underlay dial report".to_owned())?,
        client_report: last_client_report
            .ok_or_else(|| "stage89 missing SS2022 multi-PSK client report".to_owned())?,
        server_summary,
        elapsed_ns,
...
pub(super) fn apply_stage89_outcome(report: &mut Value, outcome: Stage89Outcome) {
    ...
    let server_complete = outcome.server_summary.accepted == outcome.exchange_count
        ...
        && outcome.server_summary.multi_psk_count == outcome.exchange_count
        && outcome.server_summary.upsk_last_count == outcome.exchange_count
        && outcome.server_summary.identity_header_count == outcome.exchange_count
        && outcome.server_summary.identity_header_bytes_len == outcome.exchange_count * 16
        && outcome.server_summary.identity_header_validated_count == outcome.exchange_count
    ...
    let client_complete = outcome.client_report.true_dataplane
        && outcome.client_report.psk_count >= 2
        && outcome.client_report.upsk_index == outcome.client_report.psk_count - 1
        && outcome.client_report.request_salt_echo_validated
        && outcome.client_report.identity_header_count >= 1
        && outcome.client_report.identity_header_validated
        && outcome
            .client_report
    ...
    report["ss2022_multi_psk_identity_header_smoke_passed"] = json!(passed);
    report["ss2022_multi_psk_identity_header_dataplane_admitted"] = json!(passed);
    ...
    report["ss2022_contract"]["identity_header_count"] =
        json!(outcome.client_report.identity_header_count);
    report["ss2022_contract"]["identity_header_bytes_len"] =
        json!(outcome.client_report.identity_header_bytes_len);
    report["ss2022_contract"]["identity_header_validated"] =
        json!(outcome.client_report.identity_header_validated);
    ...
    report["server_observation"] = json!({
        ...
        "multi_psk_count": outcome.server_summary.multi_psk_count,
        "upsk_last_count": outcome.server_summary.upsk_last_count,
        "identity_header_count": outcome.server_summary.identity_header_count,
        "identity_header_bytes_len": outcome.server_summary.identity_header_bytes_len,
        "identity_header_validated_count": outcome.server_summary.identity_header_validated_count,
    ...
    report["benchmark"]["ns_per_ss2022_multi_psk_exchange"] = json!(outcome.ns_per_exchange);
    ...
    report["benchmark"]["identity_header_count"] =
        json!(outcome.client_report.identity_header_count);
    report["benchmark"]["identity_header_bytes_len"] =
        json!(outcome.client_report.identity_header_bytes_len);
    report["protocol_matrix"]["ss2022_multi_psk_identity_header_dataplane_admitted"] =
        json!(passed);
}

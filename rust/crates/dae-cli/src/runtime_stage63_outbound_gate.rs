...
const DEFAULT_PAYLOAD: &[u8] = b"stage63-vless-udp-ping";

pub(crate) fn run_stage63_vless_udp_over_tcp_dataplane_admission(args: &[String]) -> RunnerOutput {
    let opts = match Stage63Options::parse(args) {
        Ok(opts) => opts,
        Err(output) => return output,
    };
    let report = stage63_report(&opts);
    let passed = report["vless_udp_over_tcp_smoke_passed"]
        .as_bool()
        .unwrap_or(false);
...
impl Default for Stage63Options {
    fn default() -> Self {
        Self {
            execute_smoke: false,
            ack_root_gate: false,
    ...
    fn parse(args: &[String]) -> Result<Self, RunnerOutput> {
            ...
            match arg.as_str() {
                ...
                "--benchmark-iters" => {
                    opts.benchmark_iters =
                        parse_usize(&next_value(&mut iter, "stage63 --benchmark-iters")?, arg)?;
                }
                "--uuid" => opts.uuid = next_value(&mut iter, "stage63 --uuid")?,
                "--target" => opts.target = next_value(&mut iter, "stage63 --target")?,
                "--payload" => {
                    opts.payload = next_value(&mut iter, "stage63 --payload")?.into_bytes();
                }
                "--so-mark" => {
                    opts.so_mark = parse_u32(&next_value(&mut iter, "stage63 --so-mark")?, arg)?;
                }
                "--mptcp" => opts.mptcp = true,
                "--no-mptcp" => opts.mptcp = false,
                "--timeout-ms" => {
                    let timeout_ms =
                        parse_u64(&next_value(&mut iter, "stage63 --timeout-ms")?, arg)?;
                ...
                _ => {
                    return Err(RunnerOutput::usage(format!(
                        "unsupported stage63 argument: {arg}"
                    )));
                }
            }
        }
        if opts.benchmark_iters == 0 {
            return Err(RunnerOutput::usage(
                "stage63 --benchmark-iters must be greater than zero",
            ));
        }
        vless::password_to_key(&opts.uuid)
            .map_err(|err| RunnerOutput::usage(format!("stage63 uuid is invalid: {err}")))?;
        Ok(opts)
    }
}

fn stage63_report(opts: &Stage63Options) -> Value {
    let key = match vless::password_to_key(&opts.uuid) {
        Ok(key) => key,
        Err(err) => {
            return json!({
                "name": "stage63-vless-udp-over-tcp-dataplane-admission",
                "stage": "stage63",
                "blocked": true,
                "blockers": [format!("stage63 uuid is invalid: {err}")]
            });
        }
    };
    ...
    let mut report = json!({
        "name": "stage63-vless-udp-over-tcp-dataplane-admission",
        "stage": "stage63",
        "evidence_class": "opt-in-protocol-vless-udp-over-tcp-true-dataplane-smoke",
    ...
    report["vless_udp_over_tcp_smoke_passed"] = json!(false);
    report["vless_udp_over_tcp_admitted"] = json!(false);
    ...
    report["vless_udp_over_tcp_contract"] = json!({
        "protocol": "vless",
        "scope": "VLESS UDP-over-TCP security=none header plus payload echo over Rust TCP stream",
        "uuid": opts.uuid,
        "key_hex": key_hex,
        "network": "udp",
        ...
        "mux_deferred": "VLESS mux rows are deferred",
    ...
    report["benchmark"] = json!({
        ...
        "elapsed_ns": null,
        "ns_per_vless_udp_over_tcp_exchange": null,
        "scope": "VLESS UDP-over-TCP first-write parse plus response-header/length-prefixed payload echo over SO_MARKed Rust TCP socket",
    ...
    report["protocol_matrix"] = json!({
        ...
        "vless_udp_over_tcp_admitted": false,
    ...
    if !opts.ack_root_gate {
        ...
        report["blockers"] = json!([
            "stage63 root-gated smoke requires --ack-root-gate because it attempts SO_MARK/MPTCP underlay socket observation"
        ]);
        return report;
    }

    match run_stage63_smoke(opts, key) {
        Ok(outcome) => apply_stage63_outcome(&mut report, outcome),
        Err(err) => {
            report["blocked"] = json!(true);
...
#[derive(Debug)]
struct Stage63Outcome {
    listener_report: TcpLoopbackListenerReport,
    last_dial_report: TcpDirectDialReport,
    server_summary: VlessUdpOverTcpServerSummary,
    elapsed_ns: u128,
    ns_per_vless_udp_over_tcp_exchange: f64,
    exchange_count: usize,
    payload_len: usize,
    key_hex: String,
}

fn run_stage63_smoke(opts: &Stage63Options, key: [u8; 16]) -> Result<Stage63Outcome, String> {
    let (server_addr, listener_report, handle) = spawn_vless_udp_over_tcp_server(opts, key)?;
    ...
    for _ in 0..opts.benchmark_iters {
        ...
        .map_err(|err| format!("stage63 magic_tcp_connect failed: {err}"))?;
        ...
        .map_err(|err| format!("stage63 VLESS UDP-over-TCP exchange failed: {err}"))?;
        if report.echoed_payload != opts.payload {
            return Err("stage63 VLESS UDP-over-TCP payload response mismatch".to_owned());
        }
    ...
    let server_summary = handle
        .join()
        .map_err(|_| "stage63 VLESS server thread panicked".to_owned())??;
    if server_summary.accepted != opts.benchmark_iters {
        return Err(format!(
            "stage63 VLESS server accepted {} connections, want {}",
    ...
    Ok(Stage63Outcome {
        ...
        last_dial_report: last_dial_report
            .ok_or_else(|| "stage63 missing underlay dial report".to_owned())?,
        server_summary,
        elapsed_ns,
        ns_per_vless_udp_over_tcp_exchange: elapsed_ns as f64 / opts.benchmark_iters as f64,
...
fn apply_stage63_outcome(report: &mut Value, outcome: Stage63Outcome) {
    ...
    let server_complete = outcome.server_summary.accepted == outcome.exchange_count
        ...
        && outcome.server_summary.length_prefix_count == outcome.exchange_count
    ...
    report["vless_udp_over_tcp_smoke_passed"] = json!(passed);
    report["vless_udp_over_tcp_admitted"] = json!(passed);
    report["vless_protocol_partial_admitted"] = json!(passed);
    report["vless_udp_over_tcp_contract"]["server"] = json!(outcome.last_dial_report.peer_addr);
    report["vless_udp_over_tcp_contract"]["key_hex"] = json!(outcome.key_hex);
    report["vless_udp_over_tcp_contract"]["version_validated"] = json!(passed);
    report["vless_udp_over_tcp_contract"]["key_validated"] = json!(passed);
    report["vless_udp_over_tcp_contract"]["addons_empty_validated"] = json!(passed);
    report["vless_udp_over_tcp_contract"]["udp_command_validated"] = json!(passed);
    report["vless_udp_over_tcp_contract"]["target_metadata_validated"] = json!(passed);
    report["vless_udp_over_tcp_contract"]["length_prefix_validated"] = json!(passed);
    report["vless_udp_over_tcp_contract"]["response_header_validated"] = json!(passed);
    report["vless_udp_over_tcp_contract"]["payload_roundtrip_validated"] = json!(passed);
    ...
    report["server_observation"] = json!({
        ...
        "length_prefix_count": outcome.server_summary.length_prefix_count,
    ...
    report["benchmark"]["ns_per_vless_udp_over_tcp_exchange"] =
        json!(outcome.ns_per_vless_udp_over_tcp_exchange);
    report["benchmark"]["payload_len"] = json!(outcome.payload_len);
    report["protocol_matrix"]["vless_udp_over_tcp_admitted"] = json!(passed);
}

#[derive(Debug, Default)]
struct VlessUdpOverTcpServerSummary {
    accepted: usize,
    version_count: usize,
...
fn spawn_vless_udp_over_tcp_server(
    opts: &Stage63Options,
    key: [u8; 16],
) -> Result<
    (
        ...
        thread::JoinHandle<Result<VlessUdpOverTcpServerSummary, String>>,
    ),
    String,
> {
    let (listener, listener_report) = bind_loopback_tcp_listener(opts.mptcp)
        .map_err(|err| format!("stage63 bind loopback VLESS server failed: {err}"))?;
    let local_addr = listener
        .local_addr()
        .map_err(|err| format!("stage63 VLESS server local_addr failed: {err}"))?;
    let server_addr = match local_addr {
        SocketAddr::V4(addr) => addr,
        SocketAddr::V6(addr) => return Err(format!("stage63 VLESS listener is not IPv4: {addr}")),
    };
    listener
        .set_nonblocking(true)
        .map_err(|err| format!("stage63 VLESS nonblocking failed: {err}"))?;
    ...
    let handle = thread::spawn(move || {
        accept_vless_udp_over_tcp(listener, iterations, key, &target, &payload, timeout)
    });
    Ok((server_addr, listener_report, handle))
}

fn accept_vless_udp_over_tcp(
    ...
    expected_payload: &[u8],
    timeout: Duration,
) -> Result<VlessUdpOverTcpServerSummary, String> {
    let mut summary = VlessUdpOverTcpServerSummary::default();
        ...
        match listener.accept() {
            Ok((mut stream, _)) => {
                stream
                    ...
                    .map_err(|err| format!("stage63 server set read timeout failed: {err}"))?;
                stream
                    .set_write_timeout(Some(timeout))
                    .map_err(|err| format!("stage63 server set write timeout failed: {err}"))?;
                handle_vless_udp_over_tcp(
                    &mut stream,
                    expected_key,
                    expected_target,
                    expected_payload,
            ...
            Err(err) if err.kind() == ErrorKind::WouldBlock => {
                return Err(format!(
                    "stage63 VLESS server timed out after accepting {} of {} connections",
                    summary.accepted, iterations
                ));
            }
            Err(err) => return Err(format!("stage63 VLESS accept failed: {err}")),
        }
    }
    Ok(summary)
}

fn handle_vless_udp_over_tcp(
    stream: &mut TcpStream,
    expected_key: [u8; 16],
    expected_target: &str,
    expected_payload: &[u8],
    summary: &mut VlessUdpOverTcpServerSummary,
) -> Result<(), String> {
    let request = vless::read_udp_request_from_stream(stream)
        .map_err(|err| format!("stage63 read VLESS request failed: {err}"))?;
    if request.version != vless::VLESS_VERSION {
        return Err(format!(
            "stage63 VLESS version mismatch: got {}, want {}",
            request.version,
            vless::VLESS_VERSION
        ));
    }
    if request.key != expected_key {
        return Err("stage63 VLESS key mismatch".to_owned());
    }
    if request.addons_len != 0 {
        return Err(format!(
            "stage63 VLESS addons length mismatch: got {}, want 0",
            request.addons_len
        ));
    }
    if request.command != dae_outbound::VMessNetwork::Udp.byte() {
        return Err(format!(
            "stage63 VLESS command mismatch: got {}, want {}",
            request.command,
    ...
    if request.target != expected_target {
        return Err(format!(
            "stage63 VLESS target mismatch: got {}, want {}",
            request.target, expected_target
        ));
    }
    if request.payload != expected_payload {
        return Err("stage63 VLESS payload mismatch".to_owned());
    }
    if request.payload_len != expected_payload.len()
        || request.packet_len != 2 + expected_payload.len()
    {
        return Err(format!(
            "stage63 VLESS UDP packet length mismatch: got payload_len={} packet_len={}, want payload_len={} packet_len={}",
            request.payload_len,
            request.packet_len,
            expected_payload.len(),
            2 + expected_payload.len()
        ));
    }
    let response = vless::udp_response_packet(&request.payload)
        .map_err(|err| format!("stage63 encode VLESS UDP response failed: {err}"))?;
    stream
        .write_all(&response)
        .map_err(|err| format!("stage63 write VLESS echo failed: {err}"))?;

...
#[derive(Debug, Clone)]
pub(super) struct Stage119Options {
...
impl Default for Stage119Options {
    fn default() -> Self {
        Self {
            execute_smoke: false,
            benchmark_iters: 1,
    ...
    pub(super) fn parse(args: &[String]) -> Result<Self, RunnerOutput> {
            ...
            match arg.as_str() {
                ...
                "--benchmark-iters" => {
                    opts.benchmark_iters =
                        parse_usize(&next_value(&mut iter, "stage119 --benchmark-iters")?, arg)?;
                }
                "--payload" => {
                    opts.payload = next_value(&mut iter, "stage119 --payload")?.into_bytes();
                }
                "--timeout-ms" => {
                    opts.timeout = Duration::from_millis(parse_u64(
                        &next_value(&mut iter, "stage119 --timeout-ms")?,
                        arg,
                    )?);
                }
                ...
                _ => {
                    return Err(RunnerOutput::usage(format!(
                        "unsupported stage119 argument: {arg}"
                    )));
                }
            }
        }
        if opts.benchmark_iters == 0 {
            return Err(RunnerOutput::usage(
                "stage119 --benchmark-iters must be greater than zero",
            ));
        }
        if opts.payload.is_empty() {
            return Err(RunnerOutput::usage("stage119 --payload cannot be empty"));
        }
        Ok(opts)
    }
}

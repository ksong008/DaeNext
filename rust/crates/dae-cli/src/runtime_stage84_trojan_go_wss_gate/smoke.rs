...
#[derive(Debug)]
pub(super) struct Stage84Outcome {
    ...
    pub(super) server_summary: TrojanGoWssServerSummary,
...
pub(super) fn run_stage84_smoke(opts: &Stage84Options) -> Result<Stage84Outcome, String> {
    let tls_options = opts
        .tls_options()
        .map_err(|err| format!("stage84 tls options invalid: {err}"))?;
    let material = shared_transport::tls_loopback_material(&tls_options)
        .map_err(|err| format!("stage84 build tls material failed: {err}"))?;
    let certificate_der_len = material.certificate_der_len;
    let (server_addr, listener_report, handle) = spawn_trojan_go_wss_server(opts, &material)?;
    ...
    for _ in 0..opts.benchmark_iters {
        ...
        .map_err(|err| format!("stage84 magic_tcp_connect failed: {err}"))?;
        ...
        .map_err(|err| format!("stage84 trojan-go WSS exchange failed: {err}"))?;
        if report.echoed_payload != opts.payload {
            return Err("stage84 trojan-go WSS payload response mismatch".to_owned());
        }
    ...
    let server_summary = handle
        .join()
        .map_err(|_| "stage84 trojan-go WSS server thread panicked".to_owned())??;
    if server_summary.accepted != opts.benchmark_iters {
        return Err(format!(
            "stage84 trojan-go WSS server accepted {} connections, want {}",
    ...
    Ok(Stage84Outcome {
        ...
        last_dial_report: last_dial_report
            .ok_or_else(|| "stage84 missing underlay dial report".to_owned())?,
        client_report: last_client_report
            .ok_or_else(|| "stage84 missing trojan-go WSS client report".to_owned())?,
        server_summary,
        certificate_der_len,
        elapsed_ns,
...
pub(super) fn apply_stage84_outcome(report: &mut Value, outcome: Stage84Outcome) {
    ...
    report["trojan_go_wss_smoke_passed"] = json!(passed);
    report["trojan_go_wss_admitted"] = json!(passed);
    report["trojan_go_shared_transport_partial_admitted"] = json!(passed);
    report["trojan_go_wss_contract"]["server"] = json!(outcome.last_dial_report.peer_addr);
    report["trojan_go_wss_contract"]["selected_alpn"] = json!(outcome.client_report.selected_alpn);
    report["trojan_go_wss_contract"]["certificate_der_len"] = json!(outcome.certificate_der_len);
    report["trojan_go_wss_contract"]["tls_handshake_validated"] = json!(passed);
    report["trojan_go_wss_contract"]["certificate_chain_validated"] = json!(passed);
    report["trojan_go_wss_contract"]["server_name_validated"] = json!(passed);
    report["trojan_go_wss_contract"]["alpn_validated"] = json!(passed);
    report["trojan_go_wss_contract"]["websocket_upgrade_validated"] = json!(passed);
    report["trojan_go_wss_contract"]["websocket_binary_frame_validated"] = json!(passed);
    report["trojan_go_wss_contract"]["password_sha224_validated"] = json!(passed);
    report["trojan_go_wss_contract"]["tcp_command_validated"] = json!(passed);
    report["trojan_go_wss_contract"]["target_metadata_validated"] = json!(passed);
    report["trojan_go_wss_contract"]["payload_roundtrip_validated"] = json!(passed);
    ...
    report["benchmark"]["ns_per_trojan_go_wss_exchange"] = json!(outcome.ns_per_exchange);
    ...
    report["protocol_matrix"]["trojan_go_wss_admitted"] = json!(passed);

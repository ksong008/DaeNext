use super::*;

#[derive(Debug, Default)]
pub(super) struct AnyTlsSessionServerSummary {
    pub(super) accepted: usize,
    pub(super) tls_handshake_count: usize,
    pub(super) alpn_validated_count: usize,
    pub(super) auth_key_match_count: usize,
    pub(super) settings_frame_count: usize,
    pub(super) syn_frame_count: usize,
    pub(super) psh_target_frame_count: usize,
    pub(super) psh_payload_frame_count: usize,
    pub(super) synack_response_count: usize,
    pub(super) payload_roundtrip_count: usize,
    pub(super) selected_alpns: Vec<String>,
    pub(super) targets: Vec<String>,
    pub(super) payload_ascii: Vec<String>,
    pub(super) response_ascii: Vec<String>,
    pub(super) settings_frame_lens: Vec<usize>,
    pub(super) psh_target_frame_lens: Vec<usize>,
    pub(super) psh_payload_frame_lens: Vec<usize>,
}

pub(super) fn spawn_anytls_session_server(
    opts: &Stage104Options,
    material: &shared_transport::TlsLoopbackMaterial,
) -> Result<
    (
        SocketAddrV4,
        TcpLoopbackListenerReport,
        thread::JoinHandle<Result<AnyTlsSessionServerSummary, String>>,
    ),
    String,
> {
    let (listener, listener_report) = bind_loopback_tcp_listener(opts.mptcp)
        .map_err(|err| format!("stage104 bind loopback anytls server failed: {err}"))?;
    let local_addr = listener
        .local_addr()
        .map_err(|err| format!("stage104 anytls server local_addr failed: {err}"))?;
    let server_addr = match local_addr {
        SocketAddr::V4(addr) => addr,
        SocketAddr::V6(addr) => {
            return Err(format!("stage104 anytls listener is not IPv4: {addr}"));
        }
    };
    listener
        .set_nonblocking(true)
        .map_err(|err| format!("stage104 anytls nonblocking failed: {err}"))?;

    let iterations = opts.benchmark_iters;
    let auth = opts.auth.clone();
    let target = opts.target.clone();
    let payload = opts.payload.clone();
    let expected_alpn = opts.alpn_protocol.clone();
    let timeout = opts.timeout;
    let server_config = material.server_config.clone();
    let handle = thread::spawn(move || {
        accept_anytls_session(
            listener,
            iterations,
            &auth,
            &target,
            &payload,
            &expected_alpn,
            timeout,
            server_config,
        )
    });
    Ok((server_addr, listener_report, handle))
}

fn accept_anytls_session(
    listener: TcpListener,
    iterations: usize,
    auth: &str,
    expected_target: &str,
    expected_payload: &[u8],
    expected_alpn: &str,
    timeout: Duration,
    server_config: std::sync::Arc<rustls::ServerConfig>,
) -> Result<AnyTlsSessionServerSummary, String> {
    let mut summary = AnyTlsSessionServerSummary::default();
    let deadline = Instant::now() + timeout.saturating_mul(iterations as u32 + 2);
    while summary.accepted < iterations {
        match listener.accept() {
            Ok((stream, _)) => {
                stream
                    .set_read_timeout(Some(timeout))
                    .map_err(|err| format!("stage104 server set read timeout failed: {err}"))?;
                stream
                    .set_write_timeout(Some(timeout))
                    .map_err(|err| format!("stage104 server set write timeout failed: {err}"))?;
                let conn = rustls::ServerConnection::new(server_config.clone())
                    .map_err(|err| format!("stage104 server tls accept failed: {err}"))?;
                let mut tls = rustls::StreamOwned::new(conn, stream);
                handle_anytls_session(
                    &mut tls,
                    auth,
                    expected_target,
                    expected_payload,
                    &mut summary,
                )?;
                let selected_alpn = tls
                    .conn
                    .alpn_protocol()
                    .map(|value| String::from_utf8_lossy(value).to_string())
                    .unwrap_or_default();
                if selected_alpn == expected_alpn {
                    summary.alpn_validated_count += 1;
                }
                summary.selected_alpns.push(selected_alpn);
                summary.tls_handshake_count += 1;
                summary.accepted += 1;
            }
            Err(err) if err.kind() == ErrorKind::WouldBlock && Instant::now() < deadline => {
                thread::sleep(Duration::from_millis(5));
            }
            Err(err) if err.kind() == ErrorKind::WouldBlock => {
                return Err(format!(
                    "stage104 anytls server timed out after accepting {} of {} connections",
                    summary.accepted, iterations
                ));
            }
            Err(err) => return Err(format!("stage104 anytls accept failed: {err}")),
        }
    }
    Ok(summary)
}

fn handle_anytls_session<S>(
    stream: &mut S,
    auth: &str,
    expected_target: &str,
    expected_payload: &[u8],
    summary: &mut AnyTlsSessionServerSummary,
) -> Result<(), String>
where
    S: Read + Write,
{
    let expected_auth = anytls::link::handshake_auth_bytes(auth);
    let mut auth_handshake = vec![0_u8; expected_auth.len()];
    stream
        .read_exact(&mut auth_handshake)
        .map_err(|err| format!("stage104 read anytls auth handshake failed: {err}"))?;
    if auth_handshake != expected_auth {
        return Err("stage104 anytls auth key mismatch".to_owned());
    }

    let settings = anytls::read_frame_from_stream(stream)
        .map_err(|err| format!("stage104 read anytls settings frame failed: {err}"))?;
    if settings.cmd != anytls::contract::CMD_SETTINGS
        || settings.sid != 1
        || settings.data != anytls::link::settings_bytes()
    {
        return Err("stage104 anytls settings frame mismatch".to_owned());
    }
    let syn = anytls::read_frame_from_stream(stream)
        .map_err(|err| format!("stage104 read anytls SYN frame failed: {err}"))?;
    if syn.cmd != anytls::contract::CMD_SYN || syn.sid != 1 || !syn.data.is_empty() {
        return Err("stage104 anytls SYN frame mismatch".to_owned());
    }
    let psh_target = anytls::read_frame_from_stream(stream)
        .map_err(|err| format!("stage104 read anytls target PSH frame failed: {err}"))?;
    if psh_target.cmd != anytls::contract::CMD_PSH || psh_target.sid != 1 {
        return Err("stage104 anytls target PSH frame mismatch".to_owned());
    }
    let (target, consumed) = Socks5Address::decode(&psh_target.data)
        .map_err(|err| format!("stage104 anytls target address decode failed: {err}"))?;
    if consumed != psh_target.data.len() {
        return Err("stage104 anytls target address trailing bytes".to_owned());
    }
    if target.authority() != expected_target {
        return Err(format!(
            "stage104 anytls target mismatch: got {}, want {}",
            target.authority(),
            expected_target
        ));
    }
    anytls::write_frame_to_stream(stream, anytls::contract::CMD_SYNACK, 1, &[])
        .map_err(|err| format!("stage104 write anytls SYNACK failed: {err}"))?;

    let psh_payload = anytls::read_frame_from_stream(stream)
        .map_err(|err| format!("stage104 read anytls payload PSH frame failed: {err}"))?;
    if psh_payload.cmd != anytls::contract::CMD_PSH
        || psh_payload.sid != 1
        || psh_payload.data != expected_payload
    {
        return Err("stage104 anytls payload PSH frame mismatch".to_owned());
    }
    anytls::write_frame_to_stream(stream, anytls::contract::CMD_PSH, 1, &psh_payload.data)
        .map_err(|err| format!("stage104 write anytls payload echo failed: {err}"))?;

    summary.auth_key_match_count += 1;
    summary.settings_frame_count += 1;
    summary.syn_frame_count += 1;
    summary.psh_target_frame_count += 1;
    summary.psh_payload_frame_count += 1;
    summary.synack_response_count += 1;
    summary.payload_roundtrip_count += 1;
    summary.targets.push(target.authority());
    summary
        .payload_ascii
        .push(String::from_utf8_lossy(&psh_payload.data).to_string());
    summary
        .response_ascii
        .push(String::from_utf8_lossy(&psh_payload.data).to_string());
    summary
        .settings_frame_lens
        .push(anytls::contract::HEADER_OVERHEAD_SIZE + settings.data.len());
    summary
        .psh_target_frame_lens
        .push(anytls::contract::HEADER_OVERHEAD_SIZE + psh_target.data.len());
    summary
        .psh_payload_frame_lens
        .push(anytls::contract::HEADER_OVERHEAD_SIZE + psh_payload.data.len());
    Ok(())
}

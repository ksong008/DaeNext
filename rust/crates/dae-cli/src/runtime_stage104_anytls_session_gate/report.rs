use super::smoke::{apply_stage104_outcome, run_stage104_smoke};
use super::*;

pub(super) fn stage104_report(opts: &Stage104Options) -> Value {
    let tls_options = match opts.tls_options() {
        Ok(options) => options,
        Err(err) => {
            return json!({
                "name": "stage104-anytls-session-frame-admission",
                "stage": "stage104",
                "blocked": true,
                "blockers": [format!("stage104 tls options invalid: {err}")]
            });
        }
    };
    if let Err(err) = Socks5Address::parse(&opts.target) {
        return json!({
            "name": "stage104-anytls-session-frame-admission",
            "stage": "stage104",
            "blocked": true,
            "blockers": [format!("stage104 target is invalid: {err}")]
        });
    }
    ...
    let target_addr = match anytls::link::socks_addr(&opts.target) {
        Ok(addr) => addr,
        Err(err) => {
            return json!({
                "name": "stage104-anytls-session-frame-admission",
                "stage": "stage104",
                "blocked": true,
                "blockers": [format!("stage104 target address invalid: {err}")]
            });
        }
    };
    let udp_stream_target = anytls::link::udp_stream_target("1.2.3.4:53").unwrap_or_default();
    let underlay = anytls::link::underlay_contract("udp", opts.so_mark, opts.mptcp);
    let mut report = json!({
        "name": "stage104-anytls-session-frame-admission",
        "stage": "stage104",
        "evidence_class": "opt-in-protocol-anytls-session-frame-true-dataplane-smoke",
    ...
    report["anytls_session_frame_smoke_passed"] = json!(false);
    report["anytls_session_frame_true_dataplane_admitted"] = json!(false);
    ...
    report["anytls_contract"] = json!({
        "protocol": "anytls",
        "scope": "AnyTLS session first flight and stream payload over rustls TLS underlay",
        "target": opts.target,
    ...
    report["anytls_contract"]["psh_addr_frame_len"] =
        json!(anytls::contract::HEADER_OVERHEAD_SIZE + target_addr.len());
    report["anytls_contract"]["psh_payload_frame_len"] =
        json!(anytls::contract::HEADER_OVERHEAD_SIZE + opts.payload.len());
    report["anytls_contract"]["settings_payload_len"] = json!(settings.len());
    report["anytls_contract"]["target_addr_len"] = json!(target_addr.len());
    ...
    report["anytls_contract"]["udp_stream_target_example"] = json!(udp_stream_target);
    ...
    report["anytls_contract"]["psh_target_validated"] = json!(false);
    ...
    report["anytls_contract"]["udp_packet_stream_deferred"] = json!(true);
    ...
    report["benchmark"] = json!({
        ...
        "elapsed_ns": null,
        "ns_per_anytls_session_frame_exchange": null,
        "scope": "rustls TLS handshake plus AnyTLS auth/settings/SYN/PSH/SYNACK frames plus payload echo over SO_MARKed Rust TCP socket",
    ...
    report["protocol_matrix"] = json!({
        ...
        "anytls_session_frame_true_dataplane_admitted": false,
    ...
    if !opts.ack_root_gate {
        ...
        report["blockers"] = json!([
            "stage104 root-gated smoke requires --ack-root-gate because it attempts SO_MARK/MPTCP underlay socket observation"
        ]);
        return report;
    }

    match run_stage104_smoke(opts) {
        Ok(outcome) => apply_stage104_outcome(&mut report, outcome),
        Err(err) => {
            report["blocked"] = json!(true);

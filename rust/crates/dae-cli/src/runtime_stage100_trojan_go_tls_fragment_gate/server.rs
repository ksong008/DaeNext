...
pub(super) struct TrojanGoTlsFragmentServerSummary {
...
pub(super) fn spawn_trojan_go_tls_fragment_server(
    opts: &Stage100Options,
    material: &shared_transport::TlsLoopbackMaterial,
) -> Result<
    (
        SocketAddrV4,
        TcpLoopbackListenerReport,
        thread::JoinHandle<Result<TrojanGoTlsFragmentServerSummary, String>>,
    ),
    String,
> {
    let (listener, listener_report) = bind_loopback_tcp_listener(opts.mptcp).map_err(|err| {
        format!("stage100 bind loopback trojan-go TLS fragment server failed: {err}")
    })?;
    let local_addr = listener.local_addr().map_err(|err| {
        format!("stage100 trojan-go TLS fragment server local_addr failed: {err}")
    })?;
    let server_addr = match local_addr {
        SocketAddr::V4(addr) => addr,
        SocketAddr::V6(addr) => {
            return Err(format!(
                "stage100 trojan-go TLS fragment listener is not IPv4: {addr}"
            ));
        }
    };
    listener
        .set_nonblocking(true)
        .map_err(|err| format!("stage100 trojan-go TLS fragment nonblocking failed: {err}"))?;
...
fn accept_trojan_go_tls_fragment(
...
) -> Result<TrojanGoTlsFragmentServerSummary, String> {
    let mut summary = TrojanGoTlsFragmentServerSummary::default();
        ...
        match listener.accept() {
            Ok((stream, _)) => {
                stream
                    ...
                    .map_err(|err| format!("stage100 server set read timeout failed: {err}"))?;
                stream
                    .set_write_timeout(Some(timeout))
                    .map_err(|err| format!("stage100 server set write timeout failed: {err}"))?;
                let conn = rustls::ServerConnection::new(server_config.clone())
                    .map_err(|err| format!("stage100 server tls accept failed: {err}"))?;
                let mut tls = rustls::StreamOwned::new(conn, stream);
                handle_trojan_go_tls_fragment(
                    &mut tls,
                    password,
                    expected_target,
                    expected_ws_host,
            ...
            Err(err) if err.kind() == ErrorKind::WouldBlock => {
                return Err(format!(
                    "stage100 trojan-go TLS fragment server timed out after accepting {} of {} connections",
                    summary.accepted, iterations
                ));
            }
            Err(err) => {
                return Err(format!(
                    "stage100 trojan-go TLS fragment accept failed: {err}"
                ));
            }
        }
    }
...
fn handle_trojan_go_tls_fragment<S>(
    stream: &mut S,
    password: &str,
    expected_target: &str,
    expected_ws_host: &str,
    expected_ws_path: &str,
    expected_payload: &[u8],
    summary: &mut TrojanGoTlsFragmentServerSummary,
) -> Result<(), String>
where
    S: Read + Write,
{
    let request_head = shared_transport::read_http_head(stream)
        .map_err(|err| format!("stage100 read WebSocket upgrade failed: {err}"))?;
    let request_head = String::from_utf8(request_head)
        .map_err(|err| format!("stage100 WebSocket upgrade is not UTF-8: {err}"))?;
    if !request_head.starts_with(&format!("GET {expected_ws_path} HTTP/1.1\r\n")) {
        return Err("stage100 WebSocket upgrade path mismatch".to_owned());
    }
    if !request_head.contains(&format!("Host: {expected_ws_host}\r\n")) {
        return Err("stage100 WebSocket Host header mismatch".to_owned());
    }
    if !request_head.contains("Upgrade: websocket\r\n") {
        return Err("stage100 WebSocket Upgrade header missing".to_owned());
    }
    stream
        .write_all(
            format!(
    ...
    if request.request.target != expected_target {
        return Err(format!(
            "stage100 trojan-go WSS target mismatch: got {}, want {}",
            request.request.target, expected_target
        ));
    }
    if request.request.payload != expected_payload {
        return Err("stage100 trojan-go WSS payload mismatch".to_owned());
    }
    let response = shared_transport::websocket_server_binary_frame(&request.request.payload)
        .map_err(|err| format!("stage100 encode WebSocket response frame failed: {err}"))?;
    stream
        .write_all(&response)
        .map_err(|err| format!("stage100 write trojan-go WSS echo failed: {err}"))?;

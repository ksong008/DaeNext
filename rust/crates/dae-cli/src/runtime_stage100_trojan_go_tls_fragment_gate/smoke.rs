...
#[derive(Debug)]
pub(super) struct Stage100Outcome {
    pub(super) listener_report: TcpLoopbackListenerReport,
    pub(super) last_dial_report: TcpDirectDialReport,
    pub(super) client_report: trojan::TrojanGoWssTcpExchangeReport,
    pub(super) server_summary: TrojanGoTlsFragmentServerSummary,
...
pub(super) fn run_stage100_smoke(opts: &Stage100Options) -> Result<Stage100Outcome, String> {
    let tls_options = opts
        .tls_options()
        .map_err(|err| format!("stage100 tls options invalid: {err}"))?;
    let fragment_options = opts
        .fragment_options()
        .map_err(|err| format!("stage100 tls fragment options invalid: {err}"))?;
    let material = shared_transport::tls_loopback_material(&tls_options)
        .map_err(|err| format!("stage100 build tls material failed: {err}"))?;
    let certificate_der_len = material.certificate_der_len;
    let (server_addr, listener_report, handle) =
        spawn_trojan_go_tls_fragment_server(opts, &material)?;
    ...
    for _ in 0..opts.benchmark_iters {
        ...
        .map_err(|err| format!("stage100 magic_tcp_connect failed: {err}"))?;
        ...
        .map_err(|err| format!("stage100 trojan-go TLS fragment WSS exchange failed: {err}"))?;
        if report.echoed_payload != opts.payload {
            return Err("stage100 trojan-go TLS fragment payload response mismatch".to_owned());
        }
    ...
    let server_summary = handle
        .join()
        .map_err(|_| "stage100 trojan-go TLS fragment server thread panicked".to_owned())??;
    if server_summary.accepted != opts.benchmark_iters {
        return Err(format!(
            "stage100 trojan-go TLS fragment server accepted {} connections, want {}",
    ...
    Ok(Stage100Outcome {
        ...
        last_dial_report: last_dial_report
            .ok_or_else(|| "stage100 missing underlay dial report".to_owned())?,
        client_report: last_client_report
            .ok_or_else(|| "stage100 missing trojan-go TLS fragment client report".to_owned())?,
        server_summary,
...
pub(super) fn apply_stage100_outcome(report: &mut Value, outcome: Stage100Outcome) {
    ...
    let client_complete = outcome.client_report.true_dataplane
        && outcome.client_report.trojan_go_wss
    ...
    report["trojan_go_tls_fragment_smoke_passed"] = json!(passed);
    report["trojan_go_tls_fragment_admitted"] = json!(passed);
    report["trojan_go_shared_transport_partial_admitted"] = json!(passed);
    report["tls_fragment_contract"]["server"] = json!(outcome.last_dial_report.peer_addr);
    report["tls_fragment_contract"]["selected_alpn"] = json!(outcome.client_report.selected_alpn);
    report["tls_fragment_contract"]["certificate_der_len"] = json!(outcome.certificate_der_len);
    report["tls_fragment_contract"]["tls_handshake_validated"] = json!(passed);
    report["tls_fragment_contract"]["certificate_chain_validated"] = json!(passed);
    report["tls_fragment_contract"]["server_name_validated"] = json!(passed);
    report["tls_fragment_contract"]["alpn_validated"] = json!(passed);
    report["tls_fragment_contract"]["websocket_upgrade_validated"] = json!(passed);
    report["tls_fragment_contract"]["websocket_binary_frame_validated"] = json!(passed);
    report["tls_fragment_contract"]["password_sha224_validated"] = json!(passed);
    report["tls_fragment_contract"]["tcp_command_validated"] = json!(passed);
    report["tls_fragment_contract"]["target_metadata_validated"] = json!(passed);
    report["tls_fragment_contract"]["payload_roundtrip_validated"] = json!(passed);
    report["tls_fragment_contract"]["fragmented_write_count"] =
        json!(outcome.fragment_stats.fragmented_write_count());
    report["tls_fragment_contract"]["fragment_record_count"] =
        json!(outcome.fragment_stats.total_fragment_records());
    report["tls_fragment_contract"]["handshake_record_fragmented"] =
        json!(outcome.fragment_stats.handshake_record_fragmented());
    report["tls_fragment_contract"]["fragment_payload_lens"] =
        json!(outcome.fragment_stats.fragment_payload_lens());
    report["tls_fragment_contract"]["reassembled_record_matches"] =
    ...
    if let Some(first) = outcome.fragment_stats.first_fragmented_write() {
        report["tls_fragment_contract"]["first_fragmented_write"] = json!({
    ...
    report["benchmark"]["ns_per_trojan_go_tls_fragment_exchange"] = json!(outcome.ns_per_exchange);
    ...
    report["benchmark"]["websocket_response_frame_len"] =
        json!(outcome.client_report.websocket_response_frame_len);
    report["protocol_matrix"]["trojan_go_tls_fragment_admitted"] = json!(passed);

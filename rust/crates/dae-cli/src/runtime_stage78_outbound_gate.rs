...
use dae_outbound::{http_proxy::HttpConnectOptions, vless};
use serde_json::{Value, json};

use crate::runner::RunnerOutput;

const DEFAULT_UUID: &str = "7c12c745-63a5-433d-9e60-022e469b5bd4";
const DEFAULT_TARGET: &str = "stage78-vless-http-target.example:443";
const DEFAULT_HTTP_PROXY_TARGET: &str = "stage78-vless-http-proxy.example:443";
const DEFAULT_HTTP_HOST: &str = "stage78-vless-http.example";
const DEFAULT_HTTP_PATH: &str = "/dae-stage78-http";
const DEFAULT_PAYLOAD: &[u8] = b"stage78-vless-http-put-ping";

pub(crate) fn run_stage78_vless_http_transport_dataplane_admission(
    args: &[String],
) -> RunnerOutput {
    let opts = match Stage78Options::parse(args) {
        Ok(opts) => opts,
        Err(output) => return output,
    };
    let report = stage78_report(&opts);
    let passed = report["vless_http_transport_put_smoke_passed"]
        .as_bool()
        .unwrap_or(false);
...
impl Default for Stage78Options {
    fn default() -> Self {
        Self {
            execute_smoke: false,
            ack_root_gate: false,
    ...
    fn parse(args: &[String]) -> Result<Self, RunnerOutput> {
            ...
            match arg.as_str() {
                ...
                "--benchmark-iters" => {
                    opts.benchmark_iters =
                        parse_usize(&next_value(&mut iter, "stage78 --benchmark-iters")?, arg)?;
                }
                "--uuid" => opts.uuid = next_value(&mut iter, "stage78 --uuid")?,
                "--target" => opts.target = next_value(&mut iter, "stage78 --target")?,
                "--http-proxy-target" => {
                    opts.http_proxy_target = next_value(&mut iter, "stage78 --http-proxy-target")?
                }
                "--http-host" => opts.http_host = next_value(&mut iter, "stage78 --http-host")?,
                "--http-path" => opts.http_path = next_value(&mut iter, "stage78 --http-path")?,
                "--payload" => {
                    opts.payload = next_value(&mut iter, "stage78 --payload")?.into_bytes();
                }
                "--so-mark" => {
                    opts.so_mark = parse_u32(&next_value(&mut iter, "stage78 --so-mark")?, arg)?;
                }
                "--mptcp" => opts.mptcp = true,
                "--no-mptcp" => opts.mptcp = false,
                "--timeout-ms" => {
                    let timeout_ms =
                        parse_u64(&next_value(&mut iter, "stage78 --timeout-ms")?, arg)?;
                ...
                _ => {
                    return Err(RunnerOutput::usage(format!(
                        "unsupported stage78 argument: {arg}"
                    )));
                }
            }
        }
        if opts.benchmark_iters == 0 {
            return Err(RunnerOutput::usage(
                "stage78 --benchmark-iters must be greater than zero",
            ));
        }
        vless::password_to_key(&opts.uuid)
            .map_err(|err| RunnerOutput::usage(format!("stage78 uuid is invalid: {err}")))?;
        dae_outbound::VMessMetadata::parse("tcp", &opts.target)
            .map_err(|err| RunnerOutput::usage(format!("stage78 target is invalid: {err}")))?;
        if opts.http_proxy_target.is_empty() {
            return Err(RunnerOutput::usage(
                "stage78 --http-proxy-target must not be empty",
            ));
        }
        if opts.http_host.is_empty() {
            return Err(RunnerOutput::usage("stage78 --http-host must not be empty"));
        }
...
fn stage78_report(opts: &Stage78Options) -> Value {
    let key = match vless::password_to_key(&opts.uuid) {
        Ok(key) => key,
        Err(err) => {
            return json!({
                "name": "stage78-vless-http-transport-dataplane-admission",
                "stage": "stage78",
                "blocked": true,
                "blockers": [format!("stage78 uuid is invalid: {err}")]
            });
        }
    };
    if let Err(err) = dae_outbound::VMessMetadata::parse("tcp", &opts.target) {
        return json!({
            ...
            "stage": "stage78",
            "blocked": true,
            "blockers": [format!("stage78 target is invalid: {err}")]
        });
    }
    ...
    let mut report = json!({
        "name": "stage78-vless-http-transport-dataplane-admission",
        "stage": "stage78",
        "evidence_class": "opt-in-protocol-vless-http-transport-put-shared-transport-true-dataplane-smoke",
    ...
    report["vless_http_transport_put_smoke_passed"] = json!(false);
    ...
    report["vless_http_transport_contract"] = json!({
        "protocol": "vless",
        "scope": "VLESS TCP request/response carried after HTTP transport PUT 200 over a Rust TCP stream",
        "uuid": opts.uuid,
        ...
        "other_shared_transport_deferred": "VLESS xHTTP/xmux, TLS/REALITY/XTLS Vision, UDP, and full shared transport require separate gates",
    ...
    report["benchmark"] = json!({
        ...
        "elapsed_ns": null,
        "ns_per_vless_http_transport_exchange": null,
        "scope": "VLESS TCP request/response carried after HTTP transport PUT 200 on a SO_MARKed Rust TCP socket",
    ...
    report["remaining_blockers"] = json!([
        ...
        "VLESS TLS/REALITY/XTLS Vision, UDP, xHTTP/xmux, full HTTP/H2 lifecycle, and full shared transport rows are still incomplete",
    ...
    if !opts.ack_root_gate {
        ...
        report["blockers"] = json!([
            "stage78 root-gated smoke requires --ack-root-gate because it attempts SO_MARK/MPTCP underlay socket observation"
        ]);
        return report;
    }

    match run_stage78_smoke(opts) {
        Ok(outcome) => apply_stage78_outcome(&mut report, outcome),
        Err(err) => {
            report["blocked"] = json!(true);
...
fn run_stage78_smoke(opts: &Stage78Options) -> Result<Stage78Outcome, String> {
    let key = vless::password_to_key(&opts.uuid)
        .map_err(|err| format!("stage78 uuid is invalid: {err}"))?;
    let (server_addr, listener_report, handle) = spawn_vless_http_transport_server(opts)?;
    ...
    for _ in 0..opts.benchmark_iters {
        ...
        .map_err(|err| format!("stage78 magic_tcp_connect failed: {err}"))?;
        ...
        .map_err(|err| format!("stage78 VLESS HTTP transport exchange failed: {err}"))?;
        if report.echoed_payload != opts.payload {
            return Err("stage78 VLESS HTTP transport payload response mismatch".to_owned());
        }
    ...
    let server_summary = handle
        .join()
        .map_err(|_| "stage78 VLESS HTTP transport server thread panicked".to_owned())??;
    if server_summary.accepted != opts.benchmark_iters {
        return Err(format!(
            "stage78 VLESS HTTP transport server accepted {} connections, want {}",
            server_summary.accepted, opts.benchmark_iters
        ));
    }
    let last_exchange =
        last_exchange.ok_or_else(|| "stage78 missing exchange report".to_owned())?;
    Ok(Stage78Outcome {
        listener_report,
        last_dial_report: last_dial_report
            .ok_or_else(|| "stage78 missing underlay dial report".to_owned())?,
        server_summary,
        elapsed_ns,
...
fn apply_stage78_outcome(report: &mut Value, outcome: Stage78Outcome) {
    ...
    report["vless_http_transport_put_smoke_passed"] = json!(passed);
    report["vless_http_transport_put_admitted"] = json!(passed);
    report["vless_shared_transport_partial_admitted"] = json!(true);
    report["vless_protocol_partial_admitted"] = json!(true);
    report["vless_http_transport_contract"]["server"] = json!(outcome.last_dial_report.peer_addr);
    report["vless_http_transport_contract"]["key_hex"] = json!(outcome.key_hex);
    report["vless_http_transport_contract"]["http_transport_put_validated"] = json!(passed);
    report["vless_http_transport_contract"]["http_status_200_validated"] = json!(passed);
    report["vless_http_transport_contract"]["request_header_validated"] = json!(passed);
    report["vless_http_transport_contract"]["response_header_validated"] = json!(passed);
    report["vless_http_transport_contract"]["empty_addons_validated"] = json!(passed);
    report["vless_http_transport_contract"]["tcp_command_validated"] = json!(passed);
    report["vless_http_transport_contract"]["target_metadata_validated"] = json!(passed);
    report["vless_http_transport_contract"]["payload_roundtrip_validated"] = json!(passed);
    ...
    report["benchmark"]["ns_per_vless_http_transport_exchange"] = json!(outcome.ns_per_exchange);
    ...
    report["protocol_matrix"]["vless_http_transport_put_admitted"] = json!(passed);
}

#[derive(Debug, Default)]
struct VlessHttpTransportServerSummary {
    accepted: usize,
...
fn spawn_vless_http_transport_server(
    opts: &Stage78Options,
) -> Result<
    (
        SocketAddrV4,
        TcpLoopbackListenerReport,
        thread::JoinHandle<Result<VlessHttpTransportServerSummary, String>>,
    ),
    String,
> {
    let (listener, listener_report) = bind_loopback_tcp_listener(opts.mptcp).map_err(|err| {
        format!("stage78 bind loopback VLESS HTTP transport server failed: {err}")
    })?;
    let local_addr = listener
        .local_addr()
        .map_err(|err| format!("stage78 VLESS HTTP transport server local_addr failed: {err}"))?;
    let server_addr = match local_addr {
        SocketAddr::V4(addr) => addr,
        SocketAddr::V6(addr) => {
            return Err(format!(
                "stage78 VLESS HTTP transport listener is not IPv4: {addr}"
            ));
        }
    };
    listener
        .set_nonblocking(true)
        .map_err(|err| format!("stage78 VLESS HTTP transport nonblocking failed: {err}"))?;
...
fn accept_vless_http_transport(
    ...
    timeout: Duration,
) -> Result<VlessHttpTransportServerSummary, String> {
    let mut summary = VlessHttpTransportServerSummary::default();
        ...
        match listener.accept() {
            Ok((mut stream, _)) => {
                stream
                    ...
                    .map_err(|err| format!("stage78 server set read timeout failed: {err}"))?;
                stream
                    .set_write_timeout(Some(timeout))
                    .map_err(|err| format!("stage78 server set write timeout failed: {err}"))?;
                handle_vless_http_transport(
                    &mut stream,
                    uuid,
                    expected_target,
                    expected_payload,
            ...
            Err(err) if err.kind() == ErrorKind::WouldBlock => {
                return Err(format!(
                    "stage78 VLESS HTTP transport server timed out after accepting {} of {} connections",
                    summary.accepted, iterations
                ));
            }
            Err(err) => return Err(format!("stage78 VLESS HTTP transport accept failed: {err}")),
        }
    }
    Ok(summary)
}

fn handle_vless_http_transport(
    stream: &mut TcpStream,
    uuid: &str,
    expected_target: &str,
    expected_payload: &[u8],
    http_options: &HttpConnectOptions,
    summary: &mut VlessHttpTransportServerSummary,
) -> Result<(), String> {
    let expected_key =
        vless::password_to_key(uuid).map_err(|err| format!("stage78 VLESS key failed: {err}"))?;
    let head = vless::read_http_transport_request_head_from_stream(stream, http_options)
        .map_err(|err| format!("stage78 read HTTP transport request failed: {err}"))?;
    stream
        .write_all(b"HTTP/1.1 200 OK\r\nContent-Length: 0\r\n\r\n")
        .map_err(|err| format!("stage78 write HTTP transport 200 failed: {err}"))?;
    let request = vless::read_tcp_request_from_stream(stream, expected_payload.len())
        .map_err(|err| format!("stage78 read VLESS HTTP transport request failed: {err}"))?;
    if request.key != expected_key {
        return Err("stage78 VLESS key mismatch".to_owned());
    }
    if request.addons_len != 0 {
        return Err(format!(
            "stage78 VLESS addons length mismatch: got {}, want 0",
            request.addons_len
        ));
    }
    if request.command != dae_outbound::VMessNetwork::Tcp.byte() {
        return Err(format!(
            "stage78 VLESS command mismatch: got {}, want {}",
            request.command,
    ...
    if request.target != expected_target {
        return Err(format!(
            "stage78 VLESS target mismatch: got {}, want {expected_target}",
            request.target
        ));
    }
    if request.payload != expected_payload {
        return Err("stage78 VLESS HTTP transport payload mismatch".to_owned());
    }
    let response = vless::response_payload_bytes(&request.payload);
    stream
        .write_all(&response)
        .map_err(|err| format!("stage78 write VLESS HTTP transport response failed: {err}"))?;

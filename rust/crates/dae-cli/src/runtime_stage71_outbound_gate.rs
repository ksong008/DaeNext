...
const DEFAULT_TARGET: &str = "stage71-vmess-grpc.example:443";
const DEFAULT_GRPC_ADDRESS: &str = "stage71-vmess-grpc-proxy.example:443";
const DEFAULT_GRPC_SERVICE_NAME: &str = "dae-stage71-grpc";
const DEFAULT_GRPC_SERVER_NAME: &str = "stage71-vmess-grpc-sni.example";
const DEFAULT_GRPC_DIALER_ID: &str = "stage71-vmess-grpc-dialer";
const DEFAULT_PAYLOAD: &[u8] = b"stage71-vmess-grpc-ping";

pub(crate) fn run_stage71_vmess_grpc_hunk_dataplane_admission(args: &[String]) -> RunnerOutput {
    let opts = match Stage71Options::parse(args) {
        Ok(opts) => opts,
        Err(output) => return output,
    };
    let report = stage71_report(&opts);
    let passed = report["vmess_grpc_hunk_smoke_passed"]
        .as_bool()
        .unwrap_or(false);
...
impl Default for Stage71Options {
    fn default() -> Self {
        Self {
            execute_smoke: false,
            ack_root_gate: false,
    ...
    fn parse(args: &[String]) -> Result<Self, RunnerOutput> {
            ...
            match arg.as_str() {
                ...
                "--benchmark-iters" => {
                    opts.benchmark_iters =
                        parse_usize(&next_value(&mut iter, "stage71 --benchmark-iters")?, arg)?;
                }
                "--uuid" => opts.uuid = next_value(&mut iter, "stage71 --uuid")?,
                "--target" => opts.target = next_value(&mut iter, "stage71 --target")?,
                "--grpc-address" => {
                    opts.grpc_address = next_value(&mut iter, "stage71 --grpc-address")?
                }
                "--grpc-service-name" => {
                    opts.grpc_service_name = next_value(&mut iter, "stage71 --grpc-service-name")?
                }
                "--grpc-server-name" => {
                    opts.grpc_server_name = next_value(&mut iter, "stage71 --grpc-server-name")?
                }
                "--grpc-dialer-id" => {
                    opts.grpc_dialer_id = next_value(&mut iter, "stage71 --grpc-dialer-id")?
                }
                "--allow-insecure" => opts.allow_insecure = true,
                "--no-allow-insecure" => opts.allow_insecure = false,
                "--payload" => {
                    opts.payload = next_value(&mut iter, "stage71 --payload")?.into_bytes();
                }
                "--so-mark" => {
                    opts.so_mark = parse_u32(&next_value(&mut iter, "stage71 --so-mark")?, arg)?;
                }
                "--mptcp" => opts.mptcp = true,
                "--no-mptcp" => opts.mptcp = false,
                "--timeout-ms" => {
                    let timeout_ms =
                        parse_u64(&next_value(&mut iter, "stage71 --timeout-ms")?, arg)?;
                ...
                _ => {
                    return Err(RunnerOutput::usage(format!(
                        "unsupported stage71 argument: {arg}"
                    )));
                }
            }
        }
        if opts.benchmark_iters == 0 {
            return Err(RunnerOutput::usage(
                "stage71 --benchmark-iters must be greater than zero",
            ));
        }
        vmess::vmess_cmd_key_from_uuid(&opts.uuid)
            .map_err(|err| RunnerOutput::usage(format!("stage71 uuid is invalid: {err}")))?;
        dae_outbound::VMessMetadata::parse("tcp", &opts.target)
            .map_err(|err| RunnerOutput::usage(format!("stage71 target is invalid: {err}")))?;
        if opts.grpc_address.is_empty() {
            return Err(RunnerOutput::usage(
                "stage71 --grpc-address must not be empty",
            ));
        }
        ...
        if opts.grpc_server_name.is_empty() {
            return Err(RunnerOutput::usage(
                "stage71 --grpc-server-name must not be empty",
            ));
        }
        if opts.grpc_dialer_id.is_empty() {
            return Err(RunnerOutput::usage(
                "stage71 --grpc-dialer-id must not be empty",
            ));
        }
        Ok(opts)
    }
...
fn stage71_report(opts: &Stage71Options) -> Value {
    let cmd_key = match vmess::vmess_cmd_key_from_uuid(&opts.uuid) {
        Ok(key) => key,
        Err(err) => {
            return json!({
                "name": "stage71-vmess-grpc-hunk-dataplane-admission",
                "stage": "stage71",
                "blocked": true,
                "blockers": [format!("stage71 uuid is invalid: {err}")]
            });
        }
    };
    if let Err(err) = dae_outbound::VMessMetadata::parse("tcp", &opts.target) {
        return json!({
            ...
            "stage": "stage71",
            "blocked": true,
            "blockers": [format!("stage71 target is invalid: {err}")]
        });
    }
    ...
    let mut report = json!({
        "name": "stage71-vmess-grpc-hunk-dataplane-admission",
        "stage": "stage71",
        "evidence_class": "opt-in-protocol-vmess-aead-grpc-hunk-shared-transport-dataplane-smoke",
    ...
    report["vmess_grpc_hunk_smoke_passed"] = json!(false);
    report["vmess_grpc_hunk_admitted"] = json!(false);
    ...
    report["vmess_grpc_hunk_contract"] = json!({
        "protocol": "vmess",
        "scope": "VMess AEAD TCP request/response carried by gRPC Tun Hunk payloads over a Rust TCP stream",
        "uuid": opts.uuid,
        ...
        "other_shared_transport_deferred": "VMess WSS, HTTP/H2, Meek, xHTTP, and full gRPC HTTP/2 require separate transport gates",
    ...
    report["benchmark"] = json!({
        ...
        "elapsed_ns": null,
        "ns_per_vmess_grpc_hunk_exchange": null,
        "scope": "VMess AEAD TCP request/response carried by gRPC hunk payloads on a SO_MARKed Rust TCP socket",
    ...
    report["protocol_matrix"] = json!({
        ...
        "vmess_grpc_hunk_admitted": false,
    ...
    if !opts.ack_root_gate {
        ...
        report["blockers"] = json!([
            "stage71 root-gated smoke requires --ack-root-gate because it attempts SO_MARK/MPTCP underlay socket observation"
        ]);
        return report;
    }

    match run_stage71_smoke(opts) {
        Ok(outcome) => apply_stage71_outcome(&mut report, outcome),
        Err(err) => {
            report["blocked"] = json!(true);
...
fn run_stage71_smoke(opts: &Stage71Options) -> Result<Stage71Outcome, String> {
    let (server_addr, listener_report, handle) = spawn_vmess_grpc_hunk_server(opts)?;
    ...
    for _ in 0..opts.benchmark_iters {
        ...
        .map_err(|err| format!("stage71 magic_tcp_connect failed: {err}"))?;
        ...
        .map_err(|err| format!("stage71 VMess gRPC hunk exchange failed: {err}"))?;
        if report.echoed_payload != opts.payload {
            return Err("stage71 VMess gRPC hunk payload response mismatch".to_owned());
        }
    ...
    let server_summary = handle
        .join()
        .map_err(|_| "stage71 VMess gRPC hunk server thread panicked".to_owned())??;
    if server_summary.accepted != opts.benchmark_iters {
        return Err(format!(
            "stage71 VMess gRPC hunk server accepted {} connections, want {}",
            server_summary.accepted, opts.benchmark_iters
        ));
    }
    let last_exchange =
        last_exchange.ok_or_else(|| "stage71 missing exchange report".to_owned())?;
    Ok(Stage71Outcome {
        listener_report,
        last_dial_report: last_dial_report
            .ok_or_else(|| "stage71 missing underlay dial report".to_owned())?,
        server_summary,
        elapsed_ns,
...
fn apply_stage71_outcome(report: &mut Value, outcome: Stage71Outcome) {
    ...
    report["vmess_grpc_hunk_smoke_passed"] = json!(passed);
    report["vmess_grpc_hunk_admitted"] = json!(passed);
    report["vmess_shared_transport_partial_admitted"] = json!(true);
    report["vmess_protocol_partial_admitted"] = json!(true);
    report["vmess_grpc_hunk_contract"]["server"] = json!(outcome.last_dial_report.peer_addr);
    report["vmess_grpc_hunk_contract"]["cmd_key_hex"] = json!(outcome.cmd_key_hex);
    report["vmess_grpc_hunk_contract"]["grpc_cache_key"] = json!(outcome.grpc_cache_key);
    report["vmess_grpc_hunk_contract"]["grpc_stream_preface_validated"] = json!(passed);
    report["vmess_grpc_hunk_contract"]["grpc_hunk_tunnel_validated"] = json!(passed);
    report["vmess_grpc_hunk_contract"]["grpc_cache_key_route_context_validated"] = json!(passed);
    report["vmess_grpc_hunk_contract"]["eauth_crc_validated"] = json!(passed);
    report["vmess_grpc_hunk_contract"]["request_header_aead_validated"] = json!(passed);
    report["vmess_grpc_hunk_contract"]["response_header_aead_validated"] = json!(passed);
    report["vmess_grpc_hunk_contract"]["shake128_chunk_masking_validated"] = json!(passed);
    report["vmess_grpc_hunk_contract"]["tcp_command_validated"] = json!(passed);
    report["vmess_grpc_hunk_contract"]["target_metadata_validated"] = json!(passed);
    report["vmess_grpc_hunk_contract"]["payload_roundtrip_validated"] = json!(passed);
    ...
    report["benchmark"]["ns_per_vmess_grpc_hunk_exchange"] = json!(outcome.ns_per_exchange);
    ...
    report["protocol_matrix"]["vmess_grpc_hunk_admitted"] = json!(passed);
}

#[derive(Debug, Default)]
struct VMessGrpcHunkServerSummary {
    accepted: usize,
...
fn spawn_vmess_grpc_hunk_server(
    opts: &Stage71Options,
) -> Result<
    (
        SocketAddrV4,
        TcpLoopbackListenerReport,
        thread::JoinHandle<Result<VMessGrpcHunkServerSummary, String>>,
    ),
    String,
> {
    let (listener, listener_report) = bind_loopback_tcp_listener(opts.mptcp)
        .map_err(|err| format!("stage71 bind loopback VMess gRPC hunk server failed: {err}"))?;
    let local_addr = listener
        .local_addr()
        .map_err(|err| format!("stage71 VMess gRPC hunk server local_addr failed: {err}"))?;
    let server_addr = match local_addr {
        SocketAddr::V4(addr) => addr,
        SocketAddr::V6(addr) => {
            return Err(format!(
                "stage71 VMess gRPC hunk listener is not IPv4: {addr}"
            ));
        }
    };
    listener
        .set_nonblocking(true)
        .map_err(|err| format!("stage71 VMess gRPC hunk nonblocking failed: {err}"))?;
...
fn accept_vmess_grpc_hunk(
    ...
    expected_payload: &[u8],
    timeout: Duration,
) -> Result<VMessGrpcHunkServerSummary, String> {
    let mut summary = VMessGrpcHunkServerSummary::default();
        ...
        match listener.accept() {
            Ok((mut stream, _)) => {
                stream
                    ...
                    .map_err(|err| format!("stage71 server set read timeout failed: {err}"))?;
                stream
                    .set_write_timeout(Some(timeout))
                    .map_err(|err| format!("stage71 server set write timeout failed: {err}"))?;
                handle_vmess_grpc_hunk(
                    &mut stream,
                    uuid,
                    expected_target,
            ...
            Err(err) if err.kind() == ErrorKind::WouldBlock => {
                return Err(format!(
                    "stage71 VMess gRPC hunk server timed out after accepting {} of {} connections",
                    summary.accepted, iterations
                ));
            }
            Err(err) => return Err(format!("stage71 VMess gRPC hunk accept failed: {err}")),
        }
    }
    Ok(summary)
}

fn handle_vmess_grpc_hunk(
    stream: &mut TcpStream,
    uuid: &str,
    expected_target: &str,
    expected_service_name: &str,
    expected_payload: &[u8],
    summary: &mut VMessGrpcHunkServerSummary,
) -> Result<(), String> {
    ...
    stream
        ...
        .map_err(|err| format!("stage71 read gRPC hunk preface failed: {err}"))?;
    if preface != expected_preface {
        return Err("stage71 gRPC hunk stream preface mismatch".to_owned());
    }
    let request = vmess::read_aead_tcp_request_from_grpc_hunk_stream(stream, uuid)
        .map_err(|err| format!("stage71 read VMess gRPC hunk request failed: {err}"))?;
    if !request.request.eauth_crc_validated {
        return Err("stage71 VMess EAuthID checksum was not validated".to_owned());
    }
    if request.request.command != dae_outbound::VMessNetwork::Tcp.byte() {
        return Err(format!(
            "stage71 VMess command mismatch: got {}, want {}",
    ...
    if request.request.target != expected_target {
        return Err(format!(
            "stage71 VMess target mismatch: got {}, want {expected_target}",
            request.request.target
        ));
    }
    if request.request.payload != expected_payload {
        return Err("stage71 VMess gRPC hunk payload mismatch".to_owned());
    }
    let response = vmess::aead_tcp_response_packet(&request.request, &request.request.payload)
        .map_err(|err| format!("stage71 encode VMess gRPC hunk response failed: {err}"))?;
    let response = shared_transport::grpc_hunk_frame(&response)
        .map_err(|err| format!("stage71 encode VMess gRPC hunk frame failed: {err}"))?;
    stream
        .write_all(&response)
        .map_err(|err| format!("stage71 write VMess gRPC hunk echo failed: {err}"))?;

...
use report::stage103_report;
use utils::*;

use crate::runner::RunnerOutput;

const DEFAULT_CIPHER: &str = "aes-128-gcm";
const DEFAULT_SS_PASSWORD: &str = "stage103-inner-ss-password";
const DEFAULT_TROJAN_PASSWORD: &str = "stage103-trojan-password";
const DEFAULT_TARGET: &str = "stage103-trojan-go.example:443";
const DEFAULT_RESPONSE_METADATA_TARGET: &str = "stage103-response.example:8443";
const DEFAULT_TLS_SERVER_NAME: &str = "stage103-trojan-go-combination.example";
const DEFAULT_WS_HOST: &str = "stage103-combo-ws-host.example";
const DEFAULT_WS_PATH: &str = "/trojan-go-combination";
const DEFAULT_PAYLOAD: &[u8] = b"stage103-trojan-go-combination-ping";
const DEFAULT_FRAGMENT_LENGTH: &str = "64-64";
const DEFAULT_FRAGMENT_INTERVAL: &str = "0-0";

pub(crate) fn run_stage103_trojan_go_combination_admission(args: &[String]) -> RunnerOutput {
    let opts = match Stage103Options::parse(args) {
        Ok(opts) => opts,
        Err(output) => return output,
    };
    let report = stage103_report(&opts);
    let passed = report["trojan_go_wss_tls_fragment_inner_shadowsocks_combination_smoke_passed"]
        .as_bool()
        .unwrap_or(false);
...
impl Default for Stage103Options {
    fn default() -> Self {
        Self {
            execute_smoke: false,
            ack_root_gate: false,
            ...
            shadowsocks_password: DEFAULT_SS_PASSWORD.to_owned(),
    ...
    fn parse(args: &[String]) -> Result<Self, RunnerOutput> {
            ...
            match arg.as_str() {
                ...
                "--benchmark-iters" => {
                    opts.benchmark_iters =
                        parse_usize(&next_value(&mut iter, "stage103 --benchmark-iters")?, arg)?;
                }
                "--cipher" => opts.cipher = next_value(&mut iter, "stage103 --cipher")?,
                "--ss-password" => {
                    opts.shadowsocks_password = next_value(&mut iter, "stage103 --ss-password")?;
                }
                "--shadowsocks-password" => {
                    opts.shadowsocks_password =
                        next_value(&mut iter, "stage103 --shadowsocks-password")?;
                }
                "--trojan-password" => {
                    opts.trojan_password = next_value(&mut iter, "stage103 --trojan-password")?;
                }
                "--target" => opts.target = next_value(&mut iter, "stage103 --target")?,
                "--response-metadata-target" => {
                    opts.response_metadata_target =
                        next_value(&mut iter, "stage103 --response-metadata-target")?;
                }
                "--tls-server-name" => {
                    opts.tls_server_name = next_value(&mut iter, "stage103 --tls-server-name")?;
                }
                "--alpn" => opts.alpn_protocol = next_value(&mut iter, "stage103 --alpn")?,
                "--ws-host" => opts.ws_host = next_value(&mut iter, "stage103 --ws-host")?,
                "--ws-path" => opts.ws_path = next_value(&mut iter, "stage103 --ws-path")?,
                "--payload" => {
                    opts.payload = next_value(&mut iter, "stage103 --payload")?.into_bytes();
                }
                "--fragment-length" => {
                    opts.fragment_length = next_value(&mut iter, "stage103 --fragment-length")?;
                }
                "--fragment-interval" => {
                    opts.fragment_interval = next_value(&mut iter, "stage103 --fragment-interval")?;
                }
                "--so-mark" => {
                    opts.so_mark = parse_u32(&next_value(&mut iter, "stage103 --so-mark")?, arg)?;
                }
                "--mptcp" => opts.mptcp = true,
                "--no-mptcp" => opts.mptcp = false,
                "--timeout-ms" => {
                    let timeout_ms =
                        parse_u64(&next_value(&mut iter, "stage103 --timeout-ms")?, arg)?;
                ...
                _ if arg.starts_with("--ss-password=") => {
                ...
                _ => {
                    return Err(RunnerOutput::usage(format!(
                        "unsupported stage103 argument: {arg}"
                    )));
                }
            }
        }
        if opts.benchmark_iters == 0 {
            return Err(RunnerOutput::usage(
                "stage103 --benchmark-iters must be greater than zero",
            ));
        }
        if opts.ws_host.trim().is_empty() {
            return Err(RunnerOutput::usage("stage103 --ws-host must not be empty"));
        }
        ...
        trojan::TrojanMetadata::parse("tcp", &opts.target)
            .map_err(|err| RunnerOutput::usage(format!("stage103 target is invalid: {err}")))?;
        shadowsocks::ShadowsocksMetadata::parse(&opts.response_metadata_target).map_err(|err| {
            RunnerOutput::usage(format!(
                "stage103 response metadata target is invalid: {err}"
            ))
        })?;
        shadowsocks::cipher_spec(&opts.cipher)
            .map_err(|err| RunnerOutput::usage(format!("stage103 cipher invalid: {err}")))?;
        opts.tls_options()
            .map_err(|err| RunnerOutput::usage(format!("stage103 tls options invalid: {err}")))?;
        opts.fragment_options().map_err(|err| {
            RunnerOutput::usage(format!("stage103 tls fragment options invalid: {err}"))
        })?;
        Ok(opts)
    }

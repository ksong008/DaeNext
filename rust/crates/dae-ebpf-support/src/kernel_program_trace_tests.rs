...
fn trace_event_abi_contract_matches_c_ringbuf_record_shape() {
    ...
    assert_eq!(abi.events_c_default_bytes, 1 << 29);
    assert_eq!(abi.events_go_default_bytes, 64 << 20);
...
#[test]
fn trace_config_rewrite_contract_keeps_go_endian_and_ringbuf_semantics() {
...
fn trace_kprobe_contract_covers_skb_argument_positions_and_lifetime_cleanup() {
    ...
    for position in 1..=5 {
        assert!(programs.iter().any(|program| {
            program.c_section == format!("kprobe/skb-{position}").as_str()
    ...
    assert!(programs.iter().any(|program| {
        program.c_section == "kprobe/skb_lifetime_termination"

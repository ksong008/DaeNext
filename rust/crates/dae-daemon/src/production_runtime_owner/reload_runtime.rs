...
pub(super) struct ReloadRuntimeEvidence {
    ...
    pub(super) invalid_config_rollback_verified: bool,
...
pub(super) fn run_reload_runtime_parity_probe(
    ...
    evidence.rollback = rollback_guard_value(handoff, options);
    evidence.invalid_config_rollback_verified = evidence.rollback["status"].as_str()
        == Some("pass")
        && evidence.rollback["current_owner_preserved_on_failure"]
            .as_bool()
            .unwrap_or(false);
    ...
    evidence.passed = evidence.live_reload_executed
        ...
        && evidence.runtime_overview_parity_verified
        && evidence.reload_scoped_resources_flushed
        && evidence.invalid_config_rollback_verified
...
fn dns_cache_migration_guard() -> Value {
        ...
        "entries": [
            {
                ...
                "domain_routing_owner_key": "example.com.|A|IN",
    ...
    json!({
        ...
        "rollback_does_not_leak_cache_into_changed_dns_config": !changed_restore,
    })
}
...
fn rollback_guard_value(
    ...
    json!({
        ...
        "listener_identity_after_rollback": listener,

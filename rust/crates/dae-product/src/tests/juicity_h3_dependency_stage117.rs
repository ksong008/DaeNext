...
#[test]
fn stage117_juicity_h3_dependency_admission_gate_matches_golden_fixture() {
    let fixture = load("product/daemon/stage117_juicity_h3_dependency_admission_gate.json");
    let contract = stage117_juicity_h3_dependency_admission_gate_contract();
    ...
    assert_eq!(
        contract.quinn_dependency_available,
        fixture["quinn_dependency_available"].as_bool().unwrap()
    );
    assert_eq!(
        contract.h3_dependency_available,
        fixture["h3_dependency_available"].as_bool().unwrap()
    );
    assert_eq!(
        contract.h3_quinn_dependency_available,
        fixture["h3_quinn_dependency_available"].as_bool().unwrap()
    );
    assert_eq!(
        contract.tokio_quic_runtime_admitted,
        fixture["tokio_quic_runtime_admitted"].as_bool().unwrap()
    );
    assert_eq!(
        contract.juicity_h3_loopback_dependency_admitted,
        fixture["juicity_h3_loopback_dependency_admitted"]
            .as_bool()
            .unwrap()
    );
    assert_eq!(
...
#[test]
fn stage117_juicity_h3_dependency_admission_keeps_live_h3_and_defaults_closed() {
    let contract = stage117_juicity_h3_dependency_admission_gate_contract();
    ...
    assert!(contract.quinn_runtime_tokio_feature_admitted);
    assert!(contract.quinn_rustls_aws_lc_rs_feature_admitted);
    assert!(contract.h3_quinn_bridge_admitted);
    ...
    assert_eq!(contract.quinn_version, "0.11.9");
    assert_eq!(contract.h3_version, "0.0.8");
    assert_eq!(contract.h3_quinn_version, "0.0.10");
    assert_eq!(contract.tokio_version, "1.52.3");
    ...
    assert_contains_text(&contract.remaining_blockers, "H3 loopback");
    assert_contains_text(&contract.validation_commands, "stage117");
}

...
#[test]
fn stage87_trojan_go_inner_shadowsocks_gate_matches_golden_fixture() {
    let fixture = load("product/daemon/stage87_trojan_go_inner_shadowsocks_dataplane_gate.json");
    let contract = stage87_trojan_go_inner_shadowsocks_gate_contract();
...
#[test]
fn stage87_trojan_go_inner_shadowsocks_gate_keeps_full_trojan_go_and_defaults_closed() {
    let contract = stage87_trojan_go_inner_shadowsocks_gate_contract();
    ...
    assert!(contract.gate_decision.contains("IsClient=false"));

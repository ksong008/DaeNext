...
#[test]
fn stage108_quic_h3_family_queue_gate_matches_golden_fixture() {
    let fixture = load("product/daemon/stage108_quic_h3_family_blocker_queue_gate.json");
    let contract = stage108_quic_h3_family_queue_gate_contract();
...
#[test]
fn stage108_quic_h3_family_queue_keeps_dependency_and_defaults_closed() {
    let contract = stage108_quic_h3_family_queue_gate_contract();
    ...
    assert_contains_text(&contract.remaining_blockers, "Hysteria2 port hopping");
    assert_contains_text(&contract.remaining_blockers, "TUIC TLS1.3");
    assert_contains_text(&contract.remaining_blockers, "Juicity H3");
    assert_contains_text(&contract.validation_commands, "stage108");
}

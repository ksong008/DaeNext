...
#[test]
fn stage111_tuic_full_quic_queue_gate_matches_golden_fixture() {
    let fixture = load("product/daemon/stage111_tuic_full_quic_client_blocker_queue_gate.json");
    let contract = stage111_tuic_full_quic_queue_gate_contract();
    ...
    assert_eq!(
        contract.tuic_udp_relay_mode_go_parity_caveat_recorded,
        fixture["tuic_udp_relay_mode_go_parity_caveat_recorded"]
            .as_bool()
            .unwrap()
    );
    assert_eq!(
...
#[test]
fn stage111_tuic_full_quic_queue_keeps_defaults_closed() {
    let contract = stage111_tuic_full_quic_queue_gate_contract();
    ...
    assert_contains_text(&contract.validation_commands, "stage111");
}

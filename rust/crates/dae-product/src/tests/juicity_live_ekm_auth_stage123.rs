...
#[test]
fn stage123_juicity_live_ekm_auth_gate_matches_golden_fixture() {
    let fixture = load("product/daemon/stage123_juicity_live_ekm_auth_gate.json");
    let contract = stage123_juicity_live_ekm_auth_gate_contract();
    ...
    assert_eq!(
        contract.juicity_auth_token_live_ekm_admitted,
        fixture["juicity_auth_token_live_ekm_admitted"]
            .as_bool()
            .unwrap()
    );
    assert_eq!(
        contract.juicity_live_ekm_auth_stream_transcript_admitted,
        fixture["juicity_live_ekm_auth_stream_transcript_admitted"]
            .as_bool()
            .unwrap()
    );
    assert_eq!(
...
#[test]
fn stage123_juicity_live_ekm_auth_keeps_packet_relay_and_defaults_closed() {
    let contract = stage123_juicity_live_ekm_auth_gate_contract();
    assert!(contract.stage_complete);
    assert!(contract.juicity_live_auth_uni_stream_harness_admitted);
    assert!(contract.juicity_auth_token_live_ekm_admitted);
    assert!(contract.juicity_live_ekm_auth_header_admitted);
    assert!(contract.juicity_live_ekm_auth_stream_transcript_admitted);
    ...
    assert_contains_text(&contract.validation_commands, "stage123");
}

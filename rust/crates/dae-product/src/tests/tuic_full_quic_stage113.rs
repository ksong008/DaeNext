...
#[test]
fn stage113_tuic_full_quic_queue_gate_matches_golden_fixture() {
    let fixture = load("product/daemon/stage113_tuic_full_quic_client_blocker_queue_gate.json");
    let contract = stage113_tuic_full_quic_queue_gate_contract();
...
#[test]
fn stage113_tuic_full_quic_queue_keeps_defaults_closed() {
    let contract = stage113_tuic_full_quic_queue_gate_contract();
    ...
    assert!(!contract.tuic_tls_quic_cert_verification_admitted);
    ...
    assert_contains_text(&contract.validation_commands, "stage113");
}

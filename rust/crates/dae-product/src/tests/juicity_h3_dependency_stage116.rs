...
#[test]
fn stage116_juicity_h3_dependency_readiness_gate_matches_golden_fixture() {
    let fixture = load("product/daemon/stage116_juicity_h3_dependency_readiness_gate.json");
    let contract = stage116_juicity_h3_dependency_readiness_gate_contract();
    ...
    assert_eq!(
        contract.rustls_dependency_available,
        fixture["rustls_dependency_available"].as_bool().unwrap()
    );
    assert_eq!(
...
#[test]
fn stage116_juicity_h3_dependency_readiness_keeps_live_h3_and_defaults_closed() {
    let contract = stage116_juicity_h3_dependency_readiness_gate_contract();
    assert!(contract.stage_complete);
    assert!(contract.rustls_dependency_available);
    assert!(contract.rcgen_dependency_available);
    ...
    assert_contains_text(&contract.remaining_blockers, "H3/QUIC dependencies");
    assert_contains_text(&contract.validation_commands, "stage116");
}

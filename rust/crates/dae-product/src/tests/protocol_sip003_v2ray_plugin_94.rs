...
#[test]
fn stage94_sip003_v2ray_plugin_gate_matches_golden_fixture() {
    let fixture = load("product/daemon/stage94_sip003_v2ray_plugin_dataplane_gate.json");
    let contract = stage94_sip003_v2ray_plugin_gate_contract();
    ...
    assert_eq!(
        contract.sip003_v2ray_plugin_admitted,
        fixture["sip003_v2ray_plugin_admitted"].as_bool().unwrap()
    );
    assert_eq!(
...
#[test]
fn stage94_sip003_v2ray_plugin_gate_opens_plugin_but_keeps_ssr_and_defaults_closed() {
    let contract = stage94_sip003_v2ray_plugin_gate_contract();
    ...
    assert!(contract.gate_decision.contains("v2ray-plugin TLS/WS/mux"));
    assert_contains_text(&contract.carried_blockers, "ShadowsocksR");

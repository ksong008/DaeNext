#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Stage151ControlPlaneOwnerPreflightGateContract {
    pub name: &'static str,
    pub stage: &'static str,
    pub prior_gate: &'static str,
    pub stage_complete: bool,
    pub rust_daemon_lifecycle_smoke_passed: bool,
    pub rust_control_plane_owner_preflight_recorded: bool,
    pub rust_control_plane_owner_smoke_passed: bool,
    pub control_plane_startup_sequence_recorded: bool,
    pub control_plane_reload_owner_sequence_recorded: bool,
    pub control_plane_rollback_sequence_recorded: bool,
    pub listener_reuse_contract_recorded: bool,
    pub bpf_owner_transfer_contract_recorded: bool,
    pub dns_cache_migration_guard_recorded: bool,
    pub reload_scoped_flush_after_current_swap_recorded: bool,
    pub production_listener_bound: bool,
    pub ebpf_attached: bool,
    pub rust_default_run_entrypoint_exists: bool,
    pub rust_default_control_plane_entrypoint_admitted: bool,
    pub benchmark_executable_now: bool,
    pub matched_go_rust_default_daemon_benchmark_recorded: bool,
    pub true_rust_default_daemon_admitted: bool,
    pub default_switch_allowed: bool,
    pub product_chain_switch_allowed: bool,
    pub go_default_path_preserved: bool,
    pub gate_decision: &'static str,
    pub rows: Vec<Stage151ControlPlaneOwnerPreflightGateRow>,
    pub validation_commands: Vec<&'static str>,
    pub remaining_blockers: Vec<&'static str>,
    pub source: Vec<&'static str>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Stage151ControlPlaneOwnerPreflightGateRow {
    pub area: &'static str,
    pub status: &'static str,
    pub evidence: &'static str,
    pub boundary: &'static str,
    pub next_action: &'static str,
}

pub fn stage151_control_plane_owner_preflight_gate_contract()
-> Stage151ControlPlaneOwnerPreflightGateContract {
    Stage151ControlPlaneOwnerPreflightGateContract {
        name: "stage151-rust-control-plane-owner-preflight-gate",
        stage: "stage151",
        prior_gate: "stage150-rust-daemon-lifecycle-smoke-gate",
        stage_complete: true,
        rust_daemon_lifecycle_smoke_passed: true,
        rust_control_plane_owner_preflight_recorded: true,
        rust_control_plane_owner_smoke_passed: true,
        control_plane_startup_sequence_recorded: true,
        control_plane_reload_owner_sequence_recorded: true,
        control_plane_rollback_sequence_recorded: true,
        listener_reuse_contract_recorded: true,
        bpf_owner_transfer_contract_recorded: true,
        dns_cache_migration_guard_recorded: true,
        reload_scoped_flush_after_current_swap_recorded: true,
        production_listener_bound: false,
        ebpf_attached: false,
        rust_default_run_entrypoint_exists: false,
        rust_default_control_plane_entrypoint_admitted: false,
        benchmark_executable_now: false,
        matched_go_rust_default_daemon_benchmark_recorded: false,
        true_rust_default_daemon_admitted: false,
        default_switch_allowed: false,
        product_chain_switch_allowed: false,
        go_default_path_preserved: true,
        gate_decision: "stage151 admits only an opt-in Rust control-plane owner preflight: startup, reload success, rollback, listener reuse, BPF owner transfer, DNS cache migration guard, and reload-scoped flush ordering are recorded under isolated paths, while production listener binding, eBPF attach, default run, benchmark, default switch, and product switch remain closed",
        rows: vec![
            Stage151ControlPlaneOwnerPreflightGateRow {
                area: "startup owner sequence",
                status: "recorded-opt-in-preflight",
                evidence: "Stage151 records config/bootstrap/wait-network/subscription/control-plane/listener-ready/on-ready order",
                boundary: "the preflight does not start production tproxy, DNS listener, or outbound traffic",
                next_action: "wire isolated signal and progress flow before any default run admission",
            },
            Stage151ControlPlaneOwnerPreflightGateRow {
                area: "reload owner sequence",
                status: "recorded-opt-in-preflight",
                evidence: "Stage151 records old BPF eject, guarded DNS cache migration, next build/inject, current swap, old close, reload scoped flush, and listener reuse",
                boundary: "the sequence is an owner contract, not a live eBPF attach or production listener reuse",
                next_action: "prove the sequence through an isolated daemon signal smoke",
            },
            Stage151ControlPlaneOwnerPreflightGateRow {
                area: "rollback owner sequence",
                status: "recorded-opt-in-preflight",
                evidence: "Stage151 records failed next build, rollback build old config, BPF return to old owner, DNS listener restart, and current remains old",
                boundary: "rollback is not yet validated against production control-plane resources",
                next_action: "add injected-failure smoke before matched default daemon benchmark",
            },
            Stage151ControlPlaneOwnerPreflightGateRow {
                area: "default/product admission",
                status: "closed",
                evidence: "Go default path is preserved and product switches remain false",
                boundary: "control-plane owner preflight is not true default daemon admission",
                next_action: "keep matched benchmark blocked until default run and control-plane admission exist",
            },
        ],
        validation_commands: vec![
            "cargo run --manifest-path rust/Cargo.toml -p dae-daemon --bin dae-daemon-optin -- stage151-control-plane-owner-preflight --root /tmp/dae-stage151-control-plane-owner-preflight",
            "python3 -m json.tool testdata/rebuild-golden/engine/runtime_stage151/rust_control_plane_owner_preflight_gate.json",
            "python3 -m json.tool testdata/rebuild-golden/product/daemon/stage151_rust_control_plane_owner_preflight_gate.json",
            "cargo run --manifest-path rust/Cargo.toml -p dae-cli --bin dae-cli-optin --quiet -- runtime stage151-rust-control-plane-owner-preflight-gate",
            "cargo run --manifest-path rust/Cargo.toml -p dae-cli --bin dae-cli-optin --quiet -- runtime stage151-rust-control-plane-owner-preflight-gate --execute-smoke --root /tmp/dae-stage151-cli-control-plane-owner",
            "cargo test --manifest-path rust/Cargo.toml -p dae-daemon stage151 -- --nocapture",
            "cargo test --manifest-path rust/Cargo.toml -p dae-cli stage151 -- --nocapture",
            "cargo test --manifest-path rust/Cargo.toml -p dae-product stage151 -- --nocapture",
            "cargo test --manifest-path rust/Cargo.toml -p dae-cli stage150 -- --nocapture",
            "cargo test --manifest-path rust/Cargo.toml -p dae-daemon -p dae-cli -p dae-product -q",
            "cargo fmt --manifest-path rust/Cargo.toml --all -- --check",
            "git diff --check",
        ],
        remaining_blockers: vec![
            "Stage151 records control-plane ownership only as an opt-in isolated preflight",
            "Rust default run entrypoint remains absent",
            "production tproxy/eBPF/control-plane traffic is not started",
            "matched Go/Rust default daemon benchmark remains blocked",
            "default daemon and product-chain switches remain closed",
        ],
        source: vec![
            "DAEX_RUST_REBUILD_PLAN_2026-05-16.md:stage151",
            "DAENEW_RUST_REBUILD_MEMO_2026-05-16.md:15.3",
            "DAENEW_RUST_REBUILD_MEMO_2026-05-16.md:15.4",
            "DAENEW_RUST_REBUILD_MEMO_2026-05-16.md:15.5",
            "rust/crates/dae-daemon/src/control_plane.rs",
            "rust/crates/dae-control/src/reload.rs",
        ],
    }
}

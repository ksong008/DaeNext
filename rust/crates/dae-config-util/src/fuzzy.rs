use crate::{GoDuration, UrlOrEmpty};
...
impl FuzzyDecode for GoDuration {
    fn fuzzy_decode(input: &str) -> Option<Self> {
        input.parse().ok()
    }
}
...
impl FuzzyDecode for Vec<GoDuration> {
    fn fuzzy_decode(input: &str) -> Option<Self> {
        ...
        $(
            impl FuzzyDecode for $ty {
                fn fuzzy_decode(input: &str) -> Option<Self> {
                    ...
                    let value = parse_go_i128_base0(input, bits)?;
    ...
    ($($ty:ty),* $(,)?) => {
        $(
            impl FuzzyDecode for $ty {
                fn fuzzy_decode(input: &str) -> Option<Self> {
                    let bits = (std::mem::size_of::<$ty>() * 8) as u32;
                    let value = parse_go_u128_base0(input, bits)?;
                    <$ty>::try_from(value).ok()
                }
            }
        )*
...
fn parse_go_i128_base0(input: &str, bits: u32) -> Option<i128> {
    let (negative, digits, radix) = split_go_base0(input)?;
...
fn parse_go_u128_base0(input: &str, bits: u32) -> Option<u128> {
    let (negative, digits, radix) = split_go_base0(input)?;
    if negative {
        return None;
    }
...
fn split_go_base0(input: &str) -> Option<(bool, &str, u32)> {
    if input.is_empty() {
        return None;
    }
...
mod tests {
    ...
    fn assert_duration_case(case: &Value) {
        for input in case["inputs"].as_array().unwrap() {
            let value = input["value"].as_str().unwrap();
            let ok = input["ok"].as_bool().unwrap();
            let got = fuzzy_decode::<GoDuration>(value);
            assert_eq!(got.is_some(), ok);
            if ok {
    ...
    fn assert_duration_slice_case(case: &Value) {
        for input in case["inputs"].as_array().unwrap() {
            let value = input["value"].as_str().unwrap();
            let ok = input["ok"].as_bool().unwrap();
            let got = fuzzy_decode::<Vec<GoDuration>>(value);
            assert_eq!(got.is_some(), ok);
            if ok {
                let got: Vec<_> = got
                    .unwrap()

...
#[test]
fn stage85_trojan_go_httpupgrade_tcp_dataplane_echoes_payload() {
    let tls_options = shared_transport::TlsUnderlayOptions::new(
        "stage85-trojan-go-httpupgrade.example",
    ...
    let password = "stage85-password";
    let target = "stage85-trojan-go.example:443";
    let httpupgrade_host = "stage85-upgrade-host.example";
    let httpupgrade_path = "/trojan-go-upgrade";
    let payload = b"stage85-trojan-go-httpupgrade-ping".to_vec();
    ...
    assert!(report.trojan_go_httpupgrade);

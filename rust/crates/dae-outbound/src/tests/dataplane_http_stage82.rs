...
#[test]
fn stage82_https_proxy_tls_connect_dataplane_echoes_payload() {
    ...
    let target = "stage82-target.example:443";
    let authority = "front.stage82.example:443";
    let expected_auth = http_proxy::request::basic_auth_header("user", "pass").unwrap();
    let payload = b"stage82-https-proxy-tls-ping".to_vec();

...
#[test]
fn stage137_vless_xhttp_h3_lifecycle_roundtrips_tcp_payload() {
    let key = vless::password_to_key("7c12c745-63a5-433d-9e60-022e469b5bd4").unwrap();
    let target = "stage137-vless-xhttp-h3-target.example:443";
    let payload = b"stage137-vless-xhttp-h3-ping".to_vec();
    let xhttp = stage137_xhttp_options("stage137-vless-xhttp-h3.example", 13701);

    let report = vless::tcp_exchange_over_xhttp_h3_loopback(
        "stage137-vless-xhttp-h3-proxy.example:443",
        &key,
        target,
        &xhttp,
        &payload,
    ...
    assert!(
        report
            .xhttp_request_path
            .contains("dae-stage137-xhttp-h3-session")
    );
...
#[test]
fn stage137_vmess_xhttp_h3_lifecycle_roundtrips_aead_tcp_payload() {
    let uuid = "7c12c745-63a5-433d-9e60-022e469b5bd4";
    let target = "stage137-vmess-xhttp-h3-target.example:443";
    let payload = b"stage137-vmess-xhttp-h3-ping".to_vec();
    let xhttp = stage137_xhttp_options("stage137-vmess-xhttp-h3.example", 13702);

    let report = vmess::aead_tcp_exchange_over_xhttp_h3_loopback(
        "stage137-vmess-xhttp-h3-proxy.example:443",
        uuid,
        target,
        &xhttp,
        &payload,
...
#[test]
fn stage137_xhttp_h3_gate_rejects_reality_and_non_h3_modes() {
    let reality_h3 = XHttpLifecycleOptions::new(
        "stage137-vless-xhttp-h3.example",
        "/dae-stage137-xhttp-h3",
        "packet-up",
        "reality",
        "h3",
        "dae-stage137-xhttp-h3-session",
        13703,
    )
    .unwrap_err();
    ...
    let h2_gate = XHttpH3LoopbackOptions::new(
        h2_options,
        b"stage137-request".to_vec(),
        b"stage137-response".to_vec(),
        1,
        Duration::from_secs(8),
    )
    .unwrap_err();
...
fn stage137_xhttp_options(host: &str, seq: u64) -> XHttpLifecycleOptions {

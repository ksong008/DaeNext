...
#[test]
fn stage81_tls_options_reject_empty_server_name_or_alpn() {
    assert!(shared_transport::TlsUnderlayOptions::new("", "http/1.1").is_err());
    assert!(shared_transport::TlsUnderlayOptions::new("stage81.example", "").is_err());
}

#[test]
fn stage81_shared_tls_underlay_echoes_payload_and_validates_alpn() {
    ...
    let payload = b"stage81-shared-tls-underlay-ping".to_vec();

...
#[test]
fn stage98_grpc_cache_cleanup_and_cancellation_stress_matches_go_contract() {
    let base = GrpcLifecycleOptions::new(
        "stage98-grpc-proxy.example:443",
        "GunService",
        "stage98-grpc-sni.example",
        "stage98-dialer",
        true,
        1234,
        true,
    ...
    assert!(report.sample_cache_keys[0].contains("stage98-grpc-proxy.example:443"));
    assert!(report.sample_cache_keys[0].contains("stage98-grpc-sni.example"));

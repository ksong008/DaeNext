...
#[test]
fn stage120_juicity_packet_state_selects_port_zero_and_stream_routes() {
    let port_zero = juicity::select_udp_packet_conn("stage120-zero.example:0").unwrap();
    assert_eq!(
        port_zero.kind,
    ...
    let stream = juicity::select_udp_packet_conn("stage120-stream.example:5353").unwrap();
    assert_eq!(
        stream.kind,
...
#[test]
fn stage120_juicity_dialauth_record_matches_go_pack_order_contract() {
    let record = juicity::build_dialauth_record_for_port_zero("stage120-zero.example:0").unwrap();
...
#[test]
fn stage120_juicity_stream_packet_frame_roundtrips_without_trojan_crlf() {
    let payload = b"stage120-udp-payload";
    let frame = juicity::seal_stream_packet_frame("stage120-stream.example:5353", payload).unwrap();
    let decoded = juicity::decode_stream_packet_frame(&frame.encoded).unwrap();

    assert_eq!(frame.target, "stage120-stream.example:5353");
...
#[test]
fn stage120_juicity_packet_state_smoke_keeps_true_dataplane_closed() {
    let report = juicity::packet_state_smoke(
        "stage120-zero.example:0",
        "stage120-stream.example:5353",
        b"stage120-udp-payload",
    )
    .unwrap();

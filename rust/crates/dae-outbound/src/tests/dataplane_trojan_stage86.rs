...
#[test]
fn stage86_trojan_go_grpc_hunk_does_not_wrap_outer_tls_and_echoes_payload() {
    let password = "stage86-password";
    let target = "stage86-trojan-go-grpc.example:443";
    let service_name = "dae-stage86-grpc";
    let payload = b"stage86-trojan-go-grpc-ping".to_vec();
    ...
    let grpc_options = shared_transport::GrpcLifecycleOptions::new(
        &endpoint,
        service_name,
        "stage86-trojan-go-grpc-sni.example",
        "stage86-trojan-go-grpc-dialer",
        true,
        1234,
        true,
    );
...
#[test]
fn stage86_trojan_go_grpc_service_name_matches_go_fallbacks() {
    assert_eq!(
        trojan::trojan_go_grpc_service_name("explicit-service", "path-service"),
        "explicit-service"
    );
    assert_eq!(
        trojan::trojan_go_grpc_service_name("", "path-service"),
        "path-service"
    );
    assert_eq!(
        trojan::trojan_go_grpc_service_name("", ""),
        trojan::TROJAN_GO_GRPC_DEFAULT_SERVICE_NAME
    );
}

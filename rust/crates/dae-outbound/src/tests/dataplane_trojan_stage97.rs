...
#[test]
fn stage97_grpc_http2_frame_helpers_encode_preface_settings_headers_and_data() {
...
#[test]
fn stage97_trojan_go_grpc_http2_lifecycle_roundtrips_trojanc_payload() {
    let password = "stage97-trojan-password";
    let target = "stage97-trojan-go-grpc-http2.example:443";
    let payload = b"stage97-trojan-go-grpc-http2-ping".to_vec();
    let grpc = GrpcLifecycleOptions::new(
        "stage97-grpc-proxy.example:443",
        "dae-stage97-grpc",
        "stage97-grpc-sni.example",
        "stage97-dialer",
        true,
        1234,
        true,
    ...
    assert!(
        report
            .grpc_cache_key
            .contains("stage97-grpc-proxy.example:443")
    );
    assert!(report.grpc_cache_key.contains("stage97-grpc-sni.example"));
}

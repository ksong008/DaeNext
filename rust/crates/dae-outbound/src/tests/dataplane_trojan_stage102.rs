...
#[test]
fn stage102_reality_session_plaintext_layout_matches_go() {
    let options = stage102_options(shared_transport::RealityAeadAlgorithm::AesGcm);
    let plaintext =
...
#[test]
fn stage102_reality_aead_mutates_session_id_and_hello_raw() {
    let options = stage102_options(shared_transport::RealityAeadAlgorithm::AesGcm);
    let plaintext =
        shared_transport::reality_session_id_plaintext(options.sid, options.unix_seconds);
...
#[test]
fn stage102_reality_chacha_branch_uses_same_nonce_and_different_ciphertext() {
    let aes = stage102_options(shared_transport::RealityAeadAlgorithm::AesGcm);
    let chacha = stage102_options(shared_transport::RealityAeadAlgorithm::ChaCha20Poly1305);
...
#[test]
fn stage102_reality_hello_raw_short_buffer_is_rejected() {

...
#[test]
fn stage136_vless_xhttp_http2_lifecycle_roundtrips_tcp_payload() {
    let key = vless::password_to_key("7c12c745-63a5-433d-9e60-022e469b5bd4").unwrap();
    let target = "stage136-vless-xhttp-h2-target.example:443";
    let payload = b"stage136-vless-xhttp-h2-ping".to_vec();
    let xhttp = stage136_xhttp_options("stage136-vless-xhttp-h2.example", 13601);
    ...
    let report = vless::tcp_exchange_over_xhttp_http2_stream(
        &mut client,
        "stage136-vless-xhttp-h2-proxy.example:443",
        &key,
        target,
        &xhttp,
        &payload,
...
#[test]
fn stage136_vmess_xhttp_http2_lifecycle_roundtrips_aead_tcp_payload() {
    let uuid = "7c12c745-63a5-433d-9e60-022e469b5bd4";
    let target = "stage136-vmess-xhttp-h2-target.example:443";
    let payload = b"stage136-vmess-xhttp-h2-ping".to_vec();
    let xhttp = stage136_xhttp_options("stage136-vmess-xhttp-h2.example", 13602);
    let (mut client, mut server) = UnixStream::pair().unwrap();
    client
        .set_read_timeout(Some(Duration::from_secs(2)))
        .unwrap();
    ...
    let report = vmess::aead_tcp_exchange_over_xhttp_http2_stream(
        &mut client,
        "stage136-vmess-xhttp-h2-proxy.example:443",
        uuid,
        target,
        &xhttp,
        &payload,
...
fn stage136_xhttp_options(host: &str, seq: u64) -> XHttpLifecycleOptions {
    XHttpLifecycleOptions::new(
        host,
        "/dae-stage136-xhttp-h2",
        "packet-up",
        "tls",
        "h2",
        "dae-stage136-xhttp-h2-session",
        seq,
    )
    .unwrap()
}

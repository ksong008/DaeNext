...
#[test]
fn stage92_sip003_simple_obfs_http_wraps_shadowsocks_aead_tcp() {
    let cipher = "aes-128-gcm";
    let password = "stage92-password";
    let target = "stage92-sip003-simple-obfs.example:8443";
    let payload = b"stage92-sip003-simple-obfs-http-ping".to_vec();
    let client_salt = hex_decode("202122232425262728292a2b2c2d2e2f");
    let server_salt = hex_decode("606162636465666768696a6b6c6d6e6f");
    let options = shadowsocks::Sip003SimpleObfsHttpOptions::new("front.example", "abc/");
    ...
    assert_eq!(report.host, "front.example");
...
#[test]
fn stage92_sip003_simple_obfs_http_options_keep_go_defaults() {

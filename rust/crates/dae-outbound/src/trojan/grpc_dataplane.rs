...
pub const TROJAN_GO_GRPC_DEFAULT_SERVICE_NAME: &str = "GunService";

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TrojanGoGrpcTcpExchangeReport {
    pub proxy: String,
    pub target: String,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TrojanGoGrpcRequest {
    pub request: TrojanTcpRequest,
    pub grpc_request_hunk_len: usize,
}

pub fn trojan_go_grpc_service_name(service_name: &str, path: &str) -> String {
    if !service_name.is_empty() {
        service_name.to_owned()
    } else if !path.is_empty() {
        path.to_owned()
    } else {
        TROJAN_GO_GRPC_DEFAULT_SERVICE_NAME.to_owned()
    }
}
...
pub fn tcp_exchange_over_grpc_hunk_stream<S>(
    ...
    payload: &[u8],
) -> Result<TrojanGoGrpcTcpExchangeReport, OutboundError>
where
    S: Read + Write,
{
    ...
    Ok(TrojanGoGrpcTcpExchangeReport {
...
pub fn read_tcp_request_from_grpc_hunk_stream<S>(
    stream: &mut S,
    payload_len: usize,
) -> Result<TrojanGoGrpcRequest, OutboundError>
where
    S: Read,
{
